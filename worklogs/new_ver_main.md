## AGENCYPREP-20260917 Agency Production Promotion Preparation

- **最終判定: `PLATFORM_PROMOTION_HOLD`。** 記録PR [#483](https://github.com/ideal-sol/oripa/pull/483)のfresh CIでdependency advisory baseline／legacy ESLint baselineの2026-09-16期限切れを検出し、security-gate／quality-gateがFAILした。固定Targetも同一baselineを含む。開始時にreadbackした2026-09-16のTarget PASSは現在日付のPASSへ読み替えない。後続追跡用[Issue #484](https://github.com/ideal-sol/oripa/issues/484)を追加したため最終Issueは#484。期限だけの延長／依存修正／gate bypassは行わず、記録PRは未Mergeで保持し、merge-valid self-reviewは未発行とする。Artifact不足だけの途中判定よりこのHOLDを優先する。

- OLD / Platform `/var/www/oripa`のみ。Issue `none`、Risk `R4`、Lane `Strict Change`、Application Runtime Activation `none`、通常Worktree、Branch `chore/AGENCYPREP-20260917-production-package`。開始時GitHub exact `refs/heads/main`／protected flag／origin main／local main／clean checkoutをreadbackし、Base／`PLATFORM_TARGET_SHA`を`a3a38c0a334028ad943da90e7d968250dd5ae42f`へ固定した。以後の記録PRによるmain移動へcandidateを自動追従させない。変更は本Worklogのみ、GitHub transport用exact-path transient policy以外のSource／Migration Allocation Lockは不要。初回の非canonical説明的Task IDはlocal metadata gateが拒否し、既存OPS番号の再利用を避け、GitHub検索で未使用のcanonical `AGENCYPREP-20260917`へ整合した。保護変更0。
- PR #470／472／473／474／475／476／477／479は全てmergedでTargetのancestor。Human指定どおりAGENCY-001～004BのOld Test Acceptanceはclosedとして扱う。既知Production API `f896b38fb5be31b0a893a92316cd011543f233a4`、Admin `f381398f06b0156b0399f5af32707da40199bd62`、DB最高`000072`は参考値だけで、今回現Production確定値とはしない。HotfixとTargetの`V2AdminFreshMfaAuthorizer.php`は同一blob `979fd03cbecf157a29344b1665a0e39242bf32df`、`V2SessionPolicy::currentTime()`共有を維持しcherry-pick 0。
- API参考値→Targetは161 paths。Agency管理／第三Realm／設定／集計／登録・外部認証attribution、SEC-020依存修復に加え、MAIL-20260916の購入日・決済方法・実grantコイン変数／HTML source編集／再帰sanitize、ADMIN-20260916の一覧初期all、SALES-20260916の日別API既存payment_method返却と表示変更も包含する。Payment／Point／Drawの会計mutation追加0。CI／test／ledger／V1 dependency変更とV2 runtime変更を分離した。
- V2 Migration sourceは75本、追加076以降NONE。後続適用順は`2026_09_29_000073_create_v2_agency_foundation.php`→`2026_09_30_000074_add_v2_agency_realm.php`→`2026_10_01_000075_add_v2_external_identity_advertising_candidate.php`。73はAgency／Code／append-only AttributionとMail3件、74は6h/12h SessionとAudit realm／Mail1件、75はnullable immutable external candidate。いずれもforward additive、backfillなし、transaction内DDL／既存table CHECK再検証は強いlockを取り得る。履歴があればdownを拒否するためschema/data保持・forward repairを優先する。Migration予約／作成／OLD・Production適用0。
- Human確定Production Agencyは`https://agent.oripa-z.com`。APIへの`V2_AGENCY_ORIGIN=https://agent.oripa-z.com`、`V2_AGENCY_LOGIN_URL=https://agent.oripa-z.com/login`の明示注入が必要。Storefront `https://oripa-z.com`／Admin `https://admin.oripa-z.com`と既存same-origin APIを維持。Agency browserはrelative `/agency/api/v2`、専用URL build argなし。Origin完全一致／Referer fallback／cross-site拒否／JSON／Agency XSRFを維持し、Session CookieはSecure／HttpOnly／host-only／Path=/／SameSite Strict。Preview ComposeはAgencyキーをwireしないため後続NEW Taskが実runtime definitionをreadbackする。env／Nginx／DNS／TLS変更0。
- Service source依存はAPI／Admin／Agency=`ACTIVATE_REQUIRED`、Identity Mail／SMS／Scheduler=`UNCHANGED`、専用Agency worker=`N/A`。Agency4通知は`DB::afterCommit`→`deliverAgency`→既存smtp/mailgun transportをAPI process内で直接送信し、Identity Mail outboxの4つのuser topicへenqueueしない。別Worker artifactは不要。実Production worker image／mailer／API egressは未readback、実Mail delivery Acceptanceは後続NEW Serverでbounded実施する。
- 固定Targetの[Required Checks](https://github.com/ideal-sol/oripa/actions/runs/35099904690)全5件／CodeQL PASSをlive確認。V2 smokeのunfiltered `phpunit.v2.xml`／`identity_tests=PASS`にAgency login・settings・Admin API・User/Sales集計・code validation・registration/OIDC candidate・mail・Admin/User Session canonical-time suitesが含まれることをsourceから確認した。Admin 247／Agency 11 frontend testsもexact Target CIでPASS。今回新規backend DB suite再実行はせず、既存exact-SHA CI証跡のreadbackと明記する。今回実行は既存Agency／Advertising OpenAPI＋artifact helper43 tests PASS、非Secret Production config／4種mail URL renderer probe16 checks PASS。Human Browser／Provider実送信はNOT RUN。
- Canonical Production ARM64 [Run 35176857257](https://github.com/ideal-sol/oripa/actions/runs/35176857257)、source PR #482／SALES-20260916でAPI＋Adminをfresh native buildした。Artifact `10479206622`、outer `500f230b4ee0178bb0befc2653ab834a3c1ce047b68e8ee68ddc8c7b0768f8c2`、manifest `731bc47dfbd3895e17b3dcf2035696462a62bb2afe15e07508fa87cd023fb860`。OCI config digest／image IDはAPI `sha256:53dad0f61e58a99242b33856b57dd58e882ed48fc2e0fbe40c1b582e40f2db8c`、Admin `sha256:15abb8630fd57c27b6d39bee34eab97ac3b8afd39d72ac89c7fa3b02a2448d70`。双方linux/arm64、source／OCI revision=Target、Docker production target。outer・safe ZIP・inner・OCI・architecture canonical verify PASS、native CI load PASS、OLD host load 0。Artifactは2026-09-24T03:08:22Z expiry、検証済local copyをrepository外に保持する。
- Result=`PLATFORM_SOURCE_READY_ARTIFACT_BLOCKED`。既存Production workflowはAPI/AdminだけでAgency未対応、Agency対応Preview workflowはAMD64で流用禁止。OLD host／Docker／唯一のbuilderはAMD64、arm64 host-checkはexpected mismatch、既存authorized native Agency build経路なし。Workflow／source／QEMU／新Serverを変更せずAgency artifactは未作成。さらにAPI archiveの`config/v2_agency.php`は旧Test URL fallbackを保持しtest sourceも同梱するため、literal「Test URL contamination 0」はPASSとしない。Production login overrideのsource probeはPASSだが実runtime readbackは未実施。Admin runtime assetの旧Domain／旧Port match 0、実.env file 0。Api内AuthorizerとAgency configはTarget blob一致。
- RollbackはAgency単体ならroute/service優先、schema／Agency／Code／attribution／security履歴は保持。APIを004A以前へ戻すならStorefront 004Bを先に旧版へ戻し、Admin/Agency UI／Mail catalog互換性とcanonical-time repairを確認後、NEW Taskがreadbackする旧exact artifactへ戻す。参考SHAをrollback artifact確定値としない。
- NEW Server／Production activation／Production DB／Operational Migration／Nginx／DNS・TLS／runtime env・Secret／Provider／Production account・Agency・attribution mutationは全0。Package buildは1 workflow／2 images、rerun0、dispatch-to-completion175秒、Runtime Activation0。本Worklog記録PRのchecks／review／mergeは別の文書gateでありcandidate再build・Production GOではない。詳細safe report／source inventory／checks／artifact provenanceはrepository外Task evidenceへ保存。READY FOR NEW SERVER / PLATFORM PRODUCTION PROMOTION=`NO`。
