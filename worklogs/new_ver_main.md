...
- clean local／origin／live protected `main@94c92f000997049eca65c197c99e6fb8bdde1416`をBaseに、Issue `#447`、Branch `security/SEC-015-browserslist-advisory-remediation`、dedicated Worktree `/var/www/oripa-worktrees/SEC-015`、Risk `R4`、Lane `Strict Change`、Activation `none`で開始した。GitHub deliveryとexplicit isolationのためexact 3-path Task Policy（root-owned mode 0600、SHA-256 `5fb6286284179242ff48e888538130bb7faaef5105a5f7191199731c529f148a`）を選択した。SEC-014／OPS-028 branch、worktree、PR、sourceは変更していない。
- canonical legacy pnpm auditで`browserslist 4.28.2`にHigh 2件、`GHSA-c83g-rgw3-j3cx`／`CVE-2026-73089`と`GHSA-73wf-gq98-2v4g`／`CVE-2026-73088`を再現した。ともにaffected `<=4.28.6`、patched `>=4.28.7`。dependency pathはlegacy standalone packageのdirect dev `eslint-config-next 16.2.11`から`eslint-plugin-react-hooks 7.1.1`→`@babel/core 7.29.7`→`@babel/helper-compilation-targets 7.29.7`→transitive Browserslistである。
- package manifestとparentを変更せず、standalone `legacy/v1-frontend/pnpm-lock.yaml`だけでBrowserslistをminimum safe `4.28.7`へ更新した。不可避なminimum closureは`baseline-browser-mapping 2.10.44`、`caniuse-lite 1.0.30001806`、`electron-to-chromium 1.5.393`、`node-releases 2.0.51`で、`update-browserslist-db 1.2.3`はversion不変のpeer bindingだけを同期した。final lock diffは25 insertions／25 deletions、unrelated update 0、root／legacy package.json、root pnpm lock、Composer filesは不変である。
- final legacy／root workspace pnpm auditはfinding 0。legacy／root frozen install、Browserslist target resolution、legacy typecheck／production build、Admin 36 files・201 tests／typecheck／lint／production build、Security Unit 10、Policy Unit 203、Quality Unit 4、release source validation、`git diff --check`をPASSした。Migrationは70件、content-set `8bc21870df410bf08b8753aed102cba06351e95c3ae2ab1a8e945d99f8206861`でbefore／after一致し、作成／変更／適用0。Production／RDS／Activation／Nginx／AWS／Secret mutationは各0である。
- SEC-014がunmergedのためcurrent protected-main Composer lockには独立CommonMark High 4件が残り、canonical repository gateは`security-gate: FAIL: Composer advisory baseline mismatch`でfail closedした。SEC-015へComposer変更を持ち込むこともbaseline／ignore／workflowを変更することも禁止されるため、Taskは`HOLD`。draft PR `#448`でscope-complete evidenceを残すが、Required 5完了、formal fresh exact-head review、merge、cleanupは行わずHuman sequencing decisionを待つ。
