...
- REL-036の変更はledger、直接のsettled-release regression、release runbook、004B handoff、本Worklogの5pathのみ。Source/API/DB/Identity/Payment/Point/Draw/CI policy/dependency/Storefrontは変更しない。Metadata最終checks・review・merge・branch cleanupはPR closeoutへ固定する。Runtime rollbackは従前AGENCY-003 API imageへ戻し、additive075は保持する。既存server空き容量約2GiBは観測するが、他Task image/cache削除は行わない。
