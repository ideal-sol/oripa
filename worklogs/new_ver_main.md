...
- `v2_private internal:true`を維持し、APIだけが別の非internal `v2_api_egress` bridgeにも接続する構成を追加した。Admin／PostgreSQL／Redisのegress参加、private networkの隔離解除、internal/non-bridge egressをDB Guard／Policy Gateで拒否する。Focused DB／Policy Unit 180 tests、Canonical Compose解決、`git diff --check`はPASSした。Migration／DB Schema／Public・Admin Contract／Artifact／Storefront／Mail Domain変更は0である。
- 初回activationはDocker自動割当のegress `192.168.32.0/20`がprivate固定IP `192.168.61.10/24`と重複してAPI起動を拒否した。即時に旧image／private-only構成へAPIだけをrollbackしhealthy／200を確認した。egressを上書き可能な最小`192.168.62.0/28`へ固定し、private subnet重複をGuardで拒否して再検証する。
