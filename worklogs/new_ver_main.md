...
## SMS-002 SMS OTP TTL / Verified Phone Pre-send Validation

- Human Browser確認PASSと追加Human DecisionをAuthorityに、clean local／origin／GitHub protected `main@7942268281450257dcb76f38c8be8743b1c66be6`からBranch `fix/SMS-002-sms-ttl-phone-precheck`を通常Worktreeに作成した。Issue／dedicated Worktree／Task Policy／exact allowed paths／Source Lockは使用せず、Risk `R4`、Lane `Strict Change`、Application Runtime Activation `deferred`とした。再開時も全Authority一致、Source／Shared Preview migration ledgerは`000001–000071`一致、Shared Preview batch 36／`000071 Ran`、`000072`未使用を確認し、Migration Allocation Lockを取得・保持してHuman承認済み`000072`を割り当てた。
- OTP TTLを60分へ変更し、FourS本文の期限表示も60分へ同期した。Active／Suspended等の再利用不可Verified Phoneをsend／resendのChallenge発行前にgeneric `PHONE_NUMBER_UNAVAILABLE`で拒否し、Challenge／Outbox／Provider requestを作らない。未認証重複、Closed／Anonymized再利用、self-number、Phone Changeのold phone保持を維持し、Verify transactionのadvisory ownership lock／release rule／DB unique constraintを最終Authorityとして保持する。
...
- OpenAPI／Client／Testkit contract、Artifact、dependency／lockfile、Shared Preview／Production Runtimeは変更しない。Focused validation、Required 5 Checks、Final Head exact fresh Strict self-review、squash merge、Authority readbackを継続する。
- Guarded isolated PostgreSQL 17 migration lifecycleは000001–000072 fresh upを2回、000072 rollback／reapply、statusをPASSした。続くV2全602 tests／6,132 assertionsは新Migration test fixtureが既存active-Challenge unique indexへ抵触した1 errorだけを検出し、production code failureは0だった。fixtureをinactive Challengeへ修正し、Migration境界1 test／4 assertions、SMS／FourS focused 35 tests／268 assertions、Verify／same-phone concurrency 2 tests／15 assertionsを最終PASSした。同じHeadの不要なfull rerunは行わずRequired ChecksをFinal authorityとする。
- PHP syntax、focused Policy Unit、`git diff --check`、migration 000072単独追加、dependency／contract delta 0、SEC-016 `league/commonmark 2.10.0`／legacy `browserslist 4.28.7`保持を確認した。Shared Previewは旧Source／000071のままであり、merge後に別Activation stepとして000072 applyとSource reactivationが必要。Production mutationは0である。
- Issue／Task Policy ceremonyはHuman判断どおり不採用。installed GitHub App wrapperのprivileged delivery hard requirementだけを満たすため、exact 15 paths・必要10 operations限定のroot-owned transient compatibility manifestをcommit外で発行し、merge／branch cleanup後に削除する。
- PR #452初回Head `c4f720a6c1ca4758d440c66b76bcbb3057a894cf`はPR metadataの`UI Verification`がcanonical `PASS`／`NOT_APPLICABLE`以外の説明付き値だったためpolicy-gateがFail Closedし、後続Checksはskipされた。同一Head rerunはせず、current Backend-only deltaを`NOT_APPLICABLE`へ正規化した新HeadでRequired Checksを実行する。
- 次Head `12f61d89a023845f7ac3691f842a22df4d7065f9`のsynchronize workflowはpush直後かつPR body更新前に開始し、GitHub event payload内の旧UI metadataを再度Fail Closedした。PR live bodyは既に`NOT_APPLICABLE`へ更新済み。同一Head rerunはせず、このevent-order correctionを記録した次Headで新規Checksを実行する。
