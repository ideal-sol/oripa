...
## SEC-014 league/commonmark Security Advisory Remediation

- clean local／origin／live protected `main@94c92f000997049eca65c197c99e6fb8bdde1416`をBaseに、Issue `#445`、Branch `security/SEC-014-commonmark-advisory-remediation`、専用Worktree `/var/www/oripa-worktrees/SEC-014`、Risk `R4`、Lane `Strict Change`、Activation `none`で開始した。exact 3-path transient Task Policy（root-owned mode 0600、SHA-256 `6ecdaf8247cd1b21823ac39e960f240e34e9e3f0b2dc1cc3af19d594a6670815`）を使用し、OPS-028 PR `#444`／branch／worktreeは変更していない。
- Stage 0でlocked `league/commonmark 2.9.0`に対するPackagist／GitHubのhigh advisory 4件をcanonical `composer audit`で再現した。共通minimum fixed versionは`2.10.0`。同packageはdirect dependencyではなく、`laravel/framework v13.15.0`が`^2.8.1`で要求するtransitive dependencyであり、Laravel 13.15.0とのComposer graph conflictは0である。
- PHP 8.4のtargeted `composer update league/commonmark --with-dependencies --minimal-changes`で`2.10.0`へ更新した。Lock diffはCommonMark 1 packageだけ、transitive／unrelated update 0、`composer.json`変更0。post-update Composer auditはadvisory 0、frozen install／strict validate／platform requirementsをPASSした。baseline／ignore／workflow／severity mutationは各0である。
- CommonMark smoke、Rich Text／Content／Mail Template／Page／Notice／sanitize/render focused 26 tests・231 assertions、full Feature/API 286 tests・1,596 assertions、Policy 203、Security 10、Quality 4 testsをPASSした。Migrationはbefore／afterとも70 files、content-set SHA-256 `8bc21870df410bf08b8753aed102cba06351e95c3ae2ab1a8e945d99f8206861`でmutation 0。Production／Shared Preview DB・Runtime・Nginx・AWS・Secret mutationは各0である。
- canonical pnpm auditがSEC-014と独立したlegacy frontendの`browserslist 4.28.2` high advisory 2件（`GHSA-c83g-rgw3-j3cx`、`GHSA-73wf-gq98-2v4g`、fixed `>=4.28.7`）を新規検出し、local `security-gate`は`pnpm advisory baseline mismatch`でFAILした。Human指定どおり別packageへScope拡張せず、legacy lock／empty baseline／security workflowを変更しない。SEC-014はRequired 5とfresh exact-head self-reviewを成立できないためHOLDし、merge／branch cleanup／OPS-028 resumeを実行しない。
