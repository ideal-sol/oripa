...
- `docker-compose.v2.yml` API serviceへ`MAIL_MAILER`、`MAIL_FROM_ADDRESS`、`MAIL_FROM_NAME`、`MAILGUN_DOMAIN`、`MAILGUN_SECRET`をrequiredとして、`MAILGUN_ENDPOINT`／`MAILGUN_SCHEME`をLaravel config既定値付きで注入した。official `symfony/mailgun-mailer` dependencyは既存のため変更0である。
