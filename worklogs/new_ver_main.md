...
- Issue／Task Policy ceremonyはHuman判断どおり不採用。installed GitHub App wrapperのprivileged delivery hard requirementだけを満たすため、exact 15 paths・必要11 operations限定のroot-owned transient compatibility manifestをcommit外で使用し、merge／branch cleanup後に削除する。Shared Previewは旧Source／000071のままで、merge後の別Activation stepで000072 applyとSource reactivationが必要。Production mutationは0である。
