## ASSETURL-20260925 — Public presentation asset URL correction

- Issue none; Risk R3; Strict Change; Application Runtime Activation deferred. GitHub protected main and remote main are `706849a57b9919bb3f01cc630ac90a3efdf5a61c`. Dedicated branch `fix/ASSETURL-20260925-public-presentation` preserves the unrelated dirty primary checkout. Exact-path policy is for GitHub App transport; no migration/source lock is needed.
- Root cause: Admin rank-effect upload stores an Admin content URL in catalog_presentation_assets.public_path. Both Public Catalog and new Draw prefixedAsset projections copied that value. The existing Public presentation content route already serves public, non-archived assets referenced by immutable Rank/image/video revisions. The two projections now derive that established path from the public asset ID; no Admin route, middleware, upload, storage or publication policy change.
- New Draw presentation/results/result-image snapshots receive the Public path before response_data persistence. Highest Rank/revision/order/tie-break/full-results authority is unchanged. Existing saved responses, including Admin paths, are not rewritten on read/replay; no backfill or data correction. Public Gacha lineup/current-video projection uses the same existing delivery route. Unrelated prize/banner image delivery is unchanged.
- Isolated focused API tests: 68 tests / 753 assertions PASS, covering Public path leakage/content/MIME/Range/access policy, Admin upload, representative selection, immutable saved responses, legacy no-rewrite and full 100/1000 HTTP POST/replay/GET. 1000 response: 1,634,725 JSON bytes, 35,651,584-byte memory delta; five-run 1000 p95 1672.885ms, 58 queries, 33 SELECTs. Initial red test reproduced Admin leakage; a cloned fixture unique-path collision and the old stored-path expectation were corrected without weakening product guards.
- Isolated real HTTP Public video probe returns 200/video-webm/full bytes for ordinary and Range requests, preserving existing full-content 200 behavior, checksum ETag and immutable cache headers. Real Chromium, without interception, decoded 320x180 video and reached ended with no media/page error and zero Admin requests. No shared account/session or runtime was used. The probe harness uses the isolated internal network and a public catalog readiness endpoint because its disposable fixture has no Redis service.
- Quality, policy, OpenAPI bundle verification and settled release-source validation PASS. The new worktree has no installed JS dependencies; OpenAPI verification reused the existing pinned validator executable after the initial missing-tool failure. Current Security baseline is empty; ESLint retains nine exact existing findings; both expire 2026-10-02 and final-head CI must revalidate them.
- Contract release decision: compatibility policy section 5.3 classifies restoring specified behavior as a Platform bug fix. No endpoint/schema/client/testkit/contract payload changes; alpha.40 and Public alpha.36 remain byte-identical. No Contract Artifact bump, rebuild or overwrite. A later API runtime artifact must use the merged source, not overwrite an existing runtime image.
- Migration creation/shared application 0; only the existing 76 migrations initialize the disposable test DB. Shared OLD Test activation, Storefront changes, Production/new-server access, ENV/Secret/Nginx changes, Source fixes outside these projections and DB history rewrites are prohibited. PR/CI/self-review/merge and exact activation handoff follow; no runtime activation is authorized by this Source Task.

## BASELINE-20260925 — Production Preparation Baseline Refresh Only

- Issue none; Risk R3; Strict Change; Application Runtime Activation none. Base `07d091e27ef05a1b766a6f2b2052e9a880f02d95`; isolated branch `chore/BASELINE-20260925-refresh`. Original dirty checkout and existing Worktrees preserved. Exact-path transport policy only; no source/migration locks.
- Fresh locked Composer, workspace pnpm and legacy pnpm audits at 2026-09-24T23:33:23Z / 2026-09-25 JST contain zero findings. Fresh legacy ESLint exactly matches all nine existing fingerprints (eight errors, one warning); raw lint exit 1 is expected, not a clean raw lint claim. No new, changed, missing or suppressed finding. Existing installed legacy tools reused read-only against byte-identical lock/config/source; no dependency installation or change.
- Renew only management reason, tracking authority and expiry in the two baselines: BASELINE-20260925, expires 2026-10-02, preserving established seven-day review window, owners, removal conditions, finding sets and fail-closed validators. Explicit September 25 validity and expiry/regression negative cases are validated; focused test/check and exact-head self-review evidence recorded on this PR.
- Runtime Target remains `e3121034c5dc7b9184673b1be64bb5076e9d3a90`; approved-source metadata and Contract are unchanged. Later baseline merge is protected-main authority only, not Runtime Source. Runtime Application, dependency versions, Dockerfiles, CI/workflows, migrations, DB data, ENV, Storefront and Agency remain unchanged.
- Run current Required Checks only, including mandatory ARM64 CI verification build if scheduled; no bypass or unnecessary rerun. Canonical deployable Production Runtime Artifact workflow, Production activation and NEW server operations NOT RUN. Reuse accepted PR #503 application/OLD Test/Browser and Contract evidence; no acceptance rerun. After merge verify canonical exact-source acceptance against new main and record cleanup/closeout in the PR.

## PRODAUTH-20260925 — PR #503 Production Authority Sync

- Issue none; Risk R3; Strict Change; Application Runtime Activation none. Base and Runtime Target `e3121034c5dc7b9184673b1be64bb5076e9d3a90`; dedicated branch `chore/PRODAUTH-20260925-catalog-target` outside the original dirty checkout. Preserve the original branch, HEAD, worklog and existing Worktrees. Exact-path transport policy only; no source/migration lock required.
- Human confirms PR #503 implementation, OLD Test Technical and Human Browser PASS. Sync only approved-source metadata, focused authority tests, runbook and this dedicated Worklog. Source PR remains #503 / CATALOG-20260924; a later metadata merge is Workflow Authority, never a replacement Runtime Target. Application/worker/dependency/contract sources, Dockerfiles, ENV and migrations remain unchanged.
- Reported Production source `be1a8f3f822d23f3251d32e616fb0b2fe422714e` to Target contains only PR #503's two runtime files; all other paths are tests or docs/metadata/authority. No unapproved runtime delta. Existing immutable Contract alpha.39 / Public alpha.35 / Admin and Webhook alpha.34 provenance remains applicable because Contract/package source trees are unchanged. All three mail/SMS workers require no update for this delta.
- Assessment date 2026-09-25: Security and ESLint baseline authority AGENCYREADY-20260917 expires 2026-09-24, therefore both REFRESH_REQUIRED. The OLD host check begins at 2026-09-24T15:18:41Z (2026-09-25 in Japan). UTC CI PASS on September 24 is not September 25 baseline readiness and does not renew the expiry.
- Human explicitly permits current Required CI, including its ARM64 Production-target verification images. No manual canonical Production Artifact workflow, deployable Artifact, activation, NEW server operation, CI/workflow modification, check bypass or unnecessary rerun. Focused tests, exact-head review, PR, merge and closeout evidence are recorded in this Change's PR. Prior application acceptance is reused, not re-executed.
...
- 最終local mocked Chromium 9/9 PASS（1440/1366/390px一覧、通常mock返信、1440/390px履歴分類・Modal・30件scroll）。画像のgeometry確認済み。ローカルBrowserに日本語fontがなく画像の日本語字形は確認対象外、DOMの日本語label・内容assertionsはPASS。Human環境の最終表示確認は別工程。PR作成前のmetadata検証でTask ID書式違いを検出、既存policyを緩和せずCONTACTHISTORY-20260921へ訂正。旧名のbase-only Remote refは内容一致確認後cleanup、PR分割なし。
...
- 開始時clean、Source branch／Remote Head `9dfbbece3e522b39641c1a1600f55ab8cdb07479`、protected base `a8e3079e2b7f9347c1932a7552016625db77a64d`。Migration075は2026_10_01、076は2026_10_02。Laravelのfull filename lexical sortと現行policy inventoryに一致するためrenameなし、body／番号／timestamp変更0。日付は実行予約や実作成日ではなく順序ラベルでありgenerator生成とは主張しない。全履歴番号の単純単調性probeは既存058/060の逆順でFAILしたが、新076の番号重複なし／末尾追加／075より後の実行順確認はPASS。既存Migration修正なし。共有Test／Production未適用は前Phase記録と一致し、Runtime DB照会なし。
- Source差分0のため既存focused testsと全5 Required Checks／CodeQL／Dependency Reviewを再利用。Strict exact-head reviewを更新しSEV-0/1=0、[fresh evidence](https://github.com/ideal-sol/oripa/pull/493#issuecomment-5749861286)後にcanonical App squash merge。PR493 squash `b63c37edac755f782f09ca5b18255fb71cee300b`、reviewed treeとmerge tree一致／content diff0。protected main／origin/main／local main一致、元branchはRemote auto-deleteとlocal cleanup済み。Migration Allocation Lock再取得後closeoutで解放。
- Dedicated contact_user_messages、4状態reopen、本人所有／new fallback、Public idempotency、二段階render、contact_reply／reply_content／5変数、current registered recipient、新topic+event filterとlegacy除外を再review。V2DatabaseTimestamp維持、Fresh auth／Admin業務Limiter復活なし。Storefront /contactは以前からログイン必須であり、今回はBackend境界との一致。次Site Phaseで既存login return queryを維持する。
- Canonical contract-only Run `35511375394`、Artifact `10606100530`、Source `b63c37edac755f782f09ca5b18255fb71cee300b`。exact protected-main／merged PR tree／Required Checks／version collisionなしをauthority helperとworkflowで確認後、1回dispatch。Client/Testkit alpha.37・Public alpha.33／76 operationsを発行しdownstream readback PASS。local canonical readbackでもouter／five-file／Manifest／packed packages／Public digest検証PASS。Manifest `064b178c9781ef706855baaedeaa8aa7836214a36c9f8ecd78b1044551d5c293`、outer `a196e21b61cef71329312cc057b4376fd6919c2a877492f6852f09006a0ac611`。Site Schema alpha.23は参照のみ、immutable再発行／上書き／Stable Tag／registry publishなし。
- Canonical runbookが別Release Metadata PRを要求するためREL-037を開始。Issue none、Risk R3、Lane Strict Change、Activation none、通常worktree、Branch `chore/REL-037-contact-artifact-metadata`、Base `b63c37edac755f782f09ca5b18255fb71cee300b`。5pathのみ：ledger、直接のrelease/policy regression、release runbook、本Worklog。App exact-path policyを利用しArtifact Release Lock保持。既存immutable historyを不変比較し、検証済み値からlatest alpha.37／candidate nullへ更新。API／Admin／Contract／generated／Migration／CI policy implementation／依存／Storefront Sourceは変更しない。
- Metadata focused tests／Required Checks／self-review／final head／merge／cleanup／CI時間は同一PR closeoutへ固定する。このPhaseのlocal API/Admin build0、application activation0、Mail0、Production0。Publication buildはClient/Testkitのみ1 workflow、referenced Schema依存buildを含む。旧Test Migration／runtime／実受信／Human受入は別Promptまで停止。

- REL-037 focused: release49 tests／policy209 tests PASS。初回release testは過去alpha.36 fixtureがcurrent latestを参照して失敗したため、過去履歴fixtureを明示し既存digest assertionsを維持、追加alpha.37 settled/no-republish assertionsもPASS。settled ledgerでcanonical bundle verify／source validation PASS、全predecessor履歴一致、secret candidates0／dangerous paths0、local policy PASS。Required CIとcloseoutをPRへ固定する。
...
- APIはBase revision、Mail／SMS Workerは `b6541e526c4943c03c3533f7cbc9d3ca560a2567` のまま。checkoutから稼働serviceへのSource mountなし。稼働service、Production ENV／DB、旧Serverの変更は0。
- `V2DatabaseTimestamp::format(DateTimeInterface|null)`は解釈済みobjectを変更せず、同一instantのUTC offset付き秒精度文字列／nullを返す。文字列解釈・validation・業務日付判断は既存呼出元に残す。`ContentBanner`／`ContentVersion`だけ保存形式を `Y-m-d H:i:sP` にし、Eloquent再変換でoffsetが落ちることを防ぐ。全Model／Session Model／Connectionへの適用なし。
- バナーcreate／update、category作成、asset upload、banner親作成、version／asset link作成、publishのQueryBuilder明示日時をhelperへ通す。対象は `created_at`／`updated_at`、versionの `publish_start_at`／`publish_end_at`／`published_at`。共有 `createVersionRow`／`publish`／`publishedQuery` とContentVersion保存形式はNotice／Static Pageにも影響するため関連focused testを含めるが、これらDomainの全write修正には広げない。公開比較値もoffset付きにし、開始inclusive／終了exclusive、開始null拒否／終了null許可、timezoneなし入力のapp timezone解釈、既存API表現・JST表示・秒精度を維持する。既存response用属性は保持してDB bindingだけ変換する。
- 既存test image `oripa-v2-api:test-OPS-028` と `postgres:17-alpine` を再利用。実測PHP 8.4.23、Laravel 13.15.0、PHPUnit 12.5.29、PostgreSQL 17.11。現行Composer lockに従うinstallを独立vendor volumeだけに行い、依存／lockfile／Production vendor変更0、image build 0。開始時free約81GB／available memory約5.9GBを確認し、PHP 1536MiB／PG 512MiBの上限を設定した。
- テストは専用internal network、外部公開portなし、PG data／PHP storage／config cacheはtmpfs、Sourceはread-only mount。Production ENV／Secret／config cacheを持ち込まず、独自test認証とarray mail／cache／session、sync queueを使用。Worker／Scheduler／実Providerを起動しない。既存 `scripts/db/v2_database.py` の接続先guardとmigration inventory確認を再利用し、空の専用DBに既存75 migrationsを1回適用した。実接続DB／schema／portを確認し、app／PHP Asia/Tokyo、接続設定と実際のSHOW TIME ZONEがUTC／Asia/Tokyoそれぞれ一致することを確認した。guard互換用MIG-999識別子はこの一時環境だけに使用し、新規Migration／予約／Taskを作成していない。
- 最終focused結果：UTC **39 tests / 619 assertions PASS**、JST **39 tests / 619 assertions PASS**。新規DatabaseTimestampPersistenceTest 4件、BannerTimestampPersistenceTest 3件、既存SessionIdleExpiryTest 16件、AdminSessionCanonicalTimeTest 5件、AdminBannerManagementTest 3件、AdminPageManagementTest 4件、StaticPageFooterContractTest 1件、ContentContactVerticalSliceTestの公開期間／immutable・legal MFA／draft・duplicate・asset拒否の3件。Storage／Content 18件は両外部接続条件で実行。Session 21件は既存test内部の接続条件指定を保持しており、全21件がJST固定で動いたとは扱わない。Base sourceのSAFE Sessionも21 tests / 394 assertions PASS。PHP構文8files／`git diff --check` PASS。全Suite／全Build／Browser／CI／Security・ESLint期限baseline更新はNOT RUN。
- 基準 `2026-09-17T10:28:20+09:00` は両条件でUTC `2026-09-17T01:28:20Z`、epoch `1789608500`、誤差0秒。JST／UTC object、既存parse後のoffset／timezoneなし入力、null、入力非破壊、既存秒精度とJST日跨ぎ、QueryBuilder insert／update、raw binding、Eloquent create／save／instance touch／自動timestamps／bulk自動updated_at／明示bulk、Model読戻し・serialization、バナー作成更新・公開境界を実DBで検証した。正常／既知+9時間の過去created_at混在fixtureで、今回更新しない履歴を補正しないことも確認した。
- 負例検証ではcheckoutを変えずBaseの対象app classesを独立PHPへpreloadし、UTCの対象4件で3件の+32400秒不一致と1件の開始終了制約違反、JSTのUTC入力caseで-32400秒不一致を検出した。これらは修正前の意図した失敗であり最終結果とは分離する。初回既存Content testのstale MFA失敗はBaseでも再現し、test fixtureだけを既存Session canonical clockで生成するよう修正した（期待値／制約変更なし）。新規testの初回Admin fixture hash形式エラーは既存PasswordPolicy利用で解消した。
- 直接 `Model::query()->touch()` はModel日時変換を迂回し、隔離probeでUTC接続では+32400秒、JST接続では0秒となることを確認。今回対象Sourceにその呼出しはなく、global Builder変更は行わない。対象で必要なbulk touch相当はhelperを通した明示updated_at updateを使い、両条件で誤差0秒を検証した。Model保存規約だけで全bulk経路が安全とは扱わない。
- Local review：SAFE Session production source変更0、Schema／新規Migration／API契約／表示仕様／日時精度変更0、Production Secret／test vendor・cache・DB生成物のGit混入0。Auth直接revoked_at、Payment／Point／Draw／Shipping／Outbox／Audit／Idempotency、Contentの未対象write、既存誤保存データは未対応。過去の誤った公開期間は別のデータ補正判断が必要。旧NEEDS_CONFIRMATION 3群は未確定のまま、バナー先行PASSを30主要write群全体のPASSとしない。
- 終了時、このTaskのPHP／PGコンテナ2個と専用networkだけを停止・削除し、tmpfs DBも破棄。再検証用の独立vendor volume `mig999-v2-datetime-phase1-vendor` と `/tmp/oripa-datetime-phase1/` の実行手順・probe・test専用設定（認証ファイル0600）はGit外に保持する。既存image／他Task resource／Production volumeの削除なし。Source・test・本記録を保持する。
- Result `PHASE1_SOURCE_AND_FOCUSED_TEST_PASS`（先行範囲のみ、Production未修正）。Production DB書込み／既存データ補正／ENV編集／実決済／実送信／Browser Loginは0、新規Migration作成／本番適用0。次のpush／Strict PR CI・Artifact生成、および各Runtimeへの反映は別途GOが必要で、API反映だけでWorker反映済みとはしない。Phase 1作業は約21分、CI wait 0、CI rerun 0、Build 0、Runtime activation 0、Human承認待ち0。ローカル再実行・負例検証は上記の通り。
...
- Source PR #475はHead `80f589fad90b9d8a61220792f84a390aaa0901f4`でRequired 5 Checks／全emitted checks PASS、[fresh self-review](https://github.com/ideal-sol/oripa/pull/475#issuecomment-5579465477) SEV-0/1=0を経て、GitHub App squash `aa5049f7efa63e9cff67b10d93e768b4006b0c09`へMerge済み。Final focused44/295、Full V2 684/7191 PASS（既存opt-in load/performance10 skip）、Client34、Testkit45、OpenAPI15、Release48、Policy206、DB45と各drift／local policy／quality PASS。初回Full2件失敗はtest fixture baseline／lock統計snapshotを修正し、final headで解消。修正中にstale PHPを読み込んだ中間Full1回は停止し、PASS根拠にしない。
- Canonical Run `34188731627`がimmutable alpha.36／Artifact `10041438541`を発行しdownstream readback PASS。Sourceは上記squash。Outer `5e6dba27ce3745bbbec27d662f2e271dafec63eded0c0f5d11788b69c7298e2e`、Manifest `101ec49daf4e30bcd2fecd10e3314d96604bd86ea0f6029de4b5fc2bf0a39292`。Localでもexact five-file／packed package／Public／outer digestを検証し、その値からledger patchを生成。latest immutable alpha.36／candidate null、過去履歴変更0、rebuild／republish／tag／Release作成0。Client/Testkit/Public各digestと004B APIはrelease runbook／handoffへ固定する。
- AGENCY-004A runtime工程は別Canonical API-only Run `34188725493`／Artifact `10041463058`からreviewed head imageをverify/loadし、既存旧Shared PreviewのAPIだけを1回activate。Migration075を1本適用、75本／pending0。Admin／Agency／Storefront／DB／Redis containerの変更0、User／Attribution／Payment等の件数変更0。health、active／stopped／invalid Code、既存configured Origin/CSRFを使うCodeなし不正emailの422 contract境界、API500/502/504=0を確認。Technical harnessの公開health経路・既存Origin/CSRF選択を修正して最終PASS。Runtime User登録、外部Mail、Provider、実決済、SMS、偽Coin/Wallet/Refundは0。隔離task DB/container/network/volume/credentialはcleanup済み。
- REL-036の変更はledger、直接のsettled-release／Policy regression、release runbook、004B handoff、本Worklogの6pathのみ。Source/API/DB/Identity/Payment/Point/Draw/CI policy implementation/dependency/Storefrontは変更しない。Metadata最終checks・review・merge・branch cleanupはPR closeoutへ固定する。Runtime rollbackは従前AGENCY-003 API imageへ戻し、additive075は保持する。既存server空き容量約2GiBは観測するが、他Task image/cache削除は行わない。
- 004Bはalpha.36 Artifactをexact-pinしてCookie/queryを接続する別Task。30-day Last Click、valid A→B=B、invalidならA維持、code-less維持、expired候補なし、停止Code新規保持なし、既存User attribution0。Cookie consent UI/banner/popup/button/alert/CMP=0。004AのHuman Browser AcceptanceはNOT REQUIRED、Storefront checkout READ/WRITE/BUILD/TEST/GIT=0、Production mutation=0。

- Metadata PR初回CIはPolicy regressionの旧latest alpha.35／candidateあり期待が残りFAIL。直接必要な`tests/ci/policy/test_policy_gate.py`を6つ目のpathへ追加し、alpha.36 source／manifest／candidate nullを厳密に検証、既存alpha.35／alpha.34履歴assertionは維持する。Policy implementation／permissions／Required Checksは変更せず、同PR内でfinal headの全gateを再実行する。

## AGENCY-004A Platform Attribution And Public Contract

- Issue `none`、Branch `feat/AGENCY-004A-platform-attribution`、通常Worktree、Base `8305e3f14db2c4bb15c03575b76b81b1370a802b`、Risk `R4`、Lane `Strict Change`、Activation `immediate`（旧Shared Preview APIのみ）。protected main／origin/main／local main／checkout一致・clean、Migration `000074`／pending 0をreadback。Storefront checkoutのREAD/WRITE/BUILD/TEST/GITは0、Production mutation 0。
- `V2AgencyAttributionService`を既存Agency Serviceと同じIdentity Domainへ追加。通常登録とExternal Identityの新規User作成transaction内だけでCode完全一致・Agency row lock・停止再検証・一度だけINSERTを行う。existing User login/link/collision/backfill、既存Attribution上書き、Cookie実装、Referral転用は0。Laravel TrimStringsの広告field除外で空白や改行を有効Codeへ変換しない。
- Migration Allocation Lock取得・latest再確認後、未使用`000075`を予約。外部認証既存transactionへnullable immutable candidateだけを追加し、opaque state／browser binding／nonce／PKCE／expiryを維持。callback任意Codeは採用せず、link／reauthには保存しない。既存DB append-only guardとAGENCY-003 queryは変更0。Migration rollbackはcandidate履歴ありなら拒否、旧API rollback時はadditive schema保持。
- Public `validateAdvertisingCode`はanonymous／no-store／valid booleanだけ。register／Google・LINE startのoptional `advertising_code`を追加し、Codeなし・不正・停止時も登録継続。Public alpha.32／76 operations、Client/Testkit alpha.36。Admin／Agency OpenAPIとUIは変更0。共通Testkit fixtureと既存Mock helperで004Bを支援する。
- Artifact lock取得時、ledger latest alpha.34／candidate alpha.35に対し、SMS-001 alpha.35は既にcanonical Run `33617554113`／Artifact `9841577025`／source `7942268281450257dcb76f38c8be8743b1c66be6`で発行済みと確認。既存verifierでouter／manifest／five-file／package／OpenAPIをreadbackし、検証値からledger reconciliationを生成。既存candidateの上書き／version再利用はせず、未使用alpha.36を次candidateへ予約する。Public-only独立versionを既存Compatibility Policyどおり検証可能にし、version後退／skip拒否testを追加。発行はmerge後canonical workflow、完了ledgerは別Release Metadata PR。
...
- Live protected `main` exact refとGitHub protected flagを確認し、Base `767a60344766115552c1602b939874f2ebbfd51c`からIssue `none`、Branch `fix/SEC-019-session-lifecycle-canonical-time`、current worktree、Risk `R4`、Lane `Strict Change`、Application Runtime Activation `immediate`（旧Test Platform API only）で開始した。GitHub App transport／Preview import用にrepository外のroot-owned mode `0600` transient policyをexact 5 pathsへ限定する。Dedicated Worktree、Source Lock、Migration Allocation Lockは不要。Production Promotionと新Server操作は本Taskの対象外である。
- `V2SessionManager::issue()`はapplication timezoneの`now()`をoffsetなしでbindする一方、SEC-018 GuardはDB connection timezoneへ同一instantを変換していた。Laravel `Asia/Tokyo`／PostgreSQL connection `UTC`で実POST Login 200後、GET Sessionが`last_activity_at < created_at`として`user_sessions_expiry_check`違反／500になることを修正前Sourceと実PostgreSQLで再現した。Synthetic rowだけのSEC-018 regressionでは新規発行との不整合を捕捉できなかった。
- 既存の共通`V2SessionPolicy`へSEC-018のDB connection configured timezone authorityを移し、`currentTime()`／`canonicalTime()`をManagerとGuardで共有する。Offset付き日時は同一instantを`setTimezone`で変換し、offsetなしbinding前にconnectionが期待するwall-clockへ合わせる。Currentはimmutable／second precisionで1回取得し、issuanceのcreated／activity／reauthentication／MFA／idle／absolute、rotationの保持日時、revocation、active-session queryにも同じ方式を適用する。UTCの決め打ち、timezone再解釈、大規模Clock abstractionは追加しない。
- Issuanceとactivityのidleはabsoluteでcapし、absolute lifetimeは延長しない。Future `created_at`はcanonical currentより後の発行を安全に認証できず、実DB constraintへ違反することをSynthetic legacy evidenceで確認したため、Guard／active-session queryは除外し、User Session endpointの既存401 `SESSION_EXPIRED`／Cookie expiryへ到達させる。Rotationもfuture creationを拒否し、future reauthenticationはFresh Authを満たさない。Past creationのlegacy rowはpersisted instantを再解釈せず利用可能で、created／absoluteは変更しない。一括失効、timestamp書換え、Production Session data repairは不要で実行0である。
- PostgreSQL 17、PHP 8.4.23、Laravel `Asia/Tokyo`／connection `UTC`、active expiry constraintの隔離環境で、実CSRF preflight→POST Login→発行Cookie→GET Session／Gachas、保存instant／ordering、通常延長、absolute cap、exact boundary、反復activity、absolute到達時失効、既存security semantics、legacy future／past、実DB error伝播を検証した。Connection `Asia/Tokyo`、User rotation／revocation、Admin issuance／MFA gate／rotation／activityも確認した。Session/Auth対象86 testsがPASS。Final Headの再検証結果とRequired Checks／fresh self-reviewはPR evidenceで固定する。
- Migration作成0、Schema／constraints／PostgreSQL timezone変更0、既存72 migrationsの適用先はTask専用一時PostgreSQLだけである。初回test setupはPostgreSQL readiness前の接続失敗とschema未作成を検出し、readinessと全migration成功後の検証だけをAcceptance authorityとした。Shared Preview／Production migration適用0。変更はGuard、Manager、既存Policy、Session regression Test、本Worklogの5 filesのみ。API／Cookie contract、Storefront、Admin UI、Payment、Coin、Draw、Shipping、SMS／Mail、Nginx、Redis config、runtime env、secretsは変更0。
- Merge後はcanonical GitHub-hosted Preview `api-only` Buildを使用し、reviewed Head／Merge tree equalityとcontent diff 0、artifact／image／OCI revisionを検証して旧Test APIだけをactivateする。現行旧Test connection timezoneは`Asia/Tokyo`なのでruntime envを変更せず、公開HTTPS実Login Acceptanceに加え、同じactivated API内の隔離HTTP kernel processでconnection `UTC`を一時設定しProduction相当の実Route lifecycleを検証する。Process終了で設定とtransactional Synthetic dataは破棄する。Final activation／runtime／cleanup結果は同一PRへ追記し、Productionを変更せず停止する。Browser／visual acceptanceとProduction Promotionは実行しない。

## SEC-018 Session Idle Expiry Cap / Timezone Consistency Repair

- Base protected main `f381398f06b0156b0399f5af32707da40199bd62`、Issue `none`、Branch `fix/SEC-018-session-idle-expiry-cap`、current worktree、Risk `R4`、Lane `Strict Change`、Application Runtime Activation `immediate`（旧Test Platform API only）で開始した。Dedicated Worktree、Source Lock、Migration Allocation Lockは使用しない。GitHub App wrapperのhard requirementだけを満たすため、exact 3 pathsと必要操作に限定したroot-owned transient policyをrepository外で使用する。新Server／Production mutationとPromotionは0とする。
- `V2RealmSessionGuard::user()`は有効Sessionのactivity更新で3回の`now()`を使い、application timezoneのmutable `Carbon` candidateをPostgreSQL connection timezoneへ正規化せずbindしていた。LaravelはDateTime bindingをoffsetなしの`Y-m-d H:i:s`へ変換するため、Application `Asia/Tokyo`／DB connection `UTC`では同じinstantが9時間後として再解釈され、PHP比較上はabsolute未満のcandidateでもDB保存値が`absolute_expires_at`を超えて`user_sessions_expiry_check`へ違反し得た。Timezone差だけでなく、absolute capがDBへ保存されるcanonical instantに対して保証されなかったことがRoot Causeである。
- Guardは使用中DB connectionのconfigured timezoneをcanonical authorityとして、1つのsecond-precision immutable currentをquery expiry判定、candidate計算、`last_activity_at`へ共有する。raw `timestamptz` absoluteはoffsetが示すinstantを保ったままcanonical timezoneへ変換し、immutable candidateと早い方を保存する。`idle_expires_at <= absolute_expires_at`を保証し、既存のrevoked／idle／absolute／User state判定後だけactivity更新する順序、QueryException伝播、Cookie／API contractを維持する。Schema／constraint／Migration／既存Session dataは変更しない。
- 新規`SessionIdleExpiryTest`は実`GET /api/v2/auth/session`経路、Synthetic Cookie／Session、Laravel `Asia/Tokyo`、PostgreSQL connection `UTC`、active `user_sessions_expiry_check`で通常延長、absolute cap、exact boundary、9時間再解釈なし、absolute付近の反復activity、no-cookie／malformed／missing／expired／revoked／unavailable／active／restricted、実DB update failureの非抑制を検証する。
- PHP 8.4.23／PostgreSQL 17の分離環境で新規focused 7 tests／53 assertions、既存Authentication／Realm Separation／Admin User Stateを含む36 tests／302 assertions、fresh V2 schemaのcanonical `phpunit.v2.xml`、72 migrationsのmigrate-fresh 2回、latest rollback/reapply、schema inventory、PostgreSQL／Redis healthがPASSした。Policy tests 204、Quality tests 4、Security policy tests 10、local policy／quality gate、PHP syntax、`git diff --check`もPASSした。Host PHP 8.3での初回起動はRepository要件PHP >= 8.4.1を満たさずtest未実行、legacy Unit/FeatureとV2を同一既存DBへ混在させた非canonical full runはschema mismatchでFAILしたため根拠に使用しない。SEC-018専用container／network／volume／test image／一時credential fileは削除し、root-only非秘密integration evidenceだけを保持した。
- Changed pathsはGuard、直接回帰Test、本Worklogだけとする。Storefront、Admin UI、Password Reset、Email／SMS／Mail、Payment、Coin／Point、Draw、Inventory、Shipping、OpenAPI、dependency／lockfile、Migration、Nginx、Redis構成、runtime env、Provider、Productionは変更しない。Final local checks、Required 5 Checks、fresh exact-head self-review、squash merge、旧Test API-only build／activation／synthetic acceptanceは後続gateで確定する。
...
- 隔離PostgreSQL 17で`migrate:fresh`を`000070`までPASSし、Canonical API/Draw/QA/Reporting focused 105 tests/1,222 assertions、並行Draw/Presentation Revision boundary 1 test/9 assertions、Admin Vitest 35 files/199 tests、Storefront Client 33 tests、Testkit 43 tests、OpenAPI gate/unit 10 tests、Artifact unit 44 tests、migration inventory plus Policy unit 235 testsをPASSした。`000070`は同じ隔離backup/restore schema dumpでsource/restore差分がnonceのみとなることを確認した。Artifact source validationもPASSし、Runtime Activation、Shared Preview Activation、Production Activation、Account Security/Payment/Save Card/SMS/LINE mutationは各0である。

## GOV-027 Storefront Contract Release Ledger Reconciliation — 2.0.0-alpha.34

- clean local／origin／GitHub protected `main@576c35137946e5effcda63d6bf750d5ecc41150f`（MIG-099 PR #433 squash merge）からBranch `docs/GOV-027-storefront-contract-alpha-34-ledger-reconciliation`を作成した。Issue／dedicated Worktree／Source Lockなし、Risk R2、immutable release historyのためLane Strict Change、Activation `none`。Migration作成／適用0のためMigration Allocation LockはN/Aである。GitHub App privileged deliveryだけにexact 5-path transient Task Policy（root-owned mode 0600、SHA-256 `cbea2c70c7f73bc9991cacb86d2ae26cd641899c51bdc77b8208962d5c192a05`）を選択する。
- governed publication workflow Run `33395772059` attempt 1をexact protected `main` sourceから実行し、authorize／publish／readback全jobがPASSした。Artifact `oripa-storefront-contract-2.0.0-alpha.34`はexact 1件、ID `9759273312`、GitHub digest `sha256:c6927b367f9d1ad1a5602792873da481405dc8c3d9c1ba12bbca1954c4e4c8fb`、Source `576c35137946e5effcda63d6bf750d5ecc41150f`である。fresh remote readbackでManifest `42f4bee68b787dac16d07accee1c6154c7cea392c521c41b14461d6b56221464`、SHA256SUMS `555ae3637e71a57bff447aa084d21e649b598c878f64766b9f044d1e59f75355`、Public OpenAPI `27d0cdcee9194989058573d7e198066fa4af62017a0f301117ea4af034e733f0`、Client `3363ebf849e3c7165b89ea9f037c681ab889d16539ce290383cad41d31c134c6`、Testkit `07916ff69e2e6882aa0e62ee676a65652382413f14f65459ba4e773a41f8a440`を一致確認した。alpha.34の再publish／overwrite／deleteは0である。
- Ledgerはalpha.34 `contract-breaking` released recordをappendし、`latest_immutable=2.0.0-alpha.34`、`candidate=null`へsettleする。Shared Preview Storefrontはalpha.33 exact-pinで旧`RankDisplay`の`ranks[].id/name/code/prizes`に依存し、canonical Rank responseとの互換はない。さらに`test.luxe-pack.biz`と`luxe-pack.biz`の`/api/v2`は同じAPI upstreamにroutingされる。よってShared Preview migration `000069/000070`、API/Admin runtime activation、Storefront activation、routing／Production operationは各0でHOLDし、Storefront alpha.34 adoptionと別Infrastructure TaskによるPreview/Production API routing分離を要求する。

## GOV-028 Settled Breaking Release Verifier Repair

- clean protected `main@22ba6ae2da393fdbe42a67c136d1a3e4efdf09ff`からBranch `fix/GOV-028-settled-breaking-release-verifier`を作成した。Issue／dedicated Worktree／Source Lockなし、Risk R3、Lane Strict Change、Activation `none`。Migration作成／適用0のためMigration Allocation LockはN/Aである。GitHub App privileged deliveryだけにexact 4-path transient Task Policy（root-owned mode 0600、SHA-256 `943b7d153bd18156c23616ebf2fe0cca60ed0661f1ccc31da8a93371cc6ef804`）を選択する。
- alpha.34はread-only live inventoryでArtifact ID `9759273312`のunexpired immutable 1件、ledger `latest_immutable=2.0.0-alpha.34`／`candidate=null`、canonical `breaking_change=true`を確認した。修正前のfresh settled-state readbackは`artifact manifest Public OpenAPI mismatch`を再現した。原因は`verification_target()`がvalidated settled immutable recordから`breaking_change`を伝播せず、manifest verifierがdefault falseと比較したことである。
- canonical breaking metadata validatorをcandidateとimmutable recordで共通化し、settled targetへそのvalidated値を伝播した。candidate／settledのbreakingとnon-breaking manifest verification、missing settled breaking metadataのfail-closed、focused Artifact 24、full Release 47、Policy Unit 191、local Policy Gate 1,626、Python compile、release source validation、actual alpha.34 settled readbackをPASSした。Artifact republish／manifest rewrite／immutable Artifact mutation／ledger semantic mutation、Migration、Preview／Production Activation、Runtime Build、DB／Rank／Payment／Auth／Coin／Draw mutationは各0である。

## GOV-029 GitHub App Git Wrapper / Clean Base Sync

- live protected `main@2bc8598867391d6d50c6e3d10cd9f90a5677a6c7`からBranch `fix/GOV-029-github-app-clean-base-sync`を専用Worktree `/var/www/oripa-worktrees/GOV-029`に作成した。Issue／Source Lockなし、Risk R4、Lane `Strict Change`、Application Runtime Activation `none`、Migration Allocation Lockはnot applicableである。Human-facing Task Policyは不要だが、installed wrapper compatibilityのためdelivery時だけexact 13 changed paths／必要操作に限定したroot-owned mode `0600`のtransient `GOV-029` policy（SHA-256 `7ae1fc1377d852021a138a7a9f36b4028f17f63104717b6373e0c3951139cf22`）を使用し、closeoutで削除する。
- installed `/usr/local/bin/oripa-github-app-git`はpre-change SHA-256 `81a5cba7ff3f8dc3ba7eb4decce2aec74fc83b11996e3c43fa7b6cfb0851114f`、`root:root`、mode `0700`を確認した。最初のsource import commit `577f4bb`は`infrastructure/github-app/oripa-github-app-git`とinstalled runtimeのbyte-identical `cmp`／SHA-256をPASSし、その後のclean-sync logic差分を独立review可能にした。GOV-029 merge前のruntime wrapper置換は0である。
- repository source、checksum／runtime metadata manifest、root-only atomic provision／rollback helperを追加した。helperはsource syntax／SHA、固定destination、clean canonical `/var/www/oripa` mainとlocal／origin／live main一致、pre-install SHA、owner／group／mode、symlinkをfail closedに検証し、same-directory temporary file、file／directory fsync、atomic replace、post-install exact verificationを行う。旧wrapperはprivate checksum backupへ保持し、pre-replace failureは旧wrapperを不変、post-replace verification failureは旧bytesをatomic restoreする。credential／secret inputとwrapper self-updateはない。
- `sync-task-branch-base`はexact Remote `H`、live protected `M`、candidate `C`についてparents `[H,M]`、policy base ancestry、H／M ancestry、Hからfast-forward、conflict 0、Git canonical clean merge treeとcandidate tree一致を要求する。Task scopeは`diff(M,C)`だけをexact `allowed_paths`へ照合し、`diff(H,C)`へ入るmain由来pathをTask scopeへ追加要求しない。stale H／Mはvalidation前後とpush直前／直後にfail closed、push refspecにforceは使用しない。実conflictは`sync_conflict_required`でrejectし、manual resolution candidateをacceptしない。
- focused helper／wrapper／provision testsは42 testsをPASSした。positiveはclean sync、複数main-only path、exact parents／ancestry／canonical tree／fast-forward、OPS-027 Stage 3B exact 17 paths＋MIG-100 main pathsを含む。negativeはTask scope混入／rename旧path省略、H／M omission、stale H／M、既知／未知形式のconflict、tree／parent forgery、main／Task omission、non-fast-forward、policy missing、branch／Task ID／base mismatch、installed drift、noncanonical provision root、pre／post replace failure、post-install main authority drift、pre-install checksum driftを含む。all Ops 101、Policy Unit 203、Quality Unit 4、Security Unit 10、local Policy Gate 1,631 tracked files、local Quality Gate（JSON 131／YAML 20／PHP 871／contract 4）、Python syntax、repository source verification、`git diff --check`をPASSした。Backend／DB／Frontend／Build／Browser E2Eはsource scope外のためlocal未実行で、canonical Strict CIに委ねる。GitHub Required Checks、fresh final-head review、merge、旧／新Server provisionは未実行である。
- canonical `policy-gate`へGitHub delivery infrastructureの既存policy／check-run／self-review／base-sync testsと新wrapper／provision testsを実行するfocused stepを追加した。Required Check名、権限、trigger、dependency、他job、assertionは変更しない。`push-new-branch`、`push-task-branch`、Task Policy／branch validation、PR／checks／self-review／squash merge／branch deleteのauthorityは拡張していない。Production／Preview／Application Runtime／V1／OPS-027 PR #437・branch・policy・evidence／DB／Redis／Migration／Nginx／ALB／ACM／DNS／Security Group／Payment／Coin／Draw／Storefront／Provider／Webhook／Secret mutationは各0である。
- PR #438の初回`policy-gate`はPR本文のChanged files省略表現だけをrejectし、実11 pathの明示列挙へ修正した同一Headの再実行でPASSした。続く`security-gate`は2026-08-31で失効したempty dependency advisory baselineだけをfail closedに検出した。元Change成立に直接必要な小規模CI blockerとして、baseからComposer／workspace pnpm／legacy pnpm lockfileを変更せず2026-09-01にfresh auditし、3 surfaceすべてadvisory 0を確認したうえでempty arraysを維持し、baseline review期限だけを2026-09-08へ更新した。Security Unit 10 testsとlocal `security-gate`（tracked 1,631、Composer／legacy pnpm／workspace pnpm finding各0）を再度PASSした。Dependency／lockfile／Security assertion／Gate／permission／protectionの変更・弱体化は0であり、このbaselineを含むexact 12-pathの新Final Headで全Required Checkを再実行する。
- その12-path Headは`policy-gate`と`security-gate`をPASSしたが、`quality-gate`が2026-08-31で失効したV1 ESLint exact-fingerprint baselineだけをfail closedに検出した。V1 source／lockfileを変更せず、CIと同じESLint 9.39.4でfresh read-only lintを実行し、既存9 findings（error 8／warning 1）がfingerprint／path／位置／rule／message hashまで完全一致することを確認した。元Change成立に直接必要な小規模CI blockerとしてbaseline metadataのreview期限だけを2026-09-08へ更新し、local `lint-baseline`とQuality Unit 4 testsをPASSした。Findingの追加・削除・変更、Quality assertion／Gateの変更・弱体化、V1 application mutationは各0であり、このbaselineを含むexact 13-pathの新Final Headで全Required Checkを再実行する。

## GOV-030 GitHub App Wrapper Detached Provision Support

- live protected `main@b757945a8d6f7fa70b562d8a1603944b935fee94`からBranch `fix/GOV-030-github-app-detached-provision`を専用Worktree `/var/www/oripa-worktrees/GOV-030`に作成した。Issue／Source Lockなし、Risk R4、Lane `Strict Change`、Application Runtime Activation `none`、Migration Allocation Lockはnot applicableである。Human指定とprivileged GitHub deliveryのためexact 4-path transient Task Policy（root-owned mode 0600、SHA-256 `177c47006f282317ea091c44b104cc20972b66fcf847728a152098d5f6518937`）を使用し、closeoutで削除する。Primary `/var/www/oripa`のbranch switch／stash／reset／cleanupとOPS-027 PR #437・branch・content mutationは各0である。
- 実装前にclean exact detached synthetic worktreeを作成し、現行`require_merged_main()`がGit metadata確認前の固定root比較で`repository_root_invalid`を返すfocused regressionを再現した。default `/var/www/oripa` clean `main` authorityを維持し、explicit `--repo-root`＋`--expected-head`ではabsolute canonical non-symlink Git worktree、canonical origin、tracked／untracked clean state、exact HEAD、local `origin/main`とlive protected `main`一致、HEAD ancestry、helper／manifest／sourceのsame-checkout regular-file／HEAD blob一致、manifest checksum、source syntax、固定runtime destinationをFail Closedに検証する。loaded bytesもHEADへ固定してinstall直前に再照合し、既存pre-SHA lock、same-directory staging、fsync、atomic replace、root:root mode 0700、post-install checksum、automatic restore、rollback contractは変更しない。Git wrapper source／manifest、clean base sync semantics、Production／Preview／Application Runtime、Migration、DB／Redis、Auth／Payment／Coin／Draw／Storefront／V1 mutationは各0である。
- exact current treeでProvision focused 32、GitHub delivery focused 81、all Ops 124、Policy Unit 203、Quality Unit 4、Security Unit 10をPASSした。local `policy-gate`はtracked 1,631、`quality-gate`はJSON 131／YAML 20／TOML 1／XML 2／PHP 871／contract 4、`security-gate`はComposer／workspace pnpm／legacy pnpm finding各0・secret candidate 0でPASSした。Python compile、CLI help、`git diff --check`、canonical wrapper SHA-256 `b5415d916a2a0e2263b51d57e955b3c97154cbe23c02b37f32a0837b9433e90f`、baseからwrapper source／manifest content diff 0もPASSした。Backend／Frontend／DB／Build／Browser E2Eはsource scope外のためlocal未実行で、Required 5はGitHub CIに委ねる。Host wrapper Provision／rollback／Runtime Activationは未実行である。

## GOV-031 GitHub App Provision Safe Directory Compatibility

- GOV-030 PR #439はFinal Head `c4bb2320a0f2a6b0c1317382b4f1b71cc413e63d`、Required 5 PASS、fresh MAX self-review finding 0でsquash mergeされ、protected `main@f555ae633ae8e6a3774506f366659aada1f7834c`となった。merge後のread-only verificationでdefault `verify-source`はPASSした一方、同じcanonical mainをexplicit `--repo-root /var/www/oripa --expected-head f555ae633ae8e6a3774506f366659aada1f7834c`で検証すると`repository_git_worktree_invalid`を再現した。helperがglobal／system Git configを安全上無効化した結果、root実行時にowner `codex:codex`のcheckoutへhost global `safe.directory`が適用されず、explicit verificationとdefault install authorityがGitのdubious ownership保護で停止することを根因と確定した。Host wrapper install／rollback、Primary branch switch／stash／reset／cleanup、OPS-027、Production／Preview mutationは各0である。
- live protected `main@f555ae633ae8e6a3774506f366659aada1f7834c`、履歴／Worklog／Task Policy／worktreeで未使用のnext ID `GOV-031`を確認し、Branch `fix/GOV-031-provision-safe-directory`、専用Worktree `/var/www/oripa-worktrees/GOV-031`、Issue／Source Lockなし、Risk R4、Lane `Strict Change`、Activation `none`で是正を開始した。GitHub deliveryだけにhelper／focused test／Worklogのexact 3-path transient Task Policy（root-owned mode 0600、SHA-256 `21973a2ecb2b3a356475064e90af2bf408efc7e6d1fbe700fbf12006b582831f`）を使用する。sanitized Gitがcheckout単位のexact `safe.directory`なしではcode 128を返すfocused regressionを実装前にPASS期待で実行し、`repository_git_worktree_invalid`のERRORを再現した。
- global／system Git config、hooks、fsmonitor、optional locksの無効化を維持し、検証済みabsolute canonical non-symlink Repository rootだけを各local Git commandのcommand-line `safe.directory=<exact-root>`へ付与する。wildcard `safe.directory=*`、global config mutation、caller config trust、GitHub write authority追加は行わない。live protected mainのpublic `ls-remote`にはlocal safe-directory設定を渡さず、origin／clean status／HEAD／ancestry／HEAD blobの全local commandとpost-install authority再検証だけをexact rootへbindする。wrapper source／manifest／clean base sync semantics、atomic install／pre-SHA／checksum／rollback contractは変更しない。
- fixed treeでroot mismatch regressionを含むProvision focused 33、GitHub delivery focused 82、all Ops 125、Policy Unit 203、Quality Unit 4、Security Unit 10をPASSした。local `policy-gate`はtracked 1,631、`quality-gate`はJSON 131／YAML 20／TOML 1／XML 2／PHP 871／contract 4、fresh dependency auditはComposer／workspace pnpm／legacy pnpm finding各0、`security-gate`はsecret candidate 0を含めPASSした。sanitized環境のpatched helperからowner `codex:codex`のactual `/var/www/oripa`へexact safe-directoryを渡したread-only HEAD checkも`f555ae633ae8e6a3774506f366659aada1f7834c`を返した。Python compile、CLI help、`git diff --check`、exact 3 changed paths、canonical wrapper SHA-256 `b5415d916a2a0e2263b51d57e955b3c97154cbe23c02b37f32a0837b9433e90f`、wrapper source／manifestのbase diff 0もPASSした。Host wrapper Provision／rollback／Runtime Activation、Build、Migration、Application／Production／Preview／OPS-027 mutationは各0である。

## OPS-027 Platform Production ARM64 Runtime / Artifact Unblock

- `git fetch origin --prune`後にlocal `main`をsafe fast-forwardし、clean local／origin／GitHub `main@a8a45a5673ca603b8fe1abaf5b9642038776e37e`をBaseとしてBranch `infra/platform-production-arm64-runtime`を作成した。Issue／dedicated Worktree／Source Lock／Task Policyなし、Migration作成／適用0のためMigration Allocation LockはN/A、Risk `R4`、Lane `Strict Change`、Activation `none`である。
- canonical Artifact helperをschema v2へ互換拡張し、Artifact kind、source SHA、explicit architecture、image reference／ID、OCI revision、archive SHA-256／sizeをFail Closedで検証する。legacy v1はPreview `linux/amd64`としてのみ安全に受理し、Preview default／image name／workflow／host guardはamd64のまま維持する。ARM64をPreview defaultへ置換せず、Production candidateだけをexplicit `linux/arm64`として追加する。
- APIはPHP 8.4 Apache production target、deterministic Composer `--no-dev`、port 8000、non-root `www-data`、OCI revision labelを追加し、既存Preview targetはdefaultのまま維持する。Adminはcanonical Node 22.22.3／pnpm 10.12.1 standalone targetをexplicit production authorityとし、port 3000、non-root `node`、OCI revision labelを維持する。Admin healthのstale skeleton placeholderはtrueへ偽装せず削除し、process-only readiness scopeを明示するexact testを追加した。
- PR Strict laneへnative GitHub-hosted ARM runnerのadditive verification jobを追加し、API／Admin build、architecture／OCI provenance、Artifact package／verify／loadを検証する。別manual workflowはmerged internal PR、exact protected main、Required 5成功を一致させた後だけimmutable Production candidate Artifactを作成するmerge-first authorityであり、deploy／SSH／Runtime Activationを含まない。
- actual t4g／aarch64でAPI／Admin production imageをnative preflight buildし、architecture `linux/arm64`、non-root user、OCI base revisionを確認した。Admin build image内の全36 files／213 testsはPASS、APIはloopback-only temporary smokeでLaravel deep healthへ到達しDB／Redis／storage未提供を理由にexpected 503、Adminはloopback-only temporary smokeでscoped process health 200を返した。temporary containersは削除し、QEMU／emulation、DB／Redis起動、Migration、public bind、Production credentialは各0である。
- Artifact unit 36、Policy unit 201、changed Python compile、`git diff --check`をpre-finalでPASSした。final exact-head native images、Artifact package／verify／load、Required 5、fresh Strict self-review、squash merge、merged-main Artifact creationは継続する。
- Existing Production／Shared Preview Runtime、Storefront Repository、Production DB／Redis／secret、Nginx、ALB／Target Group／ACM／DNS／Security Group、application trafficはaccess／mutation／activation各0。Production PostgreSQL／Redis、Assets、Storefront runtime、full Compose、host ports、Nginx／ALB routing、Production secrets／deployment／activationは後続Human checkpoint付きStageへ明示的にdeferする。
- Stage 3B-Closeout再開時、新t4g hostのbroker／autonomy／API read／API write／Git／check gate全6 runtime SHA-256が旧Server forensic Authorityと一致し、current `task_policy.py`／`task_branch_base_sync.py`／`self_review_policy.py`のhelper 28 tests、GitHub App repository／protected main readbackをPASSした。`ec2-user`を維持し、root-only operationだけ既存`sudo`経路を使用、PAT／direct broker／wrapper変更／permission変更／専用sudoersは各0である。
- preserved Branch `infra/platform-production-arm64-runtime@03486885f5073a3c9c52c46f4a163e07287b7f07`をHEAD不変で`chore/OPS-027-platform-production-arm64-runtime`へrenameし、live protected `main@22ba6ae2da393fdbe42a67c136d1a3e4efdf09ff`へrebaseした。並行MIG-099／GOV-027のApplication／Migration／Contract／Artifact ledger変更を保持し、重複`policy_gate.py`は独立加算、worklog conflictは双方の正当なappendを保持して解決した。rebase candidate `ebea81737ecf8b846009cc7621b59c5fabb6b8ea`はlatest main ancestor、clean、Stage 3B 16-path diff残存をPASSした。
- rebase candidateでArtifact helper／architecture negative／Preview amd64 regression 36 tests、Policy 203 tests、local Policy Gate 1,631 tracked files、changed Python compile、`git diff --check`をPASSした。native aarch64 host／DockerでAPI／Admin Production build、`linux/arm64`、OCI revision、package／verify／loadをPASSし、pre-final image IDはAPI `sha256:d70dd3c342d547731d782dc2974926b3280be49bd2939c7959ee3b737b915882`、Admin `sha256:dca0e00b59dbe0a434499dc0c59edafa64ffba2939bd1685bd745e5980619c21`、Manifest SHA-256 `5f966871299717b8e10a6c6627d826af1f96f22258d1cd9f74c9a11d47c0f9d4`である。これはFinal Head前のlocal evidenceでありcanonical PR／merged-main Artifact authorityには使用しない。
- Admin full suite初回は既存Announcement fixtureの公開終了時刻が実行日当日`2026-08-31T14:59:59Z`を越えたため1 failure／199 PASSとなった。Stage 3B成立に直接必要なbounded CI blockerとして対象test内の`Date.now()`だけを公開期間内へ固定し、production logicと「公開」count assertionを維持した。focused 3、full 36 files／200 tests、typecheck、lint、production buildをnetworkなしnative ARM64 builder containerで最終PASSした。Application behavior、API／Contract、DB／Migration、Storefront、Runtime／Production／Preview mutationは各0である。
- Final Conflict CloseoutでRemote H `58b486ec1103bcecef168694dcb462440859ac9b`とprotected main M `065f149df80bcde00c6bc8298304738613d07726`間のreal conflictが本WorklogのOPS-027 appendだけであることを確定した。OPS-027 appendだけを一時除外したH1 `4803dc85069e4cb86bc1af8b7e014a6b07213953`へno-force fast-forwardし、parents `[H1,M]`、conflict 0、canonical tree `326632cbdda58be9fda37a48cfe5314f94888b4e`のC `5647a521117d8b4660f02c5a2a5cb820db158702`へapproved `sync-task-branch-base`をPASSした。Required 5、fresh final-head review、squash merge、merged-main ARM64 Artifactはこれらが完了するまでauthorityに使用しない。

## REPAIR-001 Admin Rank Image Asset URL Repair

- clean local／origin／GitHub protected `main@4b1cf658ca2f008e0e69161e276d21676964b38d`をBaseに、Branch `fix/RANK-IMG-REPAIR-001-admin-asset-url`、通常Worktree、Issue／Source Lock／Migration Allocation Lockなし、Risk `R4`、Lane `Strict Change`、Activation `immediate`で開始した。Test runtimeはAPI `279d196130f0205df995bbf56ff415a763478c2b`、Admin `0944ca837dc980a7a2650421c4ce2e8f774d22b4`、healthy／restart 0、DB `oripa_v2_mig061a`、Migration 000069／000070 batch 35、Storage disk `local`／persistent volumeを確認した。
- Human Browser作成RankはDB唯一の `01a05ced-5762-7331-971e-71147842b012`（`１等賞`、Revision 1）と確定した。lineup Asset `01a05ced-5759-70af-8788-2d95147743e5`／result Asset `01a05ced-575f-7300-9e84-7cd603c80f87`はAsset row、Revision FK、public image metadata、file、size、SHA-256がすべて一致し、Public origin／loopbackでは両方 `200 image/png`。同じ相対 `/api/v2/...` はAdmin originで両方404となり、Nginx logにも実browser 404を確認した。
- Root Causeは`D_ADMIN_RENDERING_DEFECT`。AdminのRank画像だけがPublic API snapshotの相対pathをAdmin originへ直接渡していた。既存Rank Effect／Bannerのcanonical patternと同じ、Asset public IDから構成する認証付きsame-origin `/admin/api/v2/catalog/presentation-assets/{id}/content`をRank Master一覧／編集previewとGacha Rank表示へ適用した。Backend upload、Presentation Asset、Storage、Revision、Public API／Storefront contractは変更せず、既存Human Rankのdata repair／再uploadは不要である。
- Admin full 36 files／201 tests、Rank Backend focused 6 tests／110 assertions、Prize／Banner／Rank Effect upload回帰20 tests／252 assertions、Admin typecheck／lint／production build、`git diff --check`をPASSした。追加Backend testはcreate時の2 Asset ID／row／file／Admin HTTP `200 image/png`、edit時のnew Revision／new lineup Asset／old Revisionとold file不変を固定する。PHP 8.3 host停止と、隔離PHP 8.4 test DB未作成／初回接続設定不一致はassertion実行前の環境preflight failureで、Human Test DBを変更せずMIG-099隔離DBに新規`oripa_test`を作成して000001–000070適用後にPASSした。
- Public Contract change、Storefront Artifact republish、Migration作成、Test live DB／Asset mutation、API Activation、Production Activation／DB／Asset mutation、Payment／Auth／Coin／Draw mutationは各0。Final Head Required 5、fresh Strict self-review、squash merge、Admin-only Test Activation、Human Rankのpost-activation HTTP／thumbnail path確認を継続する。Browser E2EはCodex用Admin MFA sessionがないため現時点では未実行である。

## SEC-016 Superseding Repository Security Advisory Remediation

- clean local／origin／live protected `main@94c92f000997049eca65c197c99e6fb8bdde1416`をBaseに、Issue `#449`、Branch `security/SEC-016-superseding-advisory-remediation`、dedicated Worktree `/var/www/oripa-worktrees/SEC-016`、Risk `R4`、Lane `Strict Change`、Activation `none`で開始した。GitHub deliveryとHuman指定scope isolationのためexact 4-path transient Task Policy（root-owned mode 0600、SHA-256 `1d9094e7d77ffa11a3aa11c898c734ad25b4b4f130ac03f11934cfed90a6d4c0`）を使用する。OPS-028 PR `#444`／branch／worktreeとSEC-014／015 sourceは変更していない。
- SEC-014／015のbranchをmergeせずread-only diffを照合し、protected mainへvalidated minimum resolver resultsだけを再適用した。`apps/api/composer.lock`は`league/commonmark 2.9.0 -> 2.10.0`の6 insertions／6 deletionsだけ、`legacy/v1-frontend/pnpm-lock.yaml`は`browserslist 4.28.2 -> 4.28.7`と不可避な4 package closure、version不変`update-browserslist-db 1.2.3` peer bindingの25 insertions／25 deletionsだけである。各lockはSEC-014／015 validated lockとbyte-identical、unrelated update 0、manifest／root pnpm lock／security baseline／workflow変更0である。
- repository Node `22.22.3`／pnpm `10.12.1`でroot／legacy frozen install、両audit High 0・Critical 0、Browserslist 4.28.7 direct package resolution／default target CLI、legacy typecheck／Next.js production build 24 routes、Admin 36 files・201 tests／typecheck／lint／production buildをPASSした。Security Unit 10、Quality Unit 4、Policy Unit 203、Local Policy Gate 1,637 tracked files、release source validation、`git diff --check`もPASSした。
- Migrationは70 files、canonical content-set SHA-256 `8bc21870df410bf08b8753aed102cba06351e95c3ae2ab1a8e945d99f8206861`でBaseと一致し、作成／変更／適用0。HostにPHP／ComposerがなくDocker daemonも停止中のため、Composer frozen install／auditとCommonMark／full API regressionはlocalで再実行せず、byte-identical SEC-014 evidence（focused 26 tests・231 assertions、full Feature/API 286 tests・1,596 assertions）をpre-CI evidenceとして保持し、SEC-016 exact Final HeadのRequired `security-gate`／`integration-gate`をcanonical authorityとする。Docker service、Production／Shared Preview runtime、RDS、Redis、Nginx、AWS、Secret、Stage 6A mutationは各0である。
- Final changed paths、secret／PII／binary／submodule、minimal lock closure、Migration／OPS-028 isolationをreviewし、Draft PR、Required 5＋available checks、fresh exact-head Strict self-review、squash mergeを継続する。SEC-016 merge後だけPR `#446`／`#448`をwithout mergeでcloseし、Issue `#445`／`#447`をSupersededとしてcloseする。

## OPS-028 Stage 6A-FIX API `/up` Health Route Registration — Resume

- Stage 5B／Stage 6A authorityのclean protected `main@94c92f000997049eca65c197c99e6fb8bdde1416`からIssue #443、PR #444、Branch `fix/OPS-028-api-up-health-route`、専用Worktree `/var/www/oripa-worktrees/OPS-028`、Risk `R4`、Lane `Strict Change`、Activation `deferred`で開始した。Migration作成／適用0のためMigration Allocation LockはN/Aである。
- Laravel 13.15.0 `ApplicationBuilder::withRouting()`は`using`指定時に`buildRoutingCallback()`を呼ばず、`health` routeとmaintenance exceptionの両方を登録しない。custom callbackをcanonical `then` hookへ移す一語修正により、framework自身の`DiagnosingHealth`／canonical responseを使う`/up`を先に登録し、既存Public API／Admin API／Webhookのprefix・`api` middleware topologyをそのまま維持する。
- Repository-wide Advisory HOLD後、live protected `main@5c1cd6e798ac1c448ebdcf43071f83145ed02d24`（SEC-016）を再確認した。唯一のreal conflictは双方のWorklog appendで、OPS appendを一度相殺したRemote H1 `e56486f1ca4a30723fe568323fb0f4a81b1ade69`からparents `[H1, main]`、canonical tree一致、source conflict 0のC `c2665327147667accfbcc65b1d4afea730465682`へ`sync-task-branch-base`をPASSした。SEC-016記録を保持し、このOPS記録を時系列末尾へ復元する。
- synchronized treeは`league/commonmark 2.10.0`、root／legacy `browserslist 4.28.7`を保持し、fresh Composer／root pnpm／legacy pnpm auditはいずれもHigh 0／Critical 0、canonical local `security-gate`はbaseline mismatch 0、secret candidate 0でPASSした。OPS-028 dependency／manifest／lockfile／baseline／ignore／bypass mutationは0である。
- native ARM64 isolated API imageでHealth focused 3 tests／17 assertions、full Laravel 344 tests／1,865 assertions、Composer strict validation、changed PHP syntaxをPASSした。local Policy Unit 203、GitHub delivery 82、Quality Unit 4、Security Unit 10、Policy Gate 1,638 tracked files、Quality Gate JSON 133／YAML 21／TOML 1／XML 2／PHP 871／contract 4、`git diff --check`をPASSした。
- route inventoryはcurrent main 447から448となり、`/up` exact 1、`GET|HEAD`、middleware 0、duplicate 0である。`/up`除外後の全route normalized SHA-256はmain／OPSとも`8047b6a114b2e1c51e75040d9b82286c90dad84480c8d4d5c46106a382793ded`で一致し、Public 76／Admin 219／Webhook 2を維持した。DB／Redis未提供のisolated processでも`GET /up`はdefault canonical HTML 200、JSON Acceptは`{"status":"up"}` 200、deep `/api/health`はpreserved 503である。
- Migrationは000001–000070の70 files、content-set SHA-256 `8bc21870df410bf08b8753aed102cba06351e95c3ae2ab1a8e945d99f8206861`でBase／SEC-016 main／synchronized tree間が不変。Repository migration作成／変更／適用、Production RDS／SQL／Seeder、Production Redis／API／Admin／Storefront Activation、Nginx／AWS／DNS／Secret mutationは各0である。final Worklog-only headでhead-sensitive local gates、Required Checks、fresh exact-head Strict self-review、squash merge、merged-main exact ARM64 Artifactを継続する。
## SMS-001 SMS OTP Phone Ownership Verification / SMS FourS

- Human最終DecisionとPreflightをAuthorityに、clean local／origin／GitHub protected `main@94c92f000997049eca65c197c99e6fb8bdde1416`からBranch `feat/SMS-001-sms-verification-fours`を通常Worktreeに作成した。Issue／dedicated Worktree／Human-facing Task Policy／exact allowed paths／Source Lockは使用せず、Risk `R4`、Lane `Strict Change`、Application Runtime Activation `deferred`とした。Production変更・Production Activation・Shared Preview実SMS送信は各0である。
- Root／API／Admin／OpenAPI／Package／Docs／Infrastructure AGENTS、Governance REV3、Release Gates REV2、GitHub Operations ADR REV1、Identity Security REV1、API／Client Contract、Package Compatibility、migration source／live Shared Preview ledger、Artifact release ledgerを再確認した。Migrationはsource／live DBとも`000001–000070`一致、pending 0、Artifactは`2.0.0-alpha.34`がlatest immutable、candidate nullだった。Migration Allocation Lockをlive取得・保持し、本Changeで未使用`000071`、Contract additive Artifact候補`2.0.0-alpha.35`を使用する。
- JP 070／080／090 validation、E.164 canonical ownership、Phone 3/hour・10/day、SMS IP limitなし、60秒cooldown、5分6桁hash-only OTP、accepted delivery限定Verify、5 failure失効、DB unique loserのgeneric Problemを実装した。Initialは通常Session、Phone ChangeはFresh Authentication必須とし、Challenge開始時のold phone releaseを廃止してVerify transaction内のatomic switch、current Session／CSRFだけのrotation、other Sessions／Remember Device維持、`phone_changed` Mail Template exactly-onceを固定した。
- V2専用FourS adapterとencrypted Outbox workerを追加した。HTTPS form POST、explicit User-Agent、国内11桁address、safe Provider category、pending／sending／accepted／failed／unknown lifecycleを実装し、ambiguous provider boundaryはunknown terminalとしてblind retryしない。Legacy `LogSmsSender`はV2 interfaceへbindせず、OTP／Phone／Provider secretをLog／Audit／Problem／plain Outbox payloadへ出さない。
- Public address list／detail／create／update／deleteとshipping request createへBackend SMS Gateを追加し、既存shipping viewは維持した。Admin User Detailはread-only SMS status／full verified phone／verified_atを追加し、mutation surfaceは追加していない。Public／Admin／Webhook Contract metadataをalpha.31、Client／Testkitをalpha.35へ同期し、`identity.sms-phone-ownership.v2` capabilityとpending immutable candidateを記録した。
- Additive Migration `000071`はChallenge delivery state／safe Provider correlation／timestampsと固定`phone_changed` Mail Templateを追加し、legacy active Challengeをfail closedにする。Final isolated PostgreSQL 17でMigration `000001–000071` fresh up、`000071`単独rollback／reapplyをPASSし、Shared Preview／Production applyは0である。
- V2 full 597 tests／6,086 assertions（repository既定10 skip）、SMS／Shipping／Admin focused 79 tests／904 assertions、OpenAPI 10 tests＋bundle 219／75／2、Client 33、Testkit 44＋exports／network、Site Schema 10、Admin 36 files／201 tests＋typecheck／lint／production build、Release 47／source validation、Policy 203、Quality 4、Security 10、DB 44、Site Template 6、Composer validateをPASSした。Composer／workspace pnpm／legacy pnpm fresh auditは各finding 0である。
- PR #450はPR metadataと2回のadditive Contract修正をfail closedで検出・是正した。続いてMigration CHECKの暗黙castがPostgreSQL restore後に等価な別表現へdeparseされchecksum差となることを実dumpで特定し、explicit `text = ANY (text[])`へ正規化した。Final prior head `4a31ac05333291c02bde8cd987604318f598ae00`のRequired 5とcanonical CI backup／restore smokeはPASS、fresh self-reviewはSEV-0／1各0であった。
- merge直前live readbackでprotected mainがSEC-016 `5c1cd6e798ac1c448ebdcf43071f83145ed02d24`へ進み、CommonMarkとBrowserslistの独立Security修正が先行mergeされた。SMS design／Migration／Contractの変更は0と確認し、重複するComposer／legacy lock修正はSEC-016 mainをAuthorityに取り込んだ。Worklogは両Changeを保持し、installed wrapperのcanonical merge tree検証、conflict 0、no-force fast-forwardでtask branchをlatest mainへ同期した。GitHub delivery compatibility上のtransient Task Policyは同期に必要なoperationだけを追加し、closeoutで削除する。
- Human RESUME Authority `4ff2540c763bb9a507c82df2bb00f1495510a33b`とlive／local／origin mainを一致確認し、pre-resume head `a7423828de2dfe8250463e21a8ef7a68366dae7c`から指定mainへsafe-syncをexact 1回実行した。OPS-028のHealth Routeとtestを保持し、source conflict 0、canonical merge tree一致、SMS/Auth/Migration/Contract semantic drift 0である。Migrationはbase `000001–000070`不変／SMSの新規`000071`だけ、`league/commonmark 2.10.0`／legacy `browserslist 4.28.7`を維持し、SMS固有の`composer.json`／`apps/api/composer.lock`／legacy lock deltaは0である。pre-resume Checks／reviewはFinal authorityに再利用せず、同期後Final HeadのRequired 5／fresh Strict self-reviewを再実行する。

## SEC-017 Repository Security Dependency Advisory Remediation

- SMS-002 PR #452のRequired `security-gate`を阻害した2026-09-02公開advisoryを独立Changeで解消する。clean local／origin／GitHub protected `main@7942268281450257dcb76f38c8be8743b1c66be6`からBranch `security/SEC-017-dependency-advisory-remediation`を通常Worktreeに作成し、Issue／dedicated Worktree／Task Policy／Source Lock／Migration Allocation Lockは使用しない。Risk `R4`、Lane `Strict Change`、Application Runtime Activation `none`である。
- PR #452 exact Head `ecb076b212ddfe52f6ac287462367841ef4a5bf4`のsecurity-gate job `100481972260`はworkspace `pnpm audit` findingでFAILした。fresh auditは`@tiptap/core 3.30.3`をGHSA-cp6q-959q-f8rh（affected `<3.30.4`、patched `3.30.4`）としてModerate 1件、`fast-uri 3.1.5`を4件のHigh advisory（v3 patched `3.1.6+`）として検出し、resolverは現行v3 line `3.1.7`をtargetに示した。
- 必要最小限としてAdminのdirect Tiptap 5 packagesだけをcoherent `3.30.4`へ揃え、transitive `fast-uri`の既存root overrideを`3.1.7`へ更新する。SMS-002 source／TTL／phone validation、Migration、Account Security、Composer、Production、Shared Preview activationは変更しない。
- `@tiptap/react`のoptional menu packagesがminor浮動してpeer不整合にならないよう、既存root override conventionで`@tiptap/extension-bubble-menu`と`@tiptap/extension-floating-menu`だけを`3.30.4`へ固定した。frozen workspace install、fresh `pnpm audit` finding 0（High 0／Critical 0）、Rich Text Editor focused 2 files／6 tests、Admin typecheck／lint／production build 45 routes、`git diff --check`をPASSした。Migration作成／変更／適用、Composer manifest／lock、Browser E2E、Build artifact publication、Runtime／Shared Preview／Production mutationは各0である。
- PR #453初回Head `78229d6065662186ecd991f4720cf6c089d0ad0c`は既存policy-gateのexact dependency allowlistが旧Tiptap／fast-uri版を固定していたためFAILし、後続Checksはskipされた。同一Head rerunは行わない。dependency guardを弱めずremediated exact versionsへ同期し、そのfixtureだけを更新してPolicy Unit 203 testsをPASSした。新HeadでRequired 5を1系列実行する。
- 次Head `19098ac83e8071a389b4c0aa011acf820c5c08af`はdependency guardとPolicy Unitを通過したが、PR本文がcanonical見出しschemaを満たさずrepository policy stepでFAILし、後続Checksはskipされた。同一Head rerunは行わない。source／dependency／test変更なしでPR本文をcanonical `Task`／`Summary`／`Specification sources`／`Scope`／`Verification performed`／`Verification not performed`構造へ正規化し、記録更新だけの新HeadでRequired 5を実行する。

## SMS-002 SMS OTP TTL / Verified Phone Pre-send Validation — Resume

- Human Browser確認PASSと追加Human DecisionをAuthorityに、clean local／origin／GitHub protected `main@7942268281450257dcb76f38c8be8743b1c66be6`からBranch `fix/SMS-002-sms-ttl-phone-precheck`を通常Worktreeに作成した。Issue／dedicated Worktree／Task Policy／exact allowed paths／Source Lockは使用せず、Risk `R4`、Lane `Strict Change`、Application Runtime Activation `deferred`とした。Source／Shared Preview migration ledgerは`000001–000071`一致、Shared Preview batch 36／`000071 Ran`、`000072`未使用を確認し、Migration Allocation Lockを取得・継続保持してHuman承認済み`000072`を割り当てた。
- OTP TTLを60分へ変更し、FourS本文の期限表示も同一Configへ同期した。Active／Suspended等の再利用不可Verified Phoneをsend／resendのChallenge発行前にgeneric `PHONE_NUMBER_UNAVAILABLE`で拒否し、Challenge／Outbox／Provider requestを作らない。未認証重複、Closed／Anonymized再利用、self-number、Phone Changeのold phone保持を維持し、Verify transactionのadvisory ownership lock／release rule／DB unique constraintを最終Authorityとして保持する。
- Additive Migration `000072`で既存5分固定CHECKを`expires_at > created_at`かつ最大24時間のDB safety ceilingへ置換し、適用時に既存Challengeを更新しない。製品TTLは`V2_SMS_OTP_TTL_MINUTES`（default 60）を唯一の発行Authorityとし、1–1440分のintegerだけをstartup／利用時に許可する。発行時のvalidated TTLをChallengeと暗号化Outboxへsnapshotし、FourS本文も同じ値を使うため、ceiling内の将来変更はMigration不要で既存Challengeの期限も不変である。
- Guarded isolated PostgreSQL 17 migration lifecycleは000001–000072 fresh upを2回、000072 rollback／reapply、statusをPASSした。新Migration test fixtureだけの初回errorを修正後、Migration境界1 test／4 assertions、SMS／FourS focused 35 tests／268 assertions、Verify／same-phone concurrency 2 tests／15 assertionsをPASSした。同じHeadの不要なfull rerunは行わずRequired ChecksをFinal authorityとする。
- PR #452のpre-resume final Head `ecb076b212ddfe52f6ac287462367841ef4a5bf4`はpolicy／qualityをPASS後、2026-09-02公開workspace dependency advisoryでsecurity-gateがFAILした。Human判断どおりSMS PRへdependency変更を混在させずSTOPし、独立SEC-017 PR #453のSquash Mergeを待った。
- Human RESUME Authority `1157b57bc728ca260b9921b4ace0ca4269f88f8c`をlocal／origin／GitHub protected mainで一致確認した。SEC-017はMigration変更0、main ledger 000001–000071、SMS-002の000072 collision 0。pre-sync HeadのSMS worklog appendを一時除外したfast-forward Head `fc2417092f34faa53664fa07abce1756c602e1f2`からcanonical conflict-free treeへsuccessful safe-syncをexact 1回実行し、sync Head `7688b5ae316b212c99404f4b9cbea0698798734b`、tree `e31de0cb12c3002cd13c896a1cf0555d25d90b48`、conflict paths 0を得た。この記録復元はsafe-sync後の通常fast-forward commitである。
- safe-sync後のSMS service／config／migration／focused testsはpre-sync Headとbyte-identical、new mainとの差分はSMS exact 14 source/test/policy pathsに本Worklogを加えた15 pathsだけである。SEC-017の`@tiptap/* 3.30.4`／`fast-uri 3.1.7`、High 0／Critical 0を保持し、SMS固有の`package.json`／`pnpm-lock.yaml`／`composer.json`／`apps/api/composer.lock` deltaは0。OpenAPI／Client／Testkit／Artifact／Storefront sourceも変更しない。
- Issue／Task Policy ceremonyはHuman判断どおり不採用。installed GitHub App wrapperのprivileged delivery hard requirementだけを満たすため、SMS exact 15 pathsとsafe-syncで継承したSEC-017 dependency 3 paths、必要11 operations限定のroot-owned transient compatibility manifestをcommit外で使用し、merge／branch cleanup後に削除する。Shared Previewは旧Source／000071のままで、merge後の別Activation stepで000072 applyとSource reactivationが必要。Production mutationは0である。
- post-sync Head `edd2bbac139f06f7074b92ca9b409cf9a6291ae7`のpolicy-gateはPR event bodyの`Base SHA`がpre-resume AuthorityのままだったためFail Closedし、後続Checksはskipされた。live PR bodyをResume Base `1157b57bc728ca260b9921b4ace0ca4269f88f8c`へ先に更新し、同一Head rerunなしで記録だけを加えた次Headへ進む。

## OPS-029 Initial Owner Bootstrap Persistent Audit Allowlist Fix

- clean local／origin／live protected `main@6fbf41374edd23b30c4c2fb5672c0be4823c66cb`をBaseに、Issue #454、Branch `fix/OPS-029-initial-owner-audit-allowlist`、専用Worktree `/var/www/oripa-worktrees/OPS-029`、Risk `R4`、Lane `Strict Change`、Activation `deferred`で開始した。exact 4-path transient Task PolicyをGitHub deliveryだけに使用し、Migration Allocation LockとSource LockはN/Aである。
- commandはMIG-042以来のcanonical event `admin_invitation`を発行し、persistent action `identity.admin_invitation`も同時にallowlist済みだった。MIG-061BがInvitation OFFのtemporary-password mode追加時にcontext `mode`をcommandへ加えた一方、persistent sinkのcontext allowlist／metadata projectionと直結testを更新しなかったため、event名が正しくても同じfail-closed例外で拒否されることをhistoryとsourceで確定した。
- persistent sinkへ既存canonical context名`mode`を2箇所だけ追加し、`invitation`／`temporary_password`をaudit metadataへ保存する。wildcard、allow-all、exception握り潰し、audit削除、transaction変更は0で、unsupported eventとunsupported contextは同じ例外で拒否し続ける。
...
## OPS-033 Public Storefront / Admin Nginx Activation

- HumanがStage 7D `READY_FOR_HUMAN_GO`とALB Console最終確認完了を明示し、生成済みHuman Cutover Commands B／Cの実行を承認した。Issue `none`、Branch `chore/OPS-033-public-frontend-admin-activation`、通常Worktree、Base `6b71148c50e78cb506006beeb90a189c6a6c18ab`、Risk `R4`、Lane `Strict Change`、Activation `immediate`で開始した。Repository変更は本Worklogだけに限定するroot-owned mode `0600` transient Task Policyを使用し、Migration Allocation Lock／Source LockはN/Aである。
- Task開始時のPlatform local／origin／live protected `main`は`6b71148c50e78cb506006beeb90a189c6a6c18ab`へ一致しclean、Storefront local／origin／live protected `main`は`c0455220761ac752a9acf22c6914df748ae51d55`へ一致しcleanである。Task中のmain再同期は0。Platform Runtime Authority `b6541e526c4943c03c3533f7cbc9d3ca560a2567`からApplication／Migration／Composer／dependency差分0、Storefront Application Authority `39ba749914aec1142e5618aa7f5688f93feff5f7`のtreeは`1d7cb74488a911e95a907de292d586f37833f06d`へ一致した。
- PRE-CUTOVERでcurrentはexact release `/var/lib/luxe-pack-storefront-production/releases/39ba749914aec1142e5618aa7f5688f93feff5f7`、Manifest SHA-256 `34e7bc0ed7aea7cb19dec6cb9e34fdb4fcfbc2c3f4eeffa9da52aea1f1943d67`、File Manifest SHA-256 `6c0a2fc1232912f03384ddde8aade7d34252bdaa187106b77b103c9d737992e6`、全`FILE_SHA256SUMS`、Archive evidence `1ee7c0fb82834aa6bbe5e770498a778bcddaf53962e4585d26c761d2e163e04f`／private activation PASS、Build ID `d1KU3RU0iwhO7hTiwX7vu`を再確認した。Storefront unitはenabled／active／restart 0、listenerは`127.0.0.1:3200`だけである。
- PRE-CUTOVER healthはStorefront `/`／`/points`／SMS verification route各200、API `/up`／`/api/health`各200、Admin `/api/health` 200、Redis authenticated PONG、API／Admin／Mail worker／SMS worker runningをPASSした。guarded Production readbackはMigration ledger 72／latest `000072`／batch 3、SMS TTL 60、fincode disabled、Owner exact 1／active 1／verified 1。Assetsは76 files／61,799,877 bytesを維持した。EnvironmentFile未適用の通常`artisan migrate:status`はProduction readbackとして不適切な別接続結果を返したため判定へ不採用とし、secret値を表示しない既存guarded EnvironmentFile方式で上記Authorityを再確認した。DB／Owner／Asset／Provider mutationは0である。
- Nginx candidate `/var/lib/oripa-v2/runtime/nginx-candidate/oripa-production.conf`はexpected SHA-256 `11ec446db8295c9bf2624aa8f4dcb350102d23d05adb513d7c3b3f063c2b3e32`、active configは不存在、pre-cutover `nginx -t` PASSであった。rollback evidenceを`/var/lib/oripa-v2/runtime/nginx-cutover-20260903T121623Z`へroot-owned mode `0700`で保存し、pre-cutover `nginx.conf`、空の`conf.d` archive、SHA evidenceを保持した。
- Candidateを`/etc/nginx/conf.d/oripa-production.conf`へroot:root mode `0644`でinstallし、active SHA-256がcandidateと一致後に`nginx -t` PASSを取得した。その後Nginx reloadをexact 1回実行し成功。restart 0、master PID維持、active／running、post-reload `nginx -t` PASSである。Error logはgraceful worker replacement成功とknown Asset responseのtemporary buffering warningだけで、routing failureは0である。
- Local immediate acceptanceはdefault `/healthz` 200、`Host: oripa-z.com`の`/`、`/points`、`/mypage/sms-verification`、`/api/v2/gachas?limit=1`、known Asset GETが全200、`Host: admin.oripa-z.com`の`/`が200。Public HTTPS immediate acceptanceは`https://oripa-z.com/`、`/points`、SMS verification route、Public API minimal GET、known Asset GET、`https://admin.oripa-z.com/`が全200で、TLS verificationは全PASSした。Storefront HTML、Admin HTML、API JSON、Asset PNGを各expected content typeで確認した。
- ALB control-planeはHuman Console最終確認をAuthorityとし、credential探索やAWS mutationは行わなかった。public HTTPSがALB dataplane上の複数endpoint経由で全件200、local default `/healthz`も200のため、immediate routing acceptanceをPASSと判定した。
- fincode Webhook／`FINCODE_PAYMENT_ENABLED`、Payment、Mail、SMS、provider business request、DNS、ALB config、Scheduler、business Webhook、Production DBは変更0。実行したProduction mutationはcandidate active install 1とNginx reload 1だけである。Browser Acceptance、Owner browser login、fincode／Provider parity、Commercial Production GOは次Taskへdeferする。
- first real Production business write／Webhookは本Taskで0のため、rollback Authorityはactive candidate削除、`nginx -t`、reload、default `/healthz`確認として維持する。Public GET acceptance成功のためrollbackは実行していない。

### Result

`PUBLIC_FRONTEND_ADMIN_READY`

## OPS-034 Admin Login WebAuthn Runtime Configuration

- Public Browser Acceptanceで既存Owner loginがUI `403`「この操作を実行する権限がありません。」、client Request ID `bd803991-5a2e-4b47-9e7b-1673862b3703`として失敗したため、Issue `none`、Branch `fix/OPS-034-admin-login-webauthn-runtime-config`、通常Worktree、Base `03c5e0dfb6ed141109ef1acd67900a02d0bc431a`、Risk `R4`、Lane `Strict Change`、Activation `immediate`で調査・config-only修正を開始した。Repository変更は本Worklogだけに限定するroot-owned mode `0600` transient Task Policyを使用し、Migration Allocation Lock／Source LockはN/Aである。
- Task開始時のlocal／origin／live protected `main`は`03c5e0dfb6ed141109ef1acd67900a02d0bc431a`へ一致しclean。Production runtime source `b6541e526c4943c03c3533f7cbc9d3ca560a2567`との差分はWorklogだけで、Application／Migration／Composer／dependency差分0である。
- 指定Request IDはAPI／Admin stdoutとNginx logに存在せず、Nginx access logにも`POST /admin/api/v2/auth/login`は0件であった。同一Browser flow直前の`GET /admin/api/v2/auth/session`はHTTP 503。GET-only再現はlocal／publicとも`MFA_CONFIGURATION_UNAVAILABLE`、title `WebAuthn is not configured.`、HTTP 503である。
- Rejection chainは(1) Admin clientが初期`GET /admin/api/v2/auth/session`を実行、(2) APIの`V2AdminAuthController`解決時に`V2WebauthnService` constructorがWebAuthn originをfail-closed検証、(3) runtimeの`V2_WEBAUTHN_RP_ID`／`V2_WEBAUTHN_ORIGIN`欠落により503、(4) CSRF cookie未発行、(5) login submit時にAdmin clientが新しいRequest IDを生成後、fetch前の`CSRF_TOKEN_MISSING`としてlocal 403を返す、である。したがって指定Request IDはserverへ送信されず、middleware／authorization policy／Owner password検証には未到達である。
- 修正前runtime classificationは`APP_ENV=production`、APP URL／Frontend URLはexact public HTTPS、`V2_ADMIN_ORIGIN`はexact `https://admin.oripa-z.com`、`V2_PUBLIC_ORIGIN`もexact、Admin cookieはSecure／host-only／SameSite Strict、Laravel session domainはhost-only、CORSはcredentials無効でsame-origin reverse proxy運用、application trusted-host listは未設定でNginx `server_name`が両hostをallowlistする。欠落はWebAuthn RP ID／originの2項目だけである。
- Owner readbackはexact 1／active 1／verified 1、role `owner`、password hashはcanonical Argon2id classification、Owner permission mappingはcanonical effective set、login eligibility PASS。WebAuthn active credential 0、confirmed active TOTP 0、Admin authentication policyはpresent、MFA required false、invitation required falseである。Owner再作成、password検証／reset、DB row mutationは0。
- Canonical `apps/api/config/v2_identity.php`とWebAuthn RP security boundaryに従い、root:root mode `0600` non-symlink `/etc/oripa-v2/api-production.env`へ公開値`V2_WEBAUTHN_RP_ID=admin.oripa-z.com`と`V2_WEBAUTHN_ORIGIN=https://admin.oripa-z.com`だけを追加した。EnvironmentFile SHAはbefore `153f00c5010a601ce616ad7fae3220fd33ba28a0cbe001e28e9b5caba0b5bf81`、after `f2b14548ab23502f98f54fd08649886513f91716fe65eace72a5411d76a4e70a`で、owner／modeを維持した。旧Serverはapproved read-only接続Authorityがないためcredential探索せず比較NOT_RUNとした。
- Current container envを値非表示・非永続のmemory-only FIFOでcloneし、8612 candidateでAPI `/up` 200、API `/api/health` 200、Admin session 200、WebAuthn RP ID／origin exact一致、Secure／host-only／SameSite Strict CSRF cookie発行をpre-activation PASSした。その後APIだけを約2秒でstop／rename／startし、同一image digest `sha256:12ea2a5b47ce1986894e817150d5439d0ac2edccd14f69a8acfbcc770d8a1380`、OCI revision `b6541e...`、assets bind、private network、`127.0.0.1:8611`、unless-stoppedを維持した。新旧container env差分名はWebAuthn 2項目だけ、new APIはrunning／restart 0。旧containerはstopped `oripa-v2-production-api-pre-ops034`としてHuman retest完了までrollback用に保持する。
- 修正後API `/up` 200、API `/api/health` 200、Admin `/api/health` 200、public Admin root 200、public `GET /admin/api/v2/auth/session` 200／TLS PASS／anonymous／MFA false。実Owner passwordを使わず、発行されたCSRF cookieとexact Originで空JSONを`POST /admin/api/v2/auth/login`へ送るtechnical acceptanceはmiddlewareを通過しcontroller validationのexpected HTTP 422 `INVALID_REQUEST`へ到達した。これによりlogin endpoint／Origin／CSRF／routingはPASSし、Humanが既存Owner passwordで再test可能な状態である。
- Nginx candidate／active SHAは`11ec446db8295c9bf2624aa8f4dcb350102d23d05adb513d7c3b3f063c2b3e32`のまま、Nginx reload／restart 0、ALB／DNS mutation 0。fincode、Payment、Mail、SMS、provider、Scheduler、Webhook、Owner、Production DB business rowは変更0。一時CSRF headers／cookies、probe、clone scriptはexact allowlistで削除した。
...
## OPS-035 fincode Test Production Runtime Activation

- Human明示Activation承認をAuthorityに、Issue `none`、Branch `chore/OPS-035-fincode-test-production-activation-record`、通常Worktree、Base `758eb9cbfe170fc2d76cf1783a7861fa4c76ad42`、Risk `R4`、Lane `Strict Change`、Activation `immediate`で開始した。local／origin／live protected `main`はBaseへ一致しclean、Task Policyは本Worklogだけを許可するroot-owned exact scopeである。
- Production preflightは`APP_ENV=production`、fincode Test endpoint、Test Public／Secret key、Webhook signatureがPRESENT、`FINCODE_ALLOW_TEST_IN_PRODUCTION` unset／disabled、`FINCODE_PAYMENT_ENABLED=false`。Public Storefront／Admin、API `/up`／deep healthは各200、Laravel canonical `POST /webhooks/v2/fincode` routeはPRESENT、Payment／Provider event／fincode／Wallet／Point関連14 tableは全0、Provider request 0であった。Secret値の表示／記録は0である。
- Canonical merged-main native ARM64 Workflow Run `33776581295`／run #5 attempt 1がSource `758eb9cbfe170fc2d76cf1783a7861fa4c76ad42`からBuildを成功した。Artifact `9901915309`、outer／GitHub digest `sha256:16bc80db10aad21f107e7cd21f87c9ab7bec18df41927246bd1bc3a3da0a1259`、Manifest SHA-256 `5175ea413ecfd6296d8574fe8d4f18a644a355c9ddad319d0c075c88b4effa4d`、API image `sha256:c633d9adcdd9c8c2e403f0646a0d8b5aaa6de58ecad1a5624ee7823dfbe5ef51`、OCI revision exact Sourceを検証した。Artifact workflowはcanonical定義どおりAdminもBuildしたが、Production hostへloadしたのはAPIだけで、Admin runtime change／restart／redeployは0である。
- 初回8612 candidateはenv fileだけでは現行containerのRedis host／storage disk等のruntime値が不足し、`/up` 200、deep health 503でFail Closedした。Runtime切替0のまま一時candidateを削除し、現行API envをSecret非表示・非永続のmemory-only FIFOでcloneした再candidateは`/up`／deep health各200、`APP_ENV=production`、Test mode、explicit opt-in true、Payment enabled、Public Configuration bootstrap READYをProvider request 0でPASSした。
- root:root mode 0600 EnvironmentFileはbefore SHA-256 `f2b14548ab23502f98f54fd08649886513f91716fe65eace72a5411d76a4e70a`から、`FINCODE_ALLOW_TEST_IN_PRODUCTION=true`追加と`FINCODE_PAYMENT_ENABLED=true`だけで`958255fa9b59eb84bcd5376e66e1f246fd7623ce955e2775c181ad00075476b1`へ更新した。APIだけを約2秒で新imageへswapし、active `/up`／deep health、Public Storefront／Admin／pointsは各200、restart 0、Nginx access 14 requests中500／502／504各0、error／upstream error 0、Bootstrap READYを確認した。DB Migration、RDS／Redis／Nginx／ALB／DNS／Scheduler／Storefront／Admin mutationは0である。
- Activation後のexternal route確認で、active Nginxに`/webhooks/v2/fincode`のAPI proxyがなくStorefront catch-allへ到達することを検出した。Public non-mutation GETはNext.js 404、API loopback GETはcanonical POST-only route由来の405であり、fincode Dashboard URLを正しく設定しても現状のPublic WebhookはLaravelへ到達しない。事前確認がLaravel route presenceだけでPublic routingまで完了していなかったため、Acceptance前Fail-Closed blockerとした。
- PromptがNginx変更を明示的に禁止するためrouteは変更せず、新APIを停止してpre-OPS-035 API image `sha256:12ea2a5b47ce1986894e817150d5439d0ac2edccd14f69a8acfbcc770d8a1380`／OCI revision `b6541e526c4943c03c3533f7cbc9d3ca560a2567`へrollbackした。EnvironmentFileもbefore SHAへbyte-identical復帰し、`APP_ENV=production`、Test credential PRESENT、opt-in disabled、Payment disabledを復元した。Rollback後API／Storefront／Admin各200、restart 0で、一時candidate／activated containerは削除、verified artifact／API imageは保持した。
- Migration作成／適用0、Storefront change／Build／redeploy 0、Admin Activation 0、Provider request 0、Payment／Coin／Webhook business mutation 0。Schedulerはcreated／not startedを維持した。Human Card Acceptanceは開始禁止。別の明示承認TaskでNginx exact Webhook routeをAPI `127.0.0.1:8611`へ追加・検証後、fincode Test Dashboard Webhookを`https://oripa-z.com/webhooks/v2/fincode`へHumanが設定したことを確認し、同じverified API imageで再Activationする。
...
- Issue `none`、Branch `feat/SHIP-001-admin-shipping-management`、通常Worktree、Original Base `b6541e526c4943c03c3533f7cbc9d3ca560a2567`、Resume protected main `68e8cc7f56a33d977074e2b4bba1e4f27a0aa4ae`、Risk `R2`、Lane `Strict Change`、Activation `none`。Resume時の未コミット変更をstashで保全し、最新mainへ1回だけfast-forward後に競合0で復元した。新規Admin 3 pathを既存Skeleton allowlistへexact登録する直接必要なCI blocker対応に伴い、LaneをStandardからStrictへ上げた。
...
## AGENCY-002 Agency Realm And Portal

- HumanのAgency第三Realm・本人設定・`ad.luxe-pack.biz` V1用途終了／V2 Portal切替承認をAuthorityに、Issue `none`、Branch `feat/AGENCY-002-agency-realm-portal`、通常Worktree、Base `a4f631bb5972a80a98e59e6e8318cdf1959201ba`、Risk `R4`、Lane `Strict Change`、Activation `immediate`で開始した。local／origin／live protected main一致、clean、旧Test API/AdminはAGENCY-001 final head `5d33f1e409c394df14f53f959639bd679a2da369`、Migration 73／pending 0を再確認した。Migration Allocation Lock取得後に未使用`000074`を確保し、closeoutまで保持する。GitHub transport／activation用exact-path transient Task Policyを使用し、Source Lock／専用Worktreeは不要と判断した。
- `agencies`をProvider AuthorityとするAgency Realm／Guard、専用`agency_sessions`、`__Host-oripa_agency_session`／`__Host-oripa_agency_xsrf`を追加。canonical `V2SessionPolicy::currentTime/canonicalTime`、idle 6h／absolute 12h、Secure／HttpOnly session／host-only／Path=/／SameSite Strict、既存Origin／Fetch Metadata／JSON／CSRFを再利用する。Admin/User lifetime／MFA／Storefront policyは変更しない。
- LaravelだけでSession由来の自社profile、担当者名／業務電話、現在PW確認付き即時Email変更／PW変更を実装。Admin memoと他Realm情報を返さず、編集不可fieldを拒否する。Agency row lockとSession再確認により停止・credential変更とのraceを直列化し、Admin停止／PW reset／再発行で全Session失効。本人PW変更は現SessionとCSRFをrotationし他Session失効、Email変更は現SessionとCSRFだけrotationし他Session維持。absolute期限は延長しない。
- Mail catalog／DB固定row／Admin selectorへ`agency_email_changed`を追加。新Emailのみ通知、旧Email送信0、verification pendingなし。既存Password Changed templateを再利用し、PW／Session token／mail bodyをAuditやdelivery ledgerへ保存しない。Testはarray／fakeのみ、実Provider送信0。
- `openapi/agency/openapi.yaml`の7 operationsをContract-firstで定義しbundle、generated client、drift、CI対象へ追加。`apps/agency`はAdmin CSS／presentation-only Page Headerを再利用し、専用teal accentで識別する。Admin Auth／Permission providerはimportしない。現行fixed workspace／migration／artifact allowlistへの必要な追加だけを同PRに含め、既存checkを削除・skipしない。canonical Preview artifactへopt-in API/Admin/Agencyのexact 3-image modeを追加した。
- Task専用DBで74 migrations、schema inventory、latest rollback/reapplyをPASS。Agency／Mail focused 18 tests／325 assertions、停止／Login concurrency 1 test／9 assertions、Admin 215 tests、Agency frontend 7 tests、OpenAPI 12 tests、Policy 205 tests、Preview artifact 37 tests、DB helper 45 testsを実行PASS。初回Mail catalog固定件数と回帰test対象Routeの不一致は修正し再実行した。Agency Build／typecheck／lint、policy／qualityをPASS。Local Browserはmock APIでdesktop/mobile、Login・本人設定・Logout、overflow 0／pageerror 0を確認した。Backend追加Realm実Session回帰とFinal Head Required Checksは別途PR evidenceで固定する。
- Shared Preview切替対象はAgency vhostのみで、API upstream 8611／Portal候補loopback 3202、既存TLS certificate再利用、旧`/admin/`・`/api/`・`/webhooks/`は404とする。必要なAdmin Mail Template selector変更があるためAdmin Activationは1回を予定。Merge／Migration Test apply／Runtime Activation／実API Technical Acceptanceは本実装記録時点では未実施、結果は同PRのcloseoutへ追記する。Human Browser AcceptanceはTechnical PASS後もPENDINGとする。
- Public contract／Storefront Client／Testkit／immutable Artifact変更0。集計はAGENCY-003、Last Click／Attribution hookはAGENCY-004へDeferred。V1 Repository／DB／他Domain／他V1 service変更0、Production mutation／Migration apply／Activation 0。Rollbackは既存vhost復元と互換Runtime保持、security history作成後の000074 schemaは保持しforward correctionとする。旧APIの15-template固定catalogへ無条件に戻さない。
...
## SEC-020 Admin Session Canonical Time Validation Repair

- Humanの明示依頼により旧PlatformでSource修正、PostgreSQL検証、PR／Merge、旧Test API-only Activation／Acceptanceまで実行する。Issue `none`、Risk `R3`、Lane `Strict Change`、Activation `immediate`、通常Worktree、Branch `fix/SEC-020-admin-session-canonical-time`。開始時clean local HEAD、Remote exact `refs/heads/main`、GitHub protected mainはすべて`c421f580210bb3bc37af4641ebf4d67d9cb16e9d`。GitHub transport／Preview import用のroot-owned transient exact 3-path Task Policyを使用する。Source Lock／Migration Allocation LockはN/A。
- Current mainのAuthorizer／SessionPolicy／RealmSessionGuard／SessionManager／Session testsを確認し、同等fix未導入を確認した。`validSession()`のidle／absolute SQL bindingがApp timezoneの`now()`を2回取得していた。`V2SessionPolicy::currentTime()`を1回取得し両比較へ再利用する。作成日時／活動日時は当該queryで未使用、revocation／enrollment／active Admin／identity／permission条件は保持する。Fresh MFAのtimestamp比較と5分windowは変更しない。Productionの32400秒shiftはHuman提供のread-only調査結果で、本TaskからProductionへ接続しない。
- 新規`AdminSessionCanonicalTimeTest`はLaravel／PHP Asia/Tokyo、PostgreSQL connection／server session UTCを明示assertし、実Login→permissions→dashboard monthly→dashboard dailyを同一fresh cookieで検証する。persisted instant差0、6h idle／12h absolute、Fresh MFAの5分直前microsecond／exact境界、idle／absolute直前・exact・1秒後、missing／invalid／revoked／enrollment拒否、locked reauthentication pathを検証する。Admin middlewareの既存401 bodyは`Unauthenticated.`、独自Authorizerは`AUTHENTICATION_REQUIRED`であり、その違いも保持する。
- 隔離PostgreSQLのみでAdmin／Fresh MFA／Auth policy／Realm separation／Dashboard／関連User Session 49 tests／614 assertions PASS。旧`now()`比較だけを一時復元したnegative controlはLogin／permissions成功後のData APIでexpected 401として失敗し、修正を復元した。初回テスト実装時のTestCase method名衝突とAdmin error bodyの誤期待はテスト側を既存契約へ整合し、無効化・skip・認証仕様変更は行わなかった。検証DB helperの既存識別子制限には専用`mig076-v2-sec020-task`／`oripa_v2_mig076sec020`を使用する。新Migration作成0、既存75 migrationsの空の隔離DBへのfixture初期化のみ。旧Test／Staging／Production Migration適用0。
- 旧Test runtime readbackはApp Asia/Tokyo、DB connection／PostgreSQL session Asia/Tokyo。Runtime env／DB timezone設定は変更しない。公開HTTPSの実Login Acceptanceと、同じmerged API imageで実行するApp Asia/Tokyo＋DB UTCのtransaction-rollback Route Acceptanceを区別する。Browser／visual E2Eは未実行。新Server、Production、Admin frontend runtime、Storefront、Worker、Nginx、Redis config、Secrets、OpenAPI、Payment／Coin／Draw／Shipping／SMS／Mail behaviorは変更0。
- 本記録時点でSource検証済み、PR／Required Checks／fresh exact-head review／Merge／canonical API-only Preview Build／Activation／Runtime Acceptanceは後続gateで確定し、結果とSHA／image／cleanup／metricsを同一PR closeoutに記録する。Rollbackは互換schemaを保持して旧API imageへ戻す方法とし、認証を回避しない。Production API `bdcef261e340b577ded384e67a37c04af2c2eafc`からrepair Mergeまでの他runtime deltaは後続Promotionで別途review必須。本TaskでProduction Promotionしない。
- PR #477を作成し、実装修正Head `fd70c636fca8605e31d19d72a8c48dec79ce5280`でlocal final-head regression 49 tests／614 assertions、negative control後のnew focused 5 tests／96 assertionsをPASSした。GitHub `policy-gate`はPASSしたが、`security-gate`は既存dependency advisory baselineの2026-09-08期限切れでFAIL、`quality-gate`もFAILした。fresh auditはComposer 0、workspace pnpm 7件（Critical 2／High 3／Moderate 2）、legacy pnpm 5件（Critical 2／High 2／Moderate 1）で、期限延長だけの解決は不可。両pnpm集合には重複advisoryがあり、環境別exploitabilityは本修正で判定していない。fresh V1 lintは8 errors／1 warningのexact 9 fingerprintsが既存baselineと一致したが、同baselineも2026-09-08期限切れである。
- Resultは`HOLD`。耐久的なblocker追跡のためIssue #478を追加した。依存関係／lockfiles／baseline／workflowの変更0、期限延長0、finding追加許容0、Required Check bypass0。安全な小規模CI blocker修正の範囲ではCritical／High remediationを完了できず、今回の明示Source scopeを超えるためMergeしない。merge-valid fresh self-review evidenceは未発行。Auth差分reviewでSEV-0／1は検出0だが、Repository security gateの未解決findingsを0と扱わない。
- 旧Testはread-only runtime／image／timezone baseline採取までで、API Activation0、公開HTTPS login Acceptance未実行、merged imageのUTC Route Acceptance未実行。新Server／Production mutation0、Admin／Storefront／Worker／Nginx／runtime env／DB timezone mutation0、実データ修復0。PR branchとTask Policyは未Mergeのため保持し、隔離検証DBはcloseoutでcleanupする。Issue／PRに最終record Head、Remote exact refs、未完了checksと実行metricsを追記する。依存関係を別途修復後、新Base／Final Head／全Required Checks／fresh reviewを再検証してからMerge・旧Test Acceptanceへ進む。

## SEC-020 Dependency Security Remediation and Baseline Review

- 2026-09-09、remote exact protected `main@c421f580210bb3bc37af4641ebf4d67d9cb16e9d`とIssue #478をreadback。最新Human承認に従い、Risk R4、Lane `Strict Change`、Activation `none`、Branch `fix/SEC-020-dependency-security`、専用Worktreeで別Security Changeを開始した。PR #477のBranch／Head `86b16c44bc515a070d1df5bd9b716783f5f2b1af`／Admin Session sourceは変更0。Source Lockは衝突なし、Migration Allocation Lockはnot applicable。
- Fresh開始auditはworkspace 7 entries（Critical 2／High 3／Moderate 2）、legacy 5 entries（Critical 2／High 2／Moderate 1）、Composer 0。全件pre-existingであり、PR #477のdependency／audit／baseline変更による導入ではない。
- Next `16.2.11 → 16.3.3`で`GHSA-p293-qw3h-jr36`／`GHSA-2xp9-vwfh-vxw4`、sharp workspace `0.35.0`／legacy `0.35.3 → 0.35.4`で`GHSA-rgj7-g3m4-5g8c`、js-yaml `4.3.1 → 4.3.2`で`GHSA-2883-xcg3-v3hh`を修復。既存exact override authorityを維持した。
- Tiptap `3.30.4 → 3.30.5`で`GHSA-j95f-988m-3j2f`を修復。Admin direct extension／starter-kit／reactとbubble／floating overrideを更新し、lock内core／pmを含む31 packagesの同一versionを確認した。Vitest／mocker `4.1.10 → 4.1.11`でModerate `GHSA-82fw-gwwq-j7x9`、legacy baseline-browser-mapping `2.10.44 → 2.11.21`のcompatible lock-only解決で`GHSA-w5vr-8v7q-w6rv`を修復。Workspaceの同package `2.11.1`、React `19.2.7`は維持。不要なmajor更新0。
- Admin／Agencyのeslint-config-nextは`16.3.3`へ整合。Legacyは新ruleによる追加warning 3件がV1 source変更またはfingerprint追加を要求するため、不要なconfig更新を取り消して既存`16.2.11`と全ruleを維持した。修復後のcanonical fresh lintは既存9件（error 8／warning 1）がpath／位置／rule／message hashを含め完全一致。新findingをbaselineへ追加せず、V1 source変更0。
- 修復後fresh Composer／workspace pnpm／legacy pnpmはCritical／High／Moderate／Lowすべて0。これを確認した後だけ、dependency空配列とlint既存fingerprintを維持して2 baselineのreview metadataを更新。既存SEC-011／SEC-013／GOV-029の7日間review運用を継承し、review `2026-09-09`／expiry `2026-09-16`。日付だけの延長、ignore、advisory suppression、waiver、audit command変更、security gate緩和は0。
- Canonical dependency authorityはpolicy gateにもexact pinされており、初回Policy Unitが旧pin不一致を検出した。直接必要なbounded CI blockerとしてpin data／既存fixture／新compatibility testのexact pathだけを整合し、旧Next／Tiptap／Vitest pin拒否のnegative regressionを追加。Validation logic／Required Checks／security assertionsは維持した。
- Local検証はAdmin 225 tests、Agency 11 tests、実Page／Notice／Mail／image extension focused 15 tests、Admin／Agency lint・typecheck・build、legacy typecheck・build、Sharp native x64 loadと3アプリのNext optimizer PNG→WebP／AVIF出力をPASS。Next自身のAVIF入力拒否は解除せず、別decoder processで出力寸法を検証した。新testは実Tiptapを使用しjsdom未実装のRange layoutだけを補完する。初回高負荷時のtimeoutは記録を保持し、assertion／timeoutを変えずworker 1で再実行PASSした。
- Canonical security gate、Security Unit 10、Quality Unit 4、exact lint baselineをPASS。Final HeadのPolicy Unit、全Required Checks、CodeQL、Dependency Review、canonical ARM64 artifact verification、fresh exact-head self-review、Squash Mergeとpost-merge auditはPR closeoutで確定する。Browser E2Eは未実行。Migration作成／適用、DB／Session mutation、旧Test／新Server／Production／Runtime／Nginx／env／Provider／Secret変更はすべて0。
- SEC-020 merge後もPR #477を変更・mergeしない。NEW_MAINだけを引き渡し、別Taskで1回の追従、focused Admin Session／PostgreSQL Integration、全Required Checksとfresh reviewを再実施する。Rollbackは別reviewによるsource revertであり、脆弱dependencyの無断Runtime再適用は行わない。
- PR #479初回Head `dd7178503c2119c39c33333cb87a9b1128ff3229`はPolicy／Security／CodeQL／Dependency Review／native ARM64 artifactをPASSし、QualityもAdmin／Agency tests・buildとlint baselineをPASSしたが、最後のgenerated-output差分検証がNext `16.3.3`の`root-params.d.ts` import追加を検出した。Admin／Agencyのtracked `next-env.d.ts`へ実build生成内容を反映する直接必要な互換性修正を追加し、`git diff --exit-code`を維持したまま新Final Headで全checkを再実行する。Application logic／型検証の緩和0。
- Head `4205f288bd994807547499e97bf0272bb4b6a4ae`はQualityを含む依存修復検証をPASSしたが、isolated API smoke buildでbase由来の`linux-libc-dev=6.1.180-1`取得不可を検出しHOLD。Issue #478／PR #479へ記録後、最新Humanが同PRへのDockerfile／exact policy／test限定のOS pin修復を明示承認した。再開時もprotected main `c421f580210bb3bc37af4641ebf4d67d9cb16e9d`、PR #479 Head、PR #477 Headをremote exactで確認した。
- Dockerfileの実CLI Bookworm base digest内の未変更`debian.sources`（bookworm／bookworm-updates／bookworm-security、main、Debian署名keyring）をauthorityとして、read-only filesystem／tmpfsの使い捨て検証containerでAPT署名検証とarchitecture別downloadを実施。amd64／arm64双方の`linux-libc-dev=6.1.187-1`取得、archive version／architectureをPASS。deb SHA-256はamd64 `ebcd139c31438e19f161bcf277a38708402ddd0e3acbe208cbbb27bff1c52bb9`、arm64 `d076d84f30c95eef5f29931ea9b1c3e58dbb49d577823102b857d6bf3e9035a2`。Host／既存RuntimeのAPT／package変更0。初回検証用containerのcapability不足は使い捨て検証境界だけで補正し、APT trust／policyは変更していない。
- `infra/docker/backend/Dockerfile`のpreview targetだけを`6.1.187-1`へexact更新し、canonical policy expected pin／positive regressionを同値にした。旧`6.1.180-1`不在assertionを追加し、既存`6.1.177-1`拒否を保持。追加OS package／base image／business source／Session／Migration変更0。Baselineのfinding配列／fingerprint／expiry変更0。API image buildはHost build禁止を維持してcanonical GitHub CIの隔離API smoke経路で確認し、新Final Headの全Required Checks、ARM64、CodeQL、dependency review、fresh self-review、merge後security acceptanceをcloseoutに記録する。

## SALES-20260916 Dashboard Daily Sales Payment Method

- 対象は旧Server `/var/www/oripa`、Platform AdminのDashboard `/` 内「日別売上」タブの決済一覧のみ。Base／local main／origin main／live protected main `b9c95768b87ce677a4c30c985c61716c4570c549`、開始時clean、Issueなし、通常Worktree、Branch `fix/SALES-20260916-daily-method`、Risk R2、Lane Strict Change、Activation immediate。API追加・旧Test API/Admin反映は不足field調査後にHumanが明示承認した。GitHub transport／Preview import用exact 9-path transient policyを使用し、Source Lock／専用Worktree／Migration Allocation Lockは不要。
- 現行表示authorityは`item.provider`／`payments.provider_code`で、日別売上APIにMethodがなかった。対象responseだけに既存保存field `payments.payment_method`を`payment_method`として追加し、OpenAPIへoptional nullableの4 enumを定義、bundleとAdmin生成型を再生成した。Provider fieldと値は保持し、query条件／集計／pagination／権限／保存処理は変更しない。
- 対象セルは既存Admin `paymentMethodLabel`を再利用し、credit_card＝クレジットカード、paypay＝PayPay、konbini＝コンビニ決済、virtual_account＝銀行振込。Unknown／null／field未返却／空文字／Filter専用allは`-`。メールmappingは同じ名称だがPHPのprivate処理に内包されるため移動せず、既存TypeScript helperを使用。独立決済一覧とhelper本体の変更0。
- Admin focused 23 tests（対象画面16／既存決済一覧7）と既存API client 31 tests、Admin typecheck、変更TSX／generator ESLint、OpenAPI 15 tests／bundle drift／generated drift、PHP syntax／diff check PASS。新UI 9 casesは修正前のfincode表示を検出してFAILし、修正後PASS。API／既存メールmappingはTask専用隔離DBで13 tests／153 assertions PASS。初回追加日時assertionのUTC固定表現は既存DBのJST表現と異なるため同一instantのUTC比較へ修正し、runtime日時処理は変更0。
- Migration作成0、旧Test／Production適用0。既存Migrationは使い捨て検証DBの構築だけに使用。決済／Webhook／3DS／Coin Grant／保存データ／集計／Filter／DB schema／Auth／Public contract／依存関係／CI／Secretsの変更0。Production／新ServerはNOT RUN。Local全Suite・実決済・最終Human Browserは未実行、Human確認はHUMAN PENDING。
- 全Required Checks、exact-head self-review、squash merge、canonical Preview Build、旧Test API/Admin反映とtechnical確認、SHA／image／cleanup／metricsは同一PRのcloseoutに記録する。旧Test preflight HTTP smokeはPASS。Rollbackは互換性を保った従来API/Admin imageへ戻し、DB操作を必要としない。

## ADMIN-20260916 Point Purchase and Banner List Defaults

- Human指定の旧Server `/var/www/oripa` / Platform Adminだけを対象とする。Base HEAD／origin/main／live remote mainは`78c0bf8ad10e9e13f4046b8104b762ab41bab5f4`で一致、開始時の未commit／未追跡変更なし。Risk `R1`。Sourceは軽微な初期Filter変更だが、既存機械判定がpoint系pathをStrict分類するためLaneを`Lite Maintenance`から`Strict Change`へ上げる。Activationは旧Test Admin反映の最新Human承認により`immediate`。Branch `fix/ADMIN-20260916-list-defaults`、通常Worktree、Issueなし。WorklogはRoot AGENTS.mdのTask記録要件に従い含める。GitHub App transport／Preview import用にroot-owned mode 0600のtransient exact 7-path Task Policyを使用する。専用Worktree／Source Lock／Migration Allocation Lockは不要。
- ポイント一覧とバナー一覧のPage query fallback／Workspace defaultの4箇所だけを既存値`published`から`all`へ変更した。`all`は既存client経由でstatus未指定となり、既存APIが有効・無効を含む一覧を返す。明示`published`／`draft`／`all`、複数queryの先頭値優先、Filter操作、カテゴリ、並び順、pagination、Create／Update、他一覧、共通Filter、API、DB、権限、状態値は維持する。変更はAdmin source 4 files、既存focused test 2 files、本Worklogのみ。
- Focused Vitestは2画面と既存Admin API clientの3 files／54 tests PASS。初期all、有効・無効の混在表示、published／draft／allへの操作、Page query優先を確認した。変更6 filesのESLint、Admin typecheck、`git diff --check`はPASS。既存インストール済みVitest実行versionは4.1.10（manifestの4.1.11と差異あり）、dependency／lockfileは本Taskで変更しない。全Suite／全service Build／Browser E2EはNOT RUN。
- 旧Testのactive Nginx設定と稼働containerを確認し、Adminは`https://admin.luxe-pack.biz/`、既存imageの`node server.js`起動・source mountなしである。旧Host loopbackへTLS hostnameを維持して接続し、`/purchase-plans`と`/banners`はHTTP 200。ただし稼働画面は変更前であり、修正後のRuntime／Browser PASSとは扱わない。
- 最新Humanは2件を1 PRにまとめたcommit／push／Required Checks／mergeと、exact merged sourceの旧Test Adminだけへの反映を承認した。既存Preview workflowのnormal modeはAPI／Adminをbuildするが、反映対象はAdminのみとする。Host build、新配布経路、API activation、Production／新Server操作は対象外。Rollbackは反映前Admin imageを保持してそのimageへ戻す。Migration作成／適用0、DB操作0。
- PR作成前の確認で既存Strict CIがGitHub runner上の`production-arm64-artifact`を自動生成することを検出した。HumanのProduction Build／Artifact禁止との衝突について、CI内候補検証だけの実行可否を確認中。承認まではPR作成／CI起動を保留し、commit／PR本文準備まで進める。Required Checks、workflow、依存関係、保護は変更しない。結果とexact SHA／Checks／self-review／旧Test反映・technical acceptance／cleanupは同一PRの記録で確定する。Human最終Browser確認は`HUMAN PENDING`。
## ADMINLIMIT-20260918 Admin Mutation Limits Phase 1

- 最新Human指定の旧Server／Platformのみ。開始時clean、previous expected main／fetch後origin/main／採用Baseは`5c426715efa96d582511df1b22aa3891357fe26a`で一致した。Issue `none`、Risk `R3`、Lane `Strict Change`、Activation `none`、通常Worktree。既存GitHub App wrapperのTask ID要件に合わせBranchは`fix/ADMINLIMIT-20260918-phase1`。GitHub transport用のtransient exact-path Task Policyだけを使用し、merge／auto-merge／deploy権限を含めない。
- Catalog `30/600`、Critical Admin Mutation `10/600`、Financial Export `5/3600`の3系統だけを撤廃した。Catalog専用Limiterと不要configを削除し、共通AuthorizerのCritical flag／Financial limiter、直接利用ServiceのCritical呼出しを除去。`V2RateLimiter`本体とLogin／MFA／OTP／SMS／recoveryの認証防御は保持する。Catalogの`catalog_master_mutation`残存はIdempotency scopeでありLimiterではない。Historical docsは変更しない。
- Point AdjustmentとAdmin Authentication Policy更新のController／Service／UI／OpenAPIから明示`current_password`と照合処理だけを除去した。Fresh Authentication 5分、同一Session、`mfa_verified_at`、FreshMfaDialog、`/auth/reauthenticate`とMFA OFF時Password fallbackは変更0。期限切れ時の再認証はPhase 1後も必要。Owner／Permission、CSRF／Origin、Idempotency、OCC、Admin／User／Wallet／Lot lock、Ledger、Audit、Outbox、DB Transaction、入力Validation、残高・公開状態business ruleを維持する。
- Admin OpenAPIを`2.0.0-alpha.32`へ更新し、廃止した3系統のrate annotationと2 request schemaを整合、bundle／Admin clientを正規generatorで再生成した。旧clientが明示password fieldを送るrequestはadditionalProperties制約により422となるため、後続ActivationはAdmin／APIを同じ契約へ合わせる。Public／Agency contract、Storefront Artifact、dependency manifest／lockfileの変更・再発行0。
- `DATETIME_OVERLAP=PRESENT`：`V2AdminFreshMfaAuthorizer`、`V2CatalogMasterMutationService`、`V2PointPurchasePlanService`、`V2AdminPointAdjustmentService`、`V2AdminAuthenticationPolicyService`の5 filesはLimiter／Password関連行だけを変更した。日時保存／cast／binding／timezone／Session／Model／日時helperの差分0。Remoteの日時branchは未公開で実変更一覧比較は未確認、新Serverアクセス0。並行作業との統合Base更新は日時改修確定後へ保留する。
- MFA実運用Policyは旧Runtimeの単一Policy行をread-only transaction内SELECTして`mfa_required=false`を確認、rollbackした。Secret／PII／TOTPを出力せず、DB Policy／Session／Runtime設定の変更0。MFA Runtime Policyは`UNCHANGED / DISABLED`。
- API focusedは54 tests／640 assertions PASS。認証・Fresh・Session・CSRF／Origin・並行実行の回帰は90 tests／858 assertions、89 PASS／QA負荷test 1 SKIP（既存専用flagが必要）。追加Admin Login失敗防御と実ポイント並行実行は2 tests／21 assertions PASS。既存Migrationの適用は専用internal network・tmpfsの使い捨て検証DBだけ。Migration作成0、旧Preview／新旧ProductionへのMigration・Activation・Deployは0。
- Admin focused Vitest 4 files／42 tests、typecheck、lint、Next build、対象Chromium E2E 5件をPASS。E2Eは別途build済み成果物をloopbackで一時起動し、全APIをsynthetic fixtureへ置換、Previewへ反映していない。対象E2Eの古い残高単位／曖昧status locatorと不足read fixtureを現行UIへ整合し、残高3値／送信body／Idempotency-Key／単一mutation／error無発生のassertionを維持・追加した。既存node_modulesのpin差をfrozen installで整合し、Next `16.3.3`／Vitest `4.1.11`で最終検証、tracked生成環境file差分0。
- OpenAPI 15 tests、Policy 208 tests、bundle／client drift、repository policy、diff checkはPASS。既存Policy gateに埋め込まれた廃止Limiter要件だけを新仕様へ整合し、Login／MFA／Fresh保持と廃止Limiter不在の回帰を追加した。Required Checks／CI workflow／security baselineの変更・緩和0。
- Scope外の既存問題を区別して記録する。追加diagnostic Unitの1件は未変更`V2PointService`のpaid lot expiryと古いnull期待値が不一致でFAILし、日時改修に関わるため修正しない。Financial stream成功経路は未変更`V2ExportService`の`Illuminate\\Http\\StreamedResponse`戻り型と実Symfony response不一致を検出し未検証扱いとする（全3 APIのFresh／Permission拒否、6件job作成・downloadはPASS）。広いauth-shell E2Eは未変更navigation／catalog期待値7件がFAIL、対象操作5件の最終focusedはPASS。全Admin unitの誤ったfilter実行は中断し全Suite PASSとは扱わない。Required CIの全Suite結果は同一PRへ記録する。
- Final差分self-reviewは指定3系統以外のLimiter、Fresh／Password Login、Session、日時処理、公開契約境界を再確認し、今回差分のSEV-0／SEV-1は0。exact Final HEAD／全Required CI／CodeQL／dependency-review／scope結果はPR closeoutへ記録する。Rollbackは別reviewでAdmin／API契約を一緒に戻すsource revertのみで、現Runtimeは未変更。
- **DO NOT MERGE YET**。日時保存改修のprotected main mergeと新Production Technical確認が両方完了するまで、本PRのmerge／auto-merge／merge queueは実行しない。新Server／新旧Production／旧Previewへの反映0。Phase 2残件はFresh Authentication要求廃止、Fresh Password fallback廃止、FreshMfaDialog整理、`/auth/reauthenticate`のPhase 2対応。今回完了扱いにしない。
- 継続時も旧Platformの同Branch／HEAD `e13bdeca3f749c170f545850f6973fccab315745`、clean、fetch後mainは採用Baseと一致した。PR #488の初回metadata不備を本文だけで訂正し、既存wrapperがDraftのCI再実行を拒否するためReady for Reviewとして再実行した。ReadyはMerge承認ではない。Policy／Security／CodeQL／dependency-review／Strict ARM64候補検証はPASSしたが、Qualityは2 request property削除のBreaking Change、IntegrationはAdmin契約alpha.32と既存source inventory alpha.31の不一致でFAILした。Gate／immutable inventory／Storefront Artifactを変更せずHumanへ確認した。
- 最新Humanは旧client互換方式を明示承認した。この承認が本項の先行する完全field削除／alpha.32／旧client 422の記述を置き換える。2 requestの`current_password`はContract上optional／deprecated／writeOnlyとして残し、Contract versionは現行`2.0.0-alpha.31`を維持する。必須条件／Password照合／Admin UI入力は撤廃したまま、Controller先頭で受信入力から値を除去し、Serviceへ渡さない。処理／認証／Idempotency fingerprint／ログ／Audit／Outbox／DB保存へ使用せず、Fresh Authenticationは維持する。
- 追加API回帰では異なる旧Password値2種と省略requestが同一Idempotency-Keyで1回だけ実行されること、誤Passwordでも照合されないこと、response／処理後request／全実行SQLとbindings／application log eventに値が含まれないことを確認した。正しい旧Password値を送ってもFresh期限切れは拒否する。Final互換方式のAPI focusedは56 tests／686 assertions、Admin focused42 tests、PR Base比較つきOpenAPI互換性／bundle drift、OpenAPI15 tests、Policy208 tests、Release48 testsとsource inventory検証をPASS。既存Breaking Change Gate／Release Gate／version inventoryの修正・緩和0。
- Remoteの日時改修Branch `fix/DATETIME-20260918-storage`が公開されたため、SHA `cd1f774e9ce4aebaf0173b12b5998b6a340b3396`の変更file名だけをread-only比較した。実重複16 pathsはCatalog／User State／Agency／User Tag／Point Purchase Plan／QA3 Service、API test5、Policy gate／Policy test／本Worklog。従来候補のFresh Authorizer／Point Adjustment／Authentication Policy Serviceも引き続き競合候補として扱う。日時branchのcheckout／変更／push、新Serverアクセス、日時関連production行の変更はいずれも0。最終HeadのCIとReview、使い捨て検証環境cleanupは同一PRへ追記し、統合・Mergeは外部前提完了まで保留する。
- 互換修正の追加検証は認証／Fresh／Session／CSRF／Origin／並行実行91 tests／879 assertions、Admin typecheck／lint／build、対象Chromium E2E5件をすべてPASS。検証中にRemote mainが日時改修SHA `3b40d4aae01b9d3a9ef3c73c202e0f56c08fd631`へ進んだためfetchと変更名比較だけを実施した。Taskの採用Baseは元SHAを維持し、main／日時branchの取り込み、rebase、mergeは行わない。新Production Technical確認は未確認。本Taskの追加commit／pushは互換修正だけとし、PRの最新CI／競合状態を別途記録する。

### ADMINLIMIT-20260918 Phase 1 integration after datetime approval

- 最新HumanはPR #487のprotected main merge、日時SHA `3b40d4aae01b9d3a9ef3c73c202e0f56c08fd631`、新Production Technical／Human日時確認PASSを通知し、既存PR #488の同期・競合解消・全gate後squash mergeを承認した。これが先行するMerge HOLDを解除する。Production Deploy／旧Preview Activation／MFA Policy変更／Phase 2は承認されていない。新Serverへのアクセスは行わない。
- 再開時は旧Platform、同一通常Worktree・Task branch、clean、local／Remote task HEAD `5c8623db9747ba8f555884ef63346767044061b7`、fetch後mainは上記日時SHAで一致した。Issue `none`、Risk `R3`、Lane `Strict Change`、Activation `none`、既存exact 44-path policyを維持する。wrapper権限は承認された安全なbase同期・exact-head review・squash merge・merged branch cleanupだけを追加し、auto-merge／Deploy／保護迂回は追加しない。
- 初期merge-treeの実競合は3 files：AuthenticationPolicy／ReportingExport testsの追加importと本Worklog。mainの日時fixture importと記録を保持し、Phase 1のMockery import／制限撤廃test／記録も保持する事前解消commitを作り、既存のclean-tree検証付きbase-sync wrapperでno-force同期する。Production Sourceは自動merge可能で、統合後に最新main基準の日時差分をreviewする。日時helper／binding／Model規約／Session比較の巻戻しは許容しない。
- 今回はDB Migration禁止に従い、手動DB構築・migration・共有Runtime操作を行わない。DBを要するAPI回帰は既存Required CIの使い捨て隔離suiteで確認し、手元ではAdmin focused／typecheck／lint／build／E2EとContract／Policy／Release検証を再実行する。各結果、最終SHA、fresh review、merge／cleanupは同一PRのcloseoutへ記録する。

# DOMAIN-20260917 Production Domain Environment Authority

- Local初回341 testsとrepository policyはPASS。PR初回はUI Verification metadataの表記を`NOT_APPLICABLE`へ訂正して同Head再実行した。再実行のQualityは新test fixtureがprocess ENV全体をclearし、GitHub toolcache上のNodeをPATHから消していたためFAIL。fixtureを必要な4 ENVだけの差替えへ修正し、既存frontend scanner実行を保持したまま新Headで全gateを再実行する。Runtime／readiness仕様／test assertionsの緩和0。
- Human指定の旧Platform `/var/www/oripa`のみ。開始時working tree clean、live protected main／origin/main／local mainは`b296ae3dcc3ca94db5966ab3a55587c36d4e2462`で一致し、このexact SHAをBaseへ固定。Change `DOMAIN-20260917`、Issue `none`、Risk `R3`、Lane `Strict Change`、Activation `none`、Branch `ci/DOMAIN-20260917-env-authority`、通常Worktree。GitHub transport用root-owned mode 0600 transient exact-path Task Policyを使用し、Source Lock／Migration Allocation Lock／専用Worktreeは不要。
- Production configをv2へ更新し、固定URLと重複public URL fieldsを除去。4 logical domain fieldsから既存の4 ENV keyへのmappingのみを保存し、readinessはprocess ENVだけから解決する。欠落／空／不正URL／HTTP／origin内path・query・fragment／userinfo／不正host・portをfail-closedで拒否し、Agency loginとAgency originのhost／effective port一致を要求する。既存Old Test mail拒否、offline read-only network-none image検証、Session／Mail検証は保持。RuntimeのTest/dev default自体は変更0。
- Canonical Production artifact dispatchへdefaultなしの必須non-secret 4 inputsを追加し、対応ENVへ直接map。Production Environment Variablesは空、現行workflowに既存domain Variables参照なし。Repository Variables一覧はApp権限で403のため存在なしとは断定しない。PR ARM64検証は専用`.invalid` fixture ENVを明示供給し、Production dispatchと区別する。成功後の`production-readiness.json`へexact source SHA、3 image refs、4 effective domainsを記録し、既存manifest／archiveと同じimmutable GitHub artifact digestに含める。
- Scopeはreadiness config／script／2 workflows／関連ops tests／既存runbook／本Worklogの7 paths。Agency business／Admin behavior／API auth／Session／Mail semantics／attribution／DB／Migration／Runtime config／Storefront／dependencies／security baseline変更0。新Server／Production container／ENV／DB／Nginx／DNS／TLS／Secrets／Provider mutation0。Migration作成0、適用0。
- Focused ops testsでvalid ENV、各key欠落／空、不正HTTPS、origin component、login origin mismatch、新Human指定domainと別customer domain、旧Test拒否、domain mapping改変拒否、image隔離、成功時のみprovenance出力を検証。Local結果、全Required Checks／CodeQL／dependency-review、exact-head fresh self-review、mergeとfinal ARM64 build digest／effective domains／feature保持／cleanup／metricsは同一PR closeoutに記録する。Browser E2E／実Production readiness／DB integrationは本変更範囲外で未実行。Rollbackはreview済みsource revertと再artifact生成で、Runtime操作を伴わない。
## DATETIME-20260918 V2日時保存横断修正 — Phase 1継続

- Humanの最新GOは残DomainのSource・隔離検証・1 PR・Required Checks・protected merge・canonical ARM64 Artifact・新Server API／必要writerの技術反映まで。Issue `none`、Risk `R4`、Lane `Strict Change`、Activation `immediate`。既存9-file dirtyを保持して同じ通常checkout／履歴を継続し、wrapper必須Change IDに合わせ同一branchの名称だけを `fix/datetime-storage-phase1` → `fix/DATETIME-20260918-storage` に正規化した。追加branch／Worktree／Issueは作成しない。GitHub transport用の一時exact-path policyだけを使用する。
- Baseは `5c426715efa96d582511df1b22aa3891357fe26a`。継続開始02:43 UTCの実Remote mainも同じSHA、未知の変更なし。Phase 0再調査・古いlocal mainからの実装・無関係なmain差分取り込みは行わない。Source mountなし、APIはBase、Mail／SMS workerは `b6541e526c4943c03c3533f7cbc9d3ca560a2567` のままSource作業を実施した。
- 保存規約はPhase 1の `Support/V2DatabaseTimestamp::format(?DateTimeInterface): ?string` を変更せず再利用。入力を変更せずUTC offset付き秒精度文字列／nullを返し、文字列parse・validation・業務日付を引き受けない。実writerを持つ対象 `Models/V2` にだけ `Y-m-d H:i:sP` を追加。自動timestampsとcast済み明示日時を保護し、QueryBuilder／raw／bulkには別途明示bindingを行う。Session／RememberDevice Model、canonical clock、Manager／Guardは変更しない。
...
| --- | --- | --- |
| DomainTimestampPersistence | 7 / 140 | PASS / PASS |
| DatabaseTimestampPersistence・BannerTimestampPersistence・Banner/Page/Announcement/Contact・ContentContact・MailTemplate・AuditOutbox | 57 / 488 | PASS / PASS |
| Point/Payment foundation・LimitedBonus・fincode・Plan/Campaign管理 | 100 / 836 | PASS / PASS |
| Authentication/AccountSecurity・Admin policy/MFA/UserState/UserTag・PasswordResetSMS・Agency管理/Portal/Attribution・Google/LINE login | 151 / 1267 | PASS / PASS |
| Draw・Shipping・QA Draw/Plan/Guarantee | 69 / 771 | PASS / PASS |
| Catalog管理・MasterEdit・RankPrize/Effect・Draft/PublishPreflight・Probability・Presentation | 87 / 1098 | PASS / PASS |
| ReportingExport・LineMessaging・SMS4S・Referral | 33 / 329 | PASS / PASS |
| fincode・Identity recovery/external identity・Point adjustment・Shipping・QA plan・LINE concurrency | 15 / 153 | PASS / PASS |
| Footer・current User draw/point/LINE read・PointProduct・PointAdjustment API・Dashboard・Agency集計・Catalog read・DirectEmail/FailureState | 58 / 853 | PASS / PASS |
| 明示opt-in Draw日次制限／残数partial concurrency（methodごとに空DB） | 2 / 27 | PASS / PASS |

PHP構文137 files／diff whitespace／Source scope review PASS。helper／新規Model規約／public payload・署名・business date・既存制約をreviewし、SAFE Session／Schema／config／API contract／frontend／lockfile変更0。初回失敗は前述のfixture・実不具合を切り分けて修正済み。Draw opt-in 2 methodsを同DBへ連結した初回duplicate fixtureもmethod別のguard付き初期化で解消し、test期待は変更しない。全Suite／全Buildはlocalで実行せず、Required CIは省略しない。Production technical確認・Human BrowserはこのSource記録時点では未実行。

以下のfileは `apps/api/app/` 相対。VはoffsetなしCarbon binding／Model保存が脆弱だった実経路、Sは維持したSAFE経路、Iは入力依存として実Sourceで確認した経路。旧「30／10／3群」の名称対照は未収録であり、この表の行数・Model数を旧群数に合わせたり再集計したりしない。

| Domain / file・method・column | writer / 元分類 | 変更・検証 | 反映service / 残事項 |
| --- | --- | --- | --- |
| Content: `Domain/ContentContact/Services/V2ContentContactAdminService.php` createManagedBanner/updateManagedBanner/deleteManagedBanner/createBannerCategory/uploadBannerAsset; banner/category/asset created_at・updated_at | QueryBuilder / V | helper明示binding、Banner persistence／AdminBanner | API / 過去日時は未補正 |
| Content: 同file createManagedPage/updateManagedPage/createPageCategory/createContent/createVersionRow/publish/changeParentState/applyPageVisibility; parent/version/asset-link timestamps・publish_start_at/end_at/published_at | QueryBuilder＋Content各Model / V | helper＋対象Model規約、Page／Announcement／Content／Footer回帰 | API / 開始inclusive・終了exclusive、start null拒否／end null許可、naive app TZ維持 |
| Content: `Domain/ContentContact/Services/V2ContentReadService.php` publishedQuery; cv.publish_start_at/end_at比較 | QueryBuilder / V | 比較clockもhelper、開始直前／一致・終了直前／一致の実DB検証 | API / 表示一律補正なし |
| Contact: `V2ContentContactAdminService.php` updateContactStatus/addInternalNote/requestReply/contactHistory、`V2ContactService.php` submit; inquiries received_at/retention_until/closed_at、履歴occurred_at、note/reply created_at | Model＋QueryBuilder / V | helper＋ContactInquiry規約、AdminContact／ContentContact | API / retention業務規則不変 |
| 設定: `Models/V2/MailTemplate.php`, `ReferralPointSetting.php`, `LineMessagingSetting.php`; 各Domain serviceのforceFill/save updated_at | Model / V | 対象Model規約、MailTemplate／ReferralPointSettings／LineMessaging | API / 設定値・Provider有効化なし |
| Catalog: `Domain/Catalog/Services/V2CatalogMasterMutationService.php` create/update/archive/createRankMaster/updateRankMaster/reorderRankMasters、Gacha・Rank・Prize・Probability・Asset mutation; created_at/updated_at/archived_at/published_at | QueryBuilder＋Catalog対象Model / V | 明示binding、Catalog管理87件・関連read回帰 | API / lifecycleのDB CURRENT_TIMESTAMPはSとして維持 |
| Catalog: 同file updateCurrentGachaPresentation; current_publish_start_at比較・保存 | QueryBuilder / V | Carbon→offsetなしstring→parseの再消失も除去。同一instantの公開後編集とDB epoch、不変経済項目拒否を保護 | API / 制約・immutable規則は緩和しない |
| Catalog: `V2CatalogReadService.php` 公開期間比較、`V2CatalogFixtureImporter.php` import/manifest各writer; import completed_at、各timestamps、publish_start_at/end_at | QueryBuilder / V＋I | 比較helper。manifest文字列は呼出元でapp TZ parseしoffset付きで渡す。小数秒はhelperへ丸めず既存PostgreSQL秒列の丸めを維持。import replay／JST・UTC・naive・fraction実DB検証 | API / 旧I 3群との名称対応は不明、実経路の判定とは区別 |
| Catalog: `Domain/Payment/V2/Services/V2PointPurchasePlanService.php`, `V2LimitedBonusCampaignService.php`; plan/campaign timestamps | QueryBuilder / V、期間ISO入力はS | timestamps helper、Plan／LimitedBonus回帰 | API / validated ISO境界不変 |
| Identity: User/Admin/Invitation/Verification/PasswordResetToken/EmailChange/ExternalIdentity/MFA/SMS各対象Model; created_at/updated_at/verified_at/expires_at/used_at/processing_at/linked_at | Model / V | 規約追加、Auth／AccountSecurity／MFA／Google／LINE／SMS回帰 | API / Session Model除外 |
| Identity: `Domain/Identity/Services/V2AdminUserStateService.php` update、PasswordChange change、PasswordRecovery／EmailChange／ExternalIdentity直接失効; sessions/remember_devices revoked_at | QueryBuilder・UserSession bulk / V | 既存V2SessionPolicy canonicalTimeを再利用。Password変更の同一基準epochで他Session・Remember失効を実DB検証 | API / SAFE Manager経由の失効と混同しない |
| Identity: UserAuthentication／RecoveryCode／Totp／SmsVerification／ExternalIdentityの非Session bulk・履歴、AdminAuthentication invitation期限／Session比較 | QueryBuilder / V | 非Session binding helper、Session比較のみ既存canonical clock、認証・失効・retry回帰 | API / Auth基盤再設計なし |
| Agency/User tag: `Domain/Identity/Services/V2AgencyService.php` create/mutate、`V2UserTagService.php` create/update/changeAssignment; created_at/updated_at/assigned_at | QueryBuilder / V | helper＋Agency Model、Agency／UserTag回帰 | API / credential値・期限のepoch契約は不変 |
| Agency attribution/Session: `V2AgencyAttributionService.php` attributed_at、`V2AgencyPortalService.php` Session処理、V2SessionPolicy/Manager/Guard | 既存canonical clock / S | Source変更なし、SAFE Session 21件とAgency回帰 | API / Session test内部UTC/JST指定を尊重 |
| Payment: `Domain/Payment/V2/Services/V2PaymentService.php` createPayment/recordVerifiedProviderEvent/provider operation/transitionPayment/newPointOperation/grantLot/返金・chargeback; payment/provider/history/wallet/lot/ledger日時 | QueryBuilder / V、Provider ISO時刻はS | helper binding、Payment foundation／fincode／Domain timestamp。Provider確定時刻・Exactly Once・状態順序不変 | API / 実決済なし |
| fincode: `V2FincodeCardService.php` registration/expireDue/expiry比較、`V2FincodePaymentService.php`、Webhook／Reconciliation; created/updated/started/completed/failed/expires/last_used日時 | QueryBuilder / V | binding＋期限比較、fincode focusedとconcurrency | API / 新ENV・Provider仕様変更なし |
| Point: `Domain/Point/Services/V2PointService.php` initializeWallet/lockWallet/grantDrawPointBackBatch/consumeLotsForDraw; wallet、operation/lot/ledger bulk timestampsとraw CASE updated_at | insertOrIgnore・bulk insert・raw update＋Model / V | helper＋対象Model、Point foundation・Draw・手動調整・concurrency | API / balance・FIFO・locking・paid/free規則不変 |
| Point/Audit連携: `V2PointIdempotencyService.php` claim/complete、`V2PointLedgerService.php` cutoff、`V2PointSnapshotService.php` cutoff、Reconciliation各Model; created/completed/expires/occurred/generated/started日時 | raw insert・bulk update・Model・比較 / V | helper＋Model、replay／cutoff直前・一致／JST日付／snapshot checksum | API / 既存snapshot再生成・本番補正なし |
| Draw: `Domain/Draw/Services/V2DrawService.php` create/inventory raw CASE/result・user_prize bulk; occurred_at/acquired_at/storage_expires_at/created_at/updated_at/completed_at | raw・bulk＋Model / V | helper＋対象Model、Draw実DB epoch・snapshot・重複／rollback回帰 | API / 経済・確率・日次境界は不変 |
| Shipping: `Domain/PrizeShipping/Services/V2PrizeShippingService.php` createShippingRequest/exchange/changePrizeStatus/shippingHistory; item/history created_at/occurred_at/shipped_at/terminal_at、Address/Request Model timestamps | QueryBuilder＋Model / V | 明示binding＋Model、Shipping／交換／concurrency | API / 既存配送・住所を補正しない |
| QA: `Domain/QaDraw/Services/V2QaDrawResolver.php`, Admin/Plan/Execution管理; ends_at比較、updated_at bulk、execution/assignment日時 | QueryBuilder＋Model / V | helper＋対象Model、QA／Plan／Guarantee／concurrency | API / 実行権限・消費順序不変 |
| 非同期: `Domain/Outbox/Services/V2OutboxService.php` enqueue/claim/markDelivered/retry; created/updated/available/locked/lease_expires/delivered日時 | insertOrIgnore・QueryBuilder・Model / V | helper＋OutboxModel。available/lease直前・一致、所有権・再取得・dedupe・retryを実DB検証 | API＋Mail＋SMS / 古いavailable/lease誤保存は未補正 |
| Mail/SMS: `Domain/Mail/Services/V2TemplateMailDeliveryService.php` 各delivery write、`Models/V2/MailDelivery.php`, `SmsVerificationChallenge.php`; sent_at/delivery_attempted/accepted/failed_at/updated_at | QueryBuilder＋Model / V | helper＋Model、MailTemplate／AccountSecurity／DirectEmail／SMS fake回帰 | API＋Mail＋SMS / Provider実送信testなし |
| Referral/LINE: `Models/V2/UserReferral.php`, LineFriendship/PendingFollow/WebhookEvent、`Domain/Line/Services/V2LineFriendService.php` webhook insertOrIgnore; occurred/followed/unfollowed/rewarded/claimed/reply_attempted日時 | Model＋QueryBuilder / V | helper＋Model、紹介重複防止／LINE順序・再送・concurrency | API / Provider numeric/offset入力の既存parseを維持 |
| Export: `Domain/Reporting/Services/V2ExportWorker.php` claim/expireCompletedJobs、`V2ExportRowSource.php` snapshot cutoff、ExportJob; cutoff/lease/expires/started/completed/failed日時 | Model＋比較 / V | 規約＋比較helper、ReportingExport／業務集計回帰 | API image内CLI / 常駐Export・Schedulerを新規起動しない |
| Audit: `Models/V2/AuditLog.php`, `AuditDailyDigest.php`; occurred_at/generated_at | Model / V | 規約のみ、既存UTC署名payloadとJST business_date不変。新規chain・digest・concurrency／過去誤保存の未補正を検証 | API＋Mail（Audit writer） / 過去の誤ったoccurred_atによる既存chain不整合は自動修復しない |

- 上記のModel経路はcreate/save/instance touch/自動timestampsを保護するが、Builder touchのglobal解決ではない。実SourceにBuilder touch呼出しなし。必要時はhelper付き明示updateを使う。insert/update/insertOrIgnore/upsert/raw/bulkの差を保存testで確認する。legacy非V2参考実装はこのV2 runtime修正の対象に数えない。
- Phase 1のimage／vendor volume／internal network手順を再利用。PHP 8.4.23、Laravel 13.15.0、PostgreSQL 17.11。DB公開portなし、Source read-only、vendor/storage/cache/PG data分離、Production ENV/Secret/Redis/Providerなし。既存guardで各実行前後の実接続・schema・port・migration inventoryを確認。空の隔離DBへの既存75 migrations適用と、独立したfocused群間の同DB初期化だけを実施し、testの内部fork/reconnectも同一隔離接続を使用する。
- 原因切分け：旧raw test fixtureのJST offset消失による401／期限ズレをfixture側だけで修正。API raw DB文字列のoffset依存期待は同一instant比較に変更し、制約・権限・失効期待は維持／強化した。Audit/Point foundationがfixtureを残すため無関係な群を同じDB状態で連結すると既存件数期待が衝突する。新環境は作らず既存guard下で群別DB初期化を行う。初回のtest import不足・空payload fixture、Catalogの再parse offset消失を修正し、初回失敗と最終結果を分離する。
- 実DB基準は `2026-09-17T10:28:20+09:00 = 2026-09-17T01:28:20Z`、epoch `1789608500`。入力非破壊／null／naive解釈／秒精度／DB既存fraction丸め、API serialization、期間inclusive/exclusive、Point cutoff、Outbox available/lease境界、直接Session失効、Audit chain／business_date／digest／snapshot checksumを検証する。既知+9時間の過去User/Banner created_atと署名不整合Audit fixtureを新規正常writeと混在させても過去値・署名を変更しない。
- Worker互換性は旧Worker→既存API Base→今回の変更を分離確認。旧→BaseでMail/SMS worker本体・TOPICS・Provider binding・SMS/outbox設定の変更なし。共用Mail serviceへのAgency通知・購入表示変数追加は既存Security Mail TOPICSを拡張しない。readonly Runtime確認でAPI/両WorkerのDB session UTC・APP/PHP JST、既存75 migrations（073–075含む）を確認。Schema追加適用・ENV変更・送信有効化は不要。実効Mail transportは既存mailgun、SMSは既存4S provider／credential PRESENTで、test送信は行わない。
...
## ADMINFRESH-20260918 Admin追加Fresh認証撤廃 Phase 2

- Human確定仕様に従う旧Server／PlatformのSource-only変更。開始clean、previous expected main／fetch後origin/main／採用Baseは `c6c5c3e3b221f5f883815a280f46be0e520fbc6a`。Issue `none`、Risk `R3`、Lane `Strict Change`、Activation `none`、通常Worktree、Branch `fix/ADMINFRESH-20260918-phase2`。既存App wrapper用transient exact-path Policyだけを使用。新Serverアクセス、Production Deploy、旧Preview Activation、Runtime／MFA Policy変更、Migration作成・Runtime適用は0。
- `V2AdminFreshMfaAuthorizer`の3つの業務Fresh拒否だけを除去し、全authorizePermission callerの旧Fresh/action引数を整理。QA GET／preflight／Mutation、Financial Export、Point Plan／Campaign、Point Adjustment、認証Policy／Admin作成、User State／Tag、Admin Agency管理、LINE／Referral、Gacha／Probability公開・予約・販売状態、Legal Content公開／Archiveを含む。Session hash／本人／role一致、active Admin、enrollment状態、idle／absolute期限とPermissionを保持。
- Password Fresh fallbackの照合と依存を削除し、reauthenticate Controller入口で旧password値を除去。旧method=passwordはMFA検証不成功として拒否し、値を認証・log・Audit・DBへ渡さない。TOTP／WebAuthnのSession-bound検証、MFA失敗Limiter、Session／CSRF rotation、absolute期限維持、Audit、enrollment／Recovery条件は不変。Login Password照合とLogin／MFA／OTP／SMS防御は変更しない。
- 通常操作のFreshMfaDialog／先行Prompt／自動再試行を全撤去し、参照APIも通常Session＋Permissionで利用する。Recovery Code再生成のUIだけの追加Fresh要求も除去し、BackendのAuthenticator要件は維持。入力Validation、通常の公開確認Dialog、Idempotency／OCC、duplicate実行防御、Audit／Ledger／Transaction／DB lock・整合性は保持。
- Contract version `2.0.0-alpha.31`を維持。OpenAPIの業務x-fresh-mfaと必須説明を更新、password fieldと旧method enum／fresh_mfa_expires_inはdeprecated互換として残す。生成Admin clientは既存generatorのみを使用。Phase 1のoptional/deprecated/writeOnly current_password互換と3系統Limiter撤廃は復活させない。Storefront/Public/Agency/Webhook Artifact、Schema、日時保存／binding／cast／timezone変更0。
- Residual分類：`V2AdminFreshMfaAuthorizer::isFresh`と既存5分configは呼出しのないlegacy日時predicateで、日時境界回帰を維持するため今回は未変更。業務callerは0、Policyで業務gate再導入を拒否。`V2AdminReauthenticationService`／Controller／Route／WebAuthn serviceはMFA専用機構、Admin clientのrequiresFreshMfa／error mapping／旧enumは互換のみでUI callerは0。Session Model／Managerのmfa_verified_at保存は通常MFA状態として維持。ExternalIdentity／SMS／AgencyPortalのFreshは他Realmで対象外。過去ADR・Worklog、退行検出／互換testsの文字列は歴史・検証用途。
- Local実行：Admin focused 13 files／64 tests、Client／Auth／User read 3 files／47 tests PASS。対象Chromium 8件（Point Plan Create/Update・Permission拒否、Point Adjustment・Operator拒否、Auth Policy、User State、User QA、Gacha QA保証）PASS。Browserはloopbackテストserver＋synthetic API interceptionであり、実Runtime操作ではない。Policy 209、OpenAPI 16、Release 48、base-aware compatibility／bundle／generator check／quality gate／Admin typecheck・lint・build／secret scan PASS。追加Navigation 2 files／14 tests PASS。最終HEADの全Required CI、API V2 suite、exact-head fresh self-reviewはPRで結果を追記して確定する。
- API回帰はstale／null mfa timestampでも業務成功、失効Session／Disabled Admin／Permission／CSRF／Origin拒否、Login失敗／MFA検証Limiter、TOTP／WebAuthn、Point Ledger／Audit／冪等性／rollback、Policy MFA値保持、Gacha business rule、QA参照・Mutation、Export job／downloadを検証する。手動の共有DB操作やMigrationは行わず、既存CIの隔離V2 schema／全Suiteを変更せず使用する。
- 初回検証の旧Fresh期待・UI fixtureはHuman仕様へ追従。追加Navigation testの型引数誤りもtest側で修正し、最終typecheckを再実行。E2Eのpermission fixtureはSession roleと一致させ、Gacha QA fixtureにはmain既存loadStateが要求するVersion GETを追加。アプリのSession／Permissionを緩める修正ではなく、実際の拒否／成功をassertする。既存Financial stream返却型の問題は本変更外で、CSV stream成功を検証済みとは扱わない。必要なjob／downloadおよび3 endpointのpermission境界を検証する。
- 追加レビューでExportの6回job/download回帰へ期限切れtimestampを明示し、Gachaの追加成功検証はassert失敗時もfinallyで隔離transactionをrollbackする。追加Catalog／Navigation focused 5 files／29 tests PASS（先行Navigation 14件を含む）。このtest補強後のHEADで全Required CIを再実行し、旧HEADの結果を流用しない。
- 統合API CIで1 error／7 failuresを検出。今回の引数削除で残存していたUser Prize参照のnamed action引数を除去し、Repository横断の旧named引数拒否testを追加。認証Policyの追加成功testは既存no-op拒否に抵触しない変更payloadへ修正する。公開済みGacha fixtureの再公開／予約は既存lifecycle 422を維持し、正常な初回公開・予約・取消等は既存Lifecycle全testのSessionを6分経過へ変更して成功／rollback／整合性を検証する。Sourceのbusiness rule、権限、日時処理は変更せず、最終HEADで全CIを再実行する。
- 次の統合API CIは720 tests中、新規Point Plan null-MFA fixtureの1 failureだけとなった。変更のないpayloadでは既存version規約上version 1のままが正しいため、テストpayloadでnameを実際に変更し、version 2成功の期待は維持する。既存のVersion／OCC／公開済み商品規約には手を加えず、修正HEADで全CIを再実行する。
- merge条件は最新main、全Required Checks SUCCESS、final-head SEV-0/1=0、mergeable clean。auto-merge／merge queue／Deploy／Activationは使用しない。Rollbackはreview済みSource revert（追加Fresh要求が再導入される）でSchema／日時データ操作を伴わない。最終GitHub結果、実測metrics、Source equality、cleanupはPRを正本として記録する。

## ADMINLAYOUT-20260918 Admin関連7画面 レイアウト統一・縦長動画preview修正

- Human承認範囲は旧Server／Platform Adminの最小UI修正と旧Test Admin反映まで。Baseはclean local／Remote `main@319767d8a265951a40273a68644c6a7d31204c6b`、Issue `none`、Risk `R2`、Lane `Strict Change`、Application Runtime Activation `immediate`。通常worktreeの単一Branch `fix/ADMINLAYOUT-20260918-seven-pages`、単一PR。Point関連pathの機械判定とPreview Activationを含むためStrictとし、App transport／Runtime用のRepository外exact-path Policyを使用する。
- Referenceはバナー一覧の `workspace`／`announcement-workspace`／tableセル・pagination、問い合わせ詳細の `contact-detail-card`／`contact-detail-list`、認証設定の860px幅／`settings-panel`／折返し可能な `settings-actions`。Admin shellの最大1440pxとDesktop 32px／Mobile 14px padding、既存breakpointを維持する。新framework／共通shell rewriteなし。
- Route確認：ポイント一覧はnavigationの `/purchase-plans`（PointPurchaseManagementWorkspace）、ランクは `/catalog/ranks`（RankMasterWorkspace）、紹介 `/settings/referral`、LINE `/settings/line`、Admin代理店一覧 `/agencies` と詳細 `/agencies/[agencyPublicId]`（AgencyWorkspace）。Agency Portalではない。ガチャ詳細 `/catalog/gachas/[id]` のCatalogGachaRankPrizeManagerが持つ未制限videoを対象にする。
- ポイント一覧は既存toolbar・pagination配置とcard/tableセルのpaddingを適用。ランク／代理店一覧の未定義table-containerを既存catalog-table-wrapに置換し領域内scrollにする。紹介／LINEは860pxとsettings cardに統一。代理店詳細のdialog用560px／max-height制約を除去し、detail card／label-value gridと折返しaction配置、同componentのedit formへ既存form cardを適用する。
- ランク動画は最大240×135px、固定135px高、`object-fit: contain`、選択欄はcell幅以内とする。既存controls／muted／playsInline／preload／URL／選択・保存処理は変更しない。画像previewとmediaなしfallbackを保持する。API／OpenAPI／DB／Migration／Auth／Permission／Point計算／決済／抽選／Inventory／Agency attribution・Realm／LINE Secret・Token・送信処理の変更0。
- Local focused component 5 files／36 tests PASS。初回Browserは16 PASS／10 FAILで、追加fixtureのRank必須画像をnullにしたため失敗した。契約通り必須画像を付与してfixtureだけを修正し、動画なしは必須Rank画像と景品fallbackを保持する。Chromiumの実WebM portrait 360×640／landscape 640×360、image／noneをDesktop 1440px・Mobile 390pxで検証し、実寸・再生／pause・controls・contain・row高さ・登録済み景品までの相対距離とdocument overflowをassertする。7画面と代理店editのブラウザgeometry／既存保存・権限回帰を実行する。最終結果はPRに確定する。
- 旧Testの確認originは現行NginxのAdmin upstreamと既存Preview containerから確認する。canonical GitHub Preview workflowのAMD64 artifactのみ利用し、全Required Checks・final-head self-review・squash merge・head/merge tree一致・content diff 0・digest/OCI照合後、既存旧Test Adminだけを `--no-build --no-deps` で更新する。workflowのnormal modeが同時生成するAPI imageはRuntime反映しない。旧Test API・DB・Worker・Storefront・Agency・Nginx・ENVは更新しない。
- Migration作成0／適用0、Production NOT RUN、新Server NOT RUN、実データの業務操作0。Backend suiteの追加手動実行なし、既存Required CIは省略しない。Human最終Browser確認はHUMAN PENDING。Rollbackは保持した旧Admin imageの再適用で、DB／ENV変更を伴わない。merge・self-review・Runtime technical／HTTP smoke・main equality・cleanup・実測metricsはPRのcloseoutに記録する。
- Local最終結果：focused 36 tests、Admin lint／typecheck、Admin Build 2回 PASS。Browser既存操作・7画面配置等26件とmedia focused 8件の計34 unique cases PASS。再試行途中の公開中空sectionを誤選択したlocatorと、1frameしか生成されないWebM fixtureを修正し、対象region名と複数frame生成に変更した。assert緩和・本体再生処理変更なし。最終media 8件は52.7秒で全PASS。端末の日本語font描画がないためscreenshotの日本語glyph自体をvisual PASSとは扱わず、DOM・実寸・overflow・media decoding／play／pauseをtechnical根拠とする。Human実ブラウザでの最終visual確認は別途とする。
- PR #490初回Required CIはAdmin Skeletonの新規E2E file allowlistで停止。UI実装／Workflow／Policy gateの変更は追加せず、追加layout casesを既存referral設定E2E fileへ集約した。coverage／assertionsは保持し、最終headのtypecheck／lint／全対象34 Browser casesとRequired CIを再実行する。

## ADMINUI-20260920 Admin UI改善 第1弾 — 画像・Table表示調整

- Human承認範囲は旧Server／Platform AdminのUI-03・04・05・06・07・15のみと旧Test反映。Baseは作業開始時のclean local／Remote `main@e2cb91b6f9c68d9369d4287fb42da6f9f7f4767c`（PR #490適用済み）、Issue `none`、Risk `R2`、Lane `Strict Change`、Application Runtime Activation `immediate`。通常worktreeの単一Branch `fix/ADMINUI-20260920-images-tables`、単一PR。保護対象Point pathと旧Test Activationを含むためStrictを適用し、Repository外の一時exact-path App Policyを使用する。
- Referenceは既存バナー／ユーザー一覧のheader/body左揃え・middle・12px/14px padding・border/background、announcement-pagination、catalog-table-wrap、PublicAssetPreviewのcontain。#490の1440px shell・settings/detailレイアウトと240×135pxの動画controlsを保持する。
- UI-03: ガチャ詳細Rank画像を最大144×144pxのcontainへ変更。元画像比率を保ち全体を表示する。画像列・動画列・登録操作列の幅を割当てる。UI-04: Catalog一覧・Rank・演出thumbnailを112×84px、詳細登録景品を144×108pxへ拡大。既存詳細previewは維持し、画像URL・API・controls・編集／選択処理を変更しない。
- UI-05/06: Category／Tag、購入商品、配送、通知、問い合わせ、バナーVersionのUUIDを先頭8文字＋ellipsis＋末尾8文字、nowrap、title全文確認へ変更。バナーURLは120px ellipsisとtitle、既存copy/openの元URLを保持。min-widthを該当Tableだけ圧縮し、残る多列Tableの購入商品末尾・バナー公開／編集／削除だけstickyにする。配送／ガチャ詳細／その他Tableへ一律stickyを追加しない。
- UI-07: お知らせのカテゴリ・状態・短いheaderをnowrapにし、タイトル列とID列の配分を調整。UI-15: ランク演出と問い合わせを既存announcement-table／scroll wrapper／右端paginationへ揃える。Tableのpadding・border・背景・vertical alignmentを再利用し、大規模共通CSS rewriteなし。
- Category／Tagの390px検証で既存旧Testでもdocument幅762pxとなることを確認。Table内の絶対配置sr-only見出しがscroll wrapper外へ寄与するため、今回対象のtaxonomy headerだけposition:relativeとしてTable内scrollを保持する。正常な#490 shellや他画面を変更しない。
- Local focused 9 files／64 tests PASS。初回2件の全文UUID期待を短縮表示＋title保持へ更新。負荷下の非同期描画race／5秒timeoutは、お知らせtestで実Table headerを待ち、単独workerで同じ64 assertionsを再実行して全PASS。timeout延長・assertion緩和なし。Admin lint／typecheck PASS、BuildとBrowserの最終head検証結果はPRで確定する。
- Chromium synthetic API fixturesでDesktop 1440／1366、Mobile 390の画像natural dimensions／contain、動画decode/play/pause、一覧と詳細のサイズ差、UUID title／nowrap／行高、操作hit-test、scroll領域、短いlabel、header/body alignment・padding・border・背景・paginationを確認する。実データへの業務操作なし。初回36件中34 PASS・taxonomy Mobile 2 FAILから前記原因を修正し、最終Browserを再実行する。
- UI-01/02/12/13/14/19および第2弾以降は変更しない。API／OpenAPI／DB／Auth／Permission／Point／Payment／Draw／Shipping業務ロジック／Upload機能／依存・CI・保護設定の変更0。Migration作成0／適用0、Production NOT RUN、新Server NOT RUN。Backend全Suiteの追加手動実行なし、既存Required CIの検証は省略しない。
- 旧Testは既存正規GitHub AMD64 Preview artifactを使用し、全Required Checks・exact-head Strict self-review・squash merge・tree一致／diff 0・digest／OCI照合後、Adminだけno-build/no-depsで反映する。API・DB・Worker・Storefront・Agency・Nginx・ENVを変更しない。rollbackは保持した旧Admin image。最終Merge／Runtime／HTTP／Browser／cleanup／main equality／metricsはPR closeoutを正本に記録する。Human最終ブラウザ確認はHUMAN PENDING。
- 既存#490を含む55 Browser casesは51 PASS、Mobileガチャ一覧thumbnail幅の4件が89.6pxとなりFAIL。対象画像列を140pxに確保して112px表示を保持し、ガチャ画像／動画12件とバナー3件を最終Buildで再確認する。バナーstickyはDesktop 1024px以上に限定し、Mobileでは通常のTable内scrollとして本文領域を確保する。日本語glyphの最終visual確認はHuman-ownedで、CodexはDOM／実寸／hit-test／media decodeをtechnical根拠とする。

## ADMINFORM-20260920 Admin UI改善 第2弾 — Form・Modal・Spacing・入力案内調整

- 旧Server／Platform AdminのUI-08・09・11・16・17・20と旧Test反映のみ。開始時dirty／untrackedなし、local／live Remote mainは `795f4f981eeae033b7dc3f105a8cfd569edcbe0a`。GitHub APIで#490 merge `e2cb91b6f9c68d9369d4287fb42da6f9f7f4767c`／#491 merge `795f4f981eeae033b7dc3f105a8cfd569edcbe0a`を確認。Issue none、Risk R2、Lane Strict Change、Activation immediate、通常worktree、Branch `fix/ADMINFORM-20260920-forms-modals`。Point pathと旧Test操作を含むためStrict。App transport／Preview用のRepository外exact-path Policyを使用する。
- ReferenceはCatalogの18px checkbox／9px gap、既存ユーザー・決済filter card、問い合わせfilter、announcement form actions、認証／紹介設定の860px幅。#490/#491のshell・table・画像・video patternを保持し、新UI基盤は追加しない。
- UI-08: global labelのgridとcatalog-form-gridのstretchで、隣のmedia欄の高さがカテゴリSelectへ伝播する。gridをstart alignmentにして新規 `/catalog/gachas/new` と実編集route `/gachas/[gachaPublicCode]/edit`を修正。選択肢／値／保存／validation変更なし。
- UI-09: ランクの未定義checkbox-rowを既存catalog-checkboxへ統一。演出radioは全input向け42px指定から除外し、同じ18px／flex／gapを適用。ポイント商品のcheck-rowをform内に限定して同じ配置へ統一。checked／disabled／label操作／keyboardとnative inputを保持。
- UI-11: announcement-previewの720px widthが後続dialog-panelのmax-width 440pxで制限され、全panel scrollで閉じる操作も移動していた。対象selectorで860px上限とviewport内max-heightを設定し、本文だけ内部scrollとする。同じPreview classのページPreviewも直接影響するため検証対象。景品編集は既存form内にbody wrapperだけを追加し、headerと保存／キャンセルをscroll外に保持する。Modal機能・Preview本文・景品保存処理変更なし。
- UI-16: 決済／配送のresetを既存secondary buttonへ、ポイント／バナー／お知らせ／ページ一覧filterを既存決済cardへ統一。バナー上部登録構造は維持しカテゴリ追加buttonのstretchを抑える。ページ新規actionを既存page headerへ、ポイント／演出／content form幅と操作位置を設定画面patternへ揃える。代理店集計は既存contact filterをAdmin時だけ使用し、共用componentを利用するAgency PortalのDOM/classを維持する。Mail formは幅変更対象から除外。
- UI-17: 代理店の既存required項目、Catalogカテゴリ／タグ共用formのCode・Slug・名称・表示順へ「（必須）」、説明／メモへ「（任意）」を表示。既存Adminに統一必須markerがないため既存の括弧付き任意表記に合わせる。native required／pattern／min/max、submit validatorとserver契約を変更せず、入力のaccessible nameは維持する。
- UI-20: 既存invalid／invalidCampaign関数とAPI前の拒否guard、HTML制約を保持。form-local touched／attempted stateを追加しfield単位に表示する。submit attemptではnative invalid eventも捕捉し未入力を含め表示、不正な保存APIは呼ばない。送信buttonは検証attemptを受け付けるが権限・busy防御と保存可否は維持。正常な編集初期と限定bonus初期は赤字なし、不正な既存商品は既存summary警告を保持。bonus編集切替／登録成功／登録へ戻す操作で表示stateをresetする。
- Local初回focused 10 files／80 tests PASS、関連追加6 files／52 tests PASS（重複あり）。初回の全Admin指定は引数位置が想定と異なり対象を絞るため停止し、必須label変更による既存exact label期待の2 FAILを確認。input名を明示し既存accessible nameを保った上でfocusedを再実行した。最終12 files、lint／typecheck／BuildとBrowser結果はPRへ確定する。
- 最初の新規Browser 18 casesは12 PASS／6 FAIL。景品fixtureにCanonical Rank／演出一覧／first_published_at:nullが不足した3件と、Next route announcerまでvalidation alertに数えた3件を修正。fixtureとworkspace内locatorだけを補正し、assertion・本体domain ruleは弱めない。1440／1366／390でSelect実寸、隣接portrait／landscape、radio／checkbox、内部scrollと操作位置、required／optional、未操作／操作後／submit／編集／bonusを検証する。既存第1弾と関連保存／権限回帰も実行する。
- API／DB／Migration／Auth／Permission／Point計算／Payment・Gacha・Rank・Agency・LINE業務処理／Mail Preview／画像選択機能／Nginx／ENV／新Server／Productionは変更0。Migration作成0・適用0、Production NOT RUN、新Server NOT RUN、Human最終Browser確認はHUMAN PENDING。Backend全Suiteは手動追加せずRequired CIを維持する。
- 全Required Checksとfinal-head Strict self-review、squash merge、head／merge tree一致・diff 0の後、canonical GitHub AMD64 Preview artifactをdigest／OCI照合して旧Test Adminだけno-build/no-depsで更新する。API／DB／Worker／Storefront／Agency Runtimeは更新しない。rollbackは保持した旧Admin image。最終PR／SHA／artifact／technical smoke／cleanup／main equality／Build・Activation回数と時間metricsはPR closeoutを正本に記録する。
...
- Existing session lacked name/email. Its authenticated current-user response now adds optional display_name and email contract properties from users.display_name / users.email_display, with no-store/private response headers retained. Existing getSmsVerificationStatus returns only the current authenticated User's verified, non-revoked phone. No new API is necessary.
...
- Validation: OpenAPI bundle and base compatibility PASS; Client and Testkit generation/typecheck/lint/build/unit/exports/network checks PASS; OpenAPI policy regression 18 PASS; artifact validation regression 26 PASS. Backend focused initial run: 54/55 passed; the new account snapshot assertion needed a pre-snapshot database refresh (storage timestamp representation/defaults); final Contact rerun: 14 tests / 162 assertions PASS. Session disclosure regression added after source inspection. Final results and exact-head CI/self-review are recorded in the PR before merge. Human browser login and final acceptance remain pending.
...
## CONTACTBUILD-001 — Contact Production Preparation Build Path

- Human authority: preserve approved Runtime Source e16f65504dc5286de2fcd70988b770d1a16d1eaf; repair the existing canonical Production workflow only. Base 369d6670f40d28419ad22a4b93cacc44a11a6ff0; R4; Strict Change; Application Runtime Activation: none; Issue none. Dedicated clean worktree preserves the original dirty worktree and all existing worktrees/overlays. Task Policy is limited to GitHub transport and changed paths.
- The source-to-base diff has exactly five metadata/test/worklog paths and no Application Runtime delta. Protected-main metadata now binds the Human-approved source/PR/Change. Current workflow checks/policy and reviewed source checks/tree equality remain required; arbitrary ancestors, missing/malformed commits and authority mismatches fail closed. Workflow code and exact Runtime Source use separate checkouts; image OCI revision and manifests retain requested source. No automatic promotion to main.
- Local focused Source/Artifact/Check-run tests: 75 PASS; policy regression: 209 PASS; repository policy and current source secret/path scan PASS. First policy run exposed missing new-helper fixture inventory; required-file inventory was extended and all original assertions retained. Storefront focused provenance/workflow tests: 67 PASS; lint/typecheck/artifact/policy/secrets PASS; audit high threshold PASS with three moderate findings and no dependency changes.
- Source review of 000076, rollback guards, old/new Contact contracts and the dedicated reply worker is recorded in docs/operations/deployment/contact-production-preparation.md. PR #110 final-head five checks, self-review and identical squash tree verified without rerun. Actual old Production SHAs/ENV presence were not inspected; source baselines and later activation preflight are explicit.
- Canonical Production Artifact generation, Runtime Activation, Migration creation/application, ENV/Nginx/DB mutation and Mail send: NOT RUN / zero. Local full Backend/Browser/E2E and image builds not run for this workflow-only change. Required CI is preserved; its isolated PR ARM64 verification is test evidence, never an approved Runtime Artifact. No new Production workflow, secret, dependency or application change.
- Final head, CI/CodeQL/dependency-review, fresh self-review, squash actor/SHA, cleanup and main synchronization are recorded in the PR closeout. Workflow/metadata rollback is a reviewed revert; Runtime and retained DB history are unaffected. Next Human step: Production Runtime Artifact Build GO, not activation.

## CONTACT-20260924 — お問い合わせ状態 文言・遷移修正

- Human authority: small Contact status correction; one branch / PR, no Issue, no merge, no Runtime activation or Artifact publication. Base `7f635c01585d1469e1bdd376cc6038982ba126ff`; R2; Standard Change; Application Runtime Activation: none. Dedicated worktree protects the original dirty worklog and existing worktrees. Task Policy is GitHub transport compatibility only; Source Lock none; Migration Allocation Lock not applicable.
- Shared Admin label now displays `new` as 未返信; filters use the same label function as list/detail/history/modal. The existing `in_progress` label/filter/history remain readable, but UI and Backend reject new manual transitions to it. Existing closed label (完了), closure operation, timestamp and closed-reply rejection remain unchanged.
- Initial submission remains `new`. Admin reply save atomically records `replied` with the existing reply/history/outbox transaction. Owned follow-up from every status records `new`, clears `closed_at`, and retains the original inquiry and idempotency behavior. No Mail Worker, template, provider or timestamp policy changes.
- Local verification PASS: Admin focused 15 tests, targeted ESLint, Admin typecheck; Backend Contact/Admin/vertical-slice 32 tests / 312 assertions; PHP syntax for all five changed PHP files; diff whitespace review. Backend fixtures use a disposable guarded test database and existing migration files only. Initial fixture setup failed closed on a test database name marker before schema creation; corrected the isolated fixture name and completed validation. No test failure or assertion weakening.
- Migration creation 0; application to existing Test/Staging/Production databases 0; existing data correction 0. Migration/Contract/Artifact/Storefront diff 0. Existing schema is initialized only inside the disposable test fixture. No schema, auth/session, SMS, shipping, profile, point/payment/draw, infrastructure/workflow, ENV or Production mutation.
- Browser/E2E and Human visual acceptance NOT RUN; local application builds 0; Runtime Activation 0; Human wait 0. Five exact-head Required Checks and machine-readable self-review are recorded on the PR. Preserve the open PR, task branch and worktree for the dedicated merge chat. Rollback after a later merge is a reviewed source revert; no data repair is needed.
## PRIZEIMAGE-20260924 — 公開ガチャ詳細・抽選結果 景品画像Contract追加

- Human指定の旧Server／Platform API read contract拡張。Issue none、Risk R3、Lane Strict Change、Activation immediate（旧Test APIのみ）。Exact Base／local main／origin main／live protected mainは `69a260c1aa52c677fde72526de3d1d1853efea09`。元checkout `docs/CONTACT-20260920-old-test-activation-record@fda73b02ffa5dfe6b532d51effdacf6211ba85aa` の既存Worklog dirtyと未追跡Worktreeを以前のContact記録と特定し、そのまま保持。隔離Worktree／Branch `feat/PRIZEIMAGE-20260924-public-read` を使用し、元の別Task差分を混入しない。Source／Migration Allocation Lockは不要。GitHub transportにはexact-path transient policyを使用する。
- 開始時旧Test API OCI revision `428820aaf26fffac3aeb3dd6233d321ed6921736`。Client／Testkit alpha.38、Public OpenAPI alpha.34を最新mainとimmutable ledgerで再確認。今回Public alpha.35／Client・Testkit alpha.39候補、Site Schema alpha.23の既存参照を維持。
- Gacha Detailへ唯一の `prizes[]` をadditive追加。公開Versionの景品snapshot画像・名前、public Prize ID／Rank Master ID、`prize_inventories.total_quantity`、既存Version順 `sort_order, id` を返す。Rank flag・合計・既存fieldは不変。各景品総在庫を3／5／2で確認し、残数やRank合計への置換なし。
- Draw GETだけに `rank_lineup_image` をresults／high_rank_results／prize_countsへ追加。保存された当選Rank revisionのlineup imageを既存経路で返す。既存 `prize.presentation_asset` は保存済み当選画像IDを維持し、GET時にpublic content asset経路へ解決する。元のAdmin向け保存pathを新しい公開表示に使用しない。Draw POST／保存／Replay／選択／Point／Inventory mutationは変更0。
- 隔離PostgreSQL fixtureのみでAPI関連50 tests／607 assertions PASS（初回testのroute・fixture file配置・抽選fixture選択・JSON object順序期待を補正後）。Client生成差分／typecheck／lint／build／34 tests、Testkit生成差分／typecheck／lint／build／49 tests／export／network境界、OpenAPI bundle照合、release source validation PASS。Rank lineup差替後の旧結果不変を追加しfocused4 tests／66 assertionsを再PASS。Migration作成0、既存76 migrationsは使い捨て検証DB初期化のみ、共有旧Test／Production適用0。
- Humanは既存Strict Required CIのGitHub runner上 `production-arm64-artifact` 候補Build／Artifact検証だけを明示許可し、全Required Checks PASS後のPR／Merge続行を承認した。Production配置・activation・restart・DB／Migration・traffic切替、新Server接続、Nginx変更は引き続き禁止。保護やworkflowを緩和しない。旧Host残容量も事前検査で約0.5GBのため、旧Test image import時に容量を再検証し、既存image／volume／cacheを無断削除しない。
- Handoffは `docs/handoffs/PRIZEIMAGE-20260924.md`。Storefront Repository、Admin UI、Schema、Auth、公開／Version作成、抽選・在庫・会計ロジック、Production、新Serverは変更0。Commit／push／PR／Required Checks／Merge／Artifact publication／旧Test activationは検証完了後に別々に記録する。Runtime rollbackは保持済み旧API image、DB操作不要。
- 初回CIでpolicy unit fixtureのClient／Public Contract固定値が旧版のためidentity検証が停止。fixtureをalpha.39／alpha.35へ同期し、検証ロジック・assertion・Required Checkは一切変更しない。同じPRの直接必要なfixture更新として扱う。
- Policy 209 tests PASS後、既存Contact破壊的変更のOpenAPI unit fixtureも最新版を暗黙参照していることをCIで検出。既存Auth fixtureと同様に歴史的authority alpha.34を明示し、将来版を誤って許可しない既存のnegative assertionsを維持する。Production policy／workflow変更0。

## REL-039 — Immutable prize presentation contract reconciliation

- Issue none; Risk R3; Lane Strict Change; Application Runtime Activation none. Base `be1a8f3f822d23f3251d32e616fb0b2fe422714e`. The implementation remains one PR (#500); this mandatory post-publication metadata PR follows the canonical contract Artifact runbook, reusing the same isolated Worktree without another Issue or Worktree.
- PR #500 reviewed head `ec17f06ed71d90e9b7c9f0b1ae302271cefb4cef` passed all five Required Checks, CodeQL, dependency review, and the explicitly allowed GitHub CI ARM64 candidate build. Fixed-head self-review `https://github.com/ideal-sol/oripa/pull/500#issuecomment-5811497659` preceded the GitHub App squash merge; reviewed and merged trees are identical. No Production runtime operation occurred.
- Canonical contract-only publication Run `35981484873` and independent exact-ID readback verified Artifact `10800178238`, Client/Testkit `2.0.0-alpha.39`, Public OpenAPI alpha.35, and retained immutable Site Schema alpha.23. Record exact Source Commit and byte digests, preserve every predecessor unchanged, and clear the candidate. No Artifact rebuild or overwrite.
- Local release 49 tests and policy 209 tests PASS; settled-ledger verification of the downloaded immutable bundle PASS. Historical alpha.37 breaking-release tests explicitly reconstruct that historical ledger, preserving the original negative assertions and preventing current additive release metadata from changing their meaning.
- No application, generated package, OpenAPI, infrastructure, dependency, DB, auth, point, payment or draw behavior change in REL-039. Migration creation/application 0. OLD Test API activation and technical confirmation remain separate PR #500 closeout evidence. Production/new-server operations NOT RUN.
- Human authorized deletion only of this Task Worktree's generated node_modules after confirming no process used it. Only that directory was removed; shared stores/caches, Source, lockfile, settings, Artifacts, evidence and rollback images were preserved. Remaining old-host capacity was 565 MiB, so image import still requires a checked capacity gate; no additional deletion is inferred.
- Human subsequently authorized only `pnpm store prune` after a dependency-process check. No install/build/update or direct store file use was present; existing dev/start services remained unchanged. Exactly the authorized prune ran, without manual store deletion or extra node_modules/Source/Artifact/rollback deletion. Free capacity reached 957440000 bytes against 924695552 required, so canonical OLD Test API-only Artifact `10799954368` was safely imported; previous images retained.
- Source PR #500 OLD Test Public API activation is complete: exact API Source `be1a8f3f822d23f3251d32e616fb0b2fe422714e`, reviewed OCI revision `ec17f06ed71d90e9b7c9f0b1ae302271cefb4cef`. Health, environment/network/mount boundary and unrelated runtime preservation PASS. HTTP checks confirmed all three prizes and both Rank images in the populated public Gacha, stock flags and order. Exact activated-image isolated regression 50 tests / 607 assertions PASS; Draw uses synthetic HTTP fixtures without shared-DB test data. The disposable fixture DB was removed after testing. Technical evidence: `https://github.com/ideal-sol/oripa/pull/500#issuecomment-5811726678`. Migration/data correction 0; Production/new server NOT RUN. REL-039 itself remains metadata-only, activation none.

## DRAW-PRESENTATION-20260925 — Source invariant clarification required

- Issue: none; Risk: R3; Lane: Strict Change; Application Runtime Activation: deferred. GitHub protected main, wrapper ls-remote, origin/main, and local main all measured as `032aa4acd4af3f87428145afa044252e534b5e07`. The primary worktree contains another Change; isolated branch `feat/DRAW-PRESENTATION-20260925` starts at that exact authority and preserves all pre-existing worktrees and changes.
- Revalidated immutable Rank revision/display_order, full persisted Draw results, request_sequence, video snapshot, and response_data replay authority. Latest immutable Contract/Client/Testkit is alpha.39 with no candidate; no version reserved or Artifact published. Current main contains BASELINE-20260925; its review expiry is 2026-10-02. Baselines have been inspected, not rerun or reported as PASS for this Change.
- Human stop condition 6: the requested no-video successful new Draw requires clarification against existing mandatory-video invariants. V2DrawService::drawMetadata inner-joins the current video revision and asset, rejects an incomplete join with PRESENTATION_CONFIGURATION_INVALID, and prefixedAsset rejects a missing asset. Migration 000069 requires both immutable Rank and video revisions for canonical prize results, validates their ownership, requires a non-null video_asset_id, and prohibits Prize registration without a Gacha Rank video. The normal new-Draw path therefore cannot produce the required case J with a missing video snapshot.
- A defensive representative resolver returning null for an absent snapshot can be implemented without a migration, but that alone does not make a genuinely video-unconfigured Gacha drawable or prove case J through the normal Draw path. Do not silently relax mandatory-video guards, forge a dummy revision, suppress a valid video snapshot, or report a resolver-only test as successful end-to-end no-video Draw coverage. Human clarification is required on retaining the existing video prerequisite versus changing that prerequisite and its schema guards.
- Implementation, tests A-N, generation, commit, push, PR, checks, merge and publication: NOT_STARTED. Source changes: this isolated worklog only. Migration creation/application, runtime builds/activation, Production, ENV/Secret, Rank Admin, history mutation and backfill: 0. Branch/worktree retained for resumption; no locks acquired.

## DRAW-20260925 — Representative presentation and full new-Draw results

- Human resolved the mandatory-video ambiguity: Catalog/DB video prerequisites remain enforced; missing or unsafe snapshot projection alone returns null. Human additionally requires all 1/5/10/100/1000 results in new response_data and stable POST/replay/GET; high_rank_results remains filtered/capped at 20. Canonical Task ID is DRAW-20260925 (the initial working label above is not a separate Change), branch feat/DRAW-20260925-representative-presentation, dedicated clean-base worktree, Issue none, Risk R3, Strict Change, Activation deferred. Reconfirmed protected main/remote/origin/local base 032aa4acd4af3f87428145afa044252e534b5e07 before resuming.
- Resolver consumes the complete newly persisted result collection, reads only referenced immutable Rank revisions, detects ambiguous minimum display_order across distinct Rank identities, then uses request_sequence only within the highest Rank. It projects the selected saved rank/video with validation and never falls back. Existing read image enrichment is frozen at new creation so new reads return saved canonical content. Legacy read compatibility remains unchanged and never adds presentation/full results or mutates saved history.
- OpenAPI Public alpha.36 adds optional nullable DrawPresentation and expands results.maxItems to 1000; optional results remains for historical compact responses. Client/Testkit alpha.40 is the next ledger version, confirmed unused in GitHub Artifacts. Public operation count 76, unchanged Admin/Webhook alpha.34, referenced Site Schema alpha.23. Generators produced only expected contract deltas; lockfile changes only the exact workspace Client pin. Publication remains a post-merge operation with a separate metadata PR.
- Local tests: isolated API Draw/QA/presentation/history regression 49 tests / 529 assertions PASS; latest expanded representative suite 7 tests / 78 assertions PASS. Test-only PostgreSQL initialization applied the existing 76 migrations; migrations created 0, shared Test/Production applied 0. Initial test fixture password omission and JSON object-key order assumptions were corrected; no product assertions or DB guards weakened. Full 1000 HTTP POST/replay/GET: 1000 unique ordered results, 1,540,821 JSON bytes, approximately 42 MiB memory increase; 100 results: 183,870 bytes. Highest result is deliberately outside the 20-item high_rank_results. Current Rank name/order/revision/video changes leave saved presentation/results unchanged; legacy compact fixture stays absent and unmodified.
- Existing five-run 100/1000 performance checks PASS: latest 1000 p95 983.861ms, 58 queries, 33 SELECTs at either count. The historical compact-response 100KB assertion is replaced by an explicit 4MB full-result budget required by the new Human specification; time/query/transaction/rollback assertions remain intact. Focused tests retain mandatory-video rejection and fail-safe projection without DB invariant changes.
- Client generation/typecheck/lint/build/35 tests, Site Schema dependency build, Testkit generation/typecheck/lint/build/50 tests/exports/network, OpenAPI 20 tests and bundle checks, release source validation, policy 209 tests and release 49 tests PASS. Bounded necessary CI synchronization: policy's required Draw test name now matches the full-results test; policy/release fixtures track candidate alpha.40/Public alpha.36. No gate, permission, baseline, vulnerability fingerprint, or domain safeguard is relaxed. Current Security/ESLint baseline expiry remains 2026-10-02; final-head CI will validate the live findings.
- Out of scope: Storefront application/UI, Rank Admin, outcome/CSPRNG/probability/weight, draw counts, Point/Coin/Payment, inventory/counters/User Prize/Shipping, idempotency semantics, schema, legacy backfill, ENV/Secret/Nginx, Production. API behavior is additive; old clients may ignore presentation and obtain additional results. Browser/video/E2E and shared runtime activation are NOT RUN. Rollback uses the prior API image without rewriting saved responses.
- Concurrency: Rank/video edit versus Draw, inventory adjustment versus Draw, daily limit, and partial remaining-count execution all PASS independently (4 tests / 55 assertions). The first combined invocation exposed existing non-transactional fixture collisions; rerunning each case on its own freshly initialized disposable database resolved the harness issue without source/guard changes. Quality gate and 22-path secret candidate scan PASS; no binary/submodule/secret/PII or unrelated generated diff. Final-head CI, self-review, merge, publication/readback, metadata reconciliation and cleanup remain pending.

## REL-040 — Immutable Draw presentation/full-results artifact reconciliation

- Issue none; Risk R3; Strict Change; Application Runtime Activation none. Source DRAW-20260925 PR #506 head 20036e51e40a671597897ea762a3c04d9ef9d1bc passed all five Required Checks, CodeQL/dependency checks and exact-head self-review before squash merge `dadf79f3b0b2409a57e41b10a83c7b6570ea3507`; reviewed/merged trees match. Release metadata uses a separate branch/PR in the same dedicated worktree; the primary dirty worktree and concurrent branches remain untouched.
- Canonical contract-only workflow 36089413205, immutable Artifact 10845475225, version alpha.40 and independent readback PASS. Exact outer digest `3c5b35542cdaf7ed636aa0e7376e39d96463115ea14600c293d30d011e70b0e9`; source provenance remains `dadf79f3b0b2409a57e41b10a83c7b6570ea3507`, not the later metadata merge. All manifest/package/OpenAPI/SHA256SUMS digests are fixed in the ledger and release runbook; all predecessor records unchanged, Site Schema alpha.23 retained, candidate cleared, overwrite 0.
- Metadata scope: ledger, release runbook, worklog and the two existing release-state fixture assertions. No application/generated/dependency/security-policy/runtime delta. Local policy/release tests, settled-source/bundle validation and exact-head CI/self-review must PASS before merge; final outcomes and cleanup are recorded in the PR.
- Full new Draw results: 1/10/100/1000 remain ordered by request_sequence; 1000 HTTP POST/replay/GET and JSONB persistence verified, zero missing/duplicate rows, highest presentation independent of the unchanged high_rank_results cap20. Legacy absent presentation/results remain absent. Mandatory-video invalid Draw rejection remains intact; defensive snapshot failure or ambiguous highest Rank yields null without fallback.
- OLD Test activation is deferred: Source/Artifact handoff is ready without shared runtime mutation, but integrated Storefront browser acceptance requires subsequent exact-source Test API activation. No Browser/E2E claim, no Production activation, no ENV/Secret/Nginx mutation, no shared migration/history mutation. Disposable test DB/network removed; existing 76 migrations ran only in isolated testing. The primary worktree and unrelated Runtime state are preserved.
- Storefront handoff: PublicComponents["schemas"]["DrawResponse"]["presentation"] is optional DrawPresentation|null; DrawPresentation contains rank:RankReference and video_snapshot:PresentationAsset. Absent means legacy; null means safe representative unavailable; object means the one saved representative video. loading -> video -> result onEnded/Skip/onError; absent/null -> result. Never compare Rank IDs/names/sequence/array order or use high_rank_results/current Master/other video fallback. Remove per-prize-card video playback in the later Storefront task; all results are available from new response_data.
- Local metadata validation: policy 209 tests, release 49 tests, settled-source validation, exact downloaded bundle verification against the settled ledger, immutable predecessor equality and whitespace checks PASS. Five-path public-safe scope review confirms no binary/submodule or secret/PII changes. Required Checks and fresh fixed-head self-review remain merge gates; no checks are bypassed.
## PRODAUTH-20260925-A40 — Latest Platform Production Authority Sync

- Canonical daily Task ID `PRODAUTH-20260925`; this alpha.40 Change is PR #509 / branch `chore/PRODAUTH-20260925-A40-sync`, distinct from earlier #504 Catalog approval. Initial CI rejected the extra `-A40` Task ID suffix before application jobs; corrected PR/transport metadata to existing policy format without changing any CI rule. Final-head checks rerun after this record update.
- Issue none; Risk R3; Lane Strict Change; Application Runtime Activation none. Human authorizes isolated continuation while preserving the original dirty branch, Worklog and existing Worktrees. Dedicated branch `chore/PRODAUTH-20260925-A40-sync`; base/live protected main/fetched origin/main `e4361ece51fc1249a5cfb2c64cf56d3aa4bb0c29`. Runtime Target has the same initial value but remains a separate fixed authority after this metadata merge. Exact-path transient transport policy; no Source or Migration Lock needed.
- Git comparison base `e3121034c5dc7b9184673b1be64bb5076e9d3a90` is an ancestor, not a claim of observed current Production. Full five-commit inventory: #504 approved-source metadata, #505 baseline metadata, #506 accepted API Draw/Contract implementation, #507 release metadata only, #508 accepted API Public asset URL correction. Unapproved Application Runtime delta NONE. Runtime paths are CatalogReadService, DrawService and new DrawPresentationResolver; tests/docs/worklog, release ledger, OpenAPI/Client/Testkit and exact workspace Client lock pin account for remaining paths. No external dependency/workflow/migration/ENV/config changes.
- Fresh canonical alpha.40 exact Artifact `10845475225` readback PASS: source `dadf79f3b0b2409a57e41b10a83c7b6570ea3507`, run `36089413205`, outer `3c5b35542cdaf7ed636aa0e7376e39d96463115ea14600c293d30d011e70b0e9`, manifest `5fba7399e21cff0226e9ae43a9a3fa73dc7c61a0716ce00787b0f4b931778a59`, Client `724ff53616a5c05fd73a0570fd2c1455fc762a64bfa9936e61b73131ed5d2d99`, Testkit `555705b61664116144da8b7ea3a87dca1ff0066e4b027bdbba87e2e7750b052e`, Public OpenAPI `b469064b322d99b125995031aadfe765907651e014c003ccc82997a91843cd23`. Artifact Source is an ancestor of Target; OpenAPI/packages/lockfile byte-identical through #508. Ledger untouched; no republish or rebuild.
- Both live baseline files declare BASELINE-20260925 / expiry 2026-10-02, merged #505 at `032aa4acd4af3f87428145afa044252e534b5e07`; valid on September 25, refresh unnecessary. Existing accepted Technical/Human Browser evidence is reused, not rerun or claimed as newly executed.
- Authority-only scope: approved-source manifest, focused authority/provenance tests, Production authority runbook and this Worklog. Source identity ASSETURL-20260925 / #508; activation_authorized stays false. Canonical workflow, safeguards and Required Checks unchanged. Post-merge read-only exact-source authorization, checks, self-review, merge and cleanup evidence will be recorded on the same PR.
- Future Production scope API YES; Admin/Contact Reply/Identity/SMS/Agency/Scheduler NO. Application changes from this sync NONE; migrations created/applied NONE; ENV NONE; Production Runtime Artifact NOT BUILT; new server NOT RUN; activation NOT RUN; Storefront untouched. Mandatory Strict CI verification is separate from deployable Runtime Artifact authority. Rollback is a reviewed source-approval metadata PR, with no runtime/data action.
## SHIPONLY-20260926 — Snapshot-fixed shipping-only prizes

- Issue none; Risk R3; Lane Strict Change; Application Runtime Activation immediate, OLD Test only. Starting protected main/origin/main/local main `d823c2f80c1500990289b06da8e5504d7b48c2e5`. Dedicated clean worktree/branch isolates the existing dirty root and unmerged work; no stash/reset/clean. NEW and Production are excluded.
- Migration Allocation and Contract Artifact locks acquired before allocation. Source/runtime ledger 000076; new forward-only 000077 adds false-default booleans to Prize Master, Version relation and ownership Snapshot, extends immutable ownership trigger and rejects shipping-only exchange states at DB level. No business backfill, no existing migration rewrite. Only isolated synthetic test DB migrated before merge; shared Test migration is a separate activation step.
- Admin create/update/read/UI and formal Contract accept shipping_only without weakening unknown-field rejection or Revision OCC. Published economic-condition updates remain rejected. Version-to-ownership Snapshot is authoritative; current Master never decides acquired exchange rights. Public lineup uses Published Version, owned-prize uses acquired Snapshot.
- Canonical prizeAllowedActions blocks point_exchange with shipping_only after existing common reasons. Single/direct/mixed bulk requests reject before any exchange transition/grant/success outbox; existing transaction/lock/idempotency behavior retained. Draw selection, random source, coin consumption and inventory algorithm unchanged.
- Bounded expiration Command locks users/wallets/ownership, rechecks expiry and active holds, writes expired/terminal_at/history/system Audit only. No Point, Exchange Request, Inventory or automatic Shipping mutation. Active holds remain intact; after normal release to stored the next sweep converges. Existing Scheduler and unrelated Workers are not enabled.
- Contract-first source/generation: Public alpha.37, Admin alpha.35, Client/Testkit/bundle candidate alpha.41; Site Schema alpha.23 reference and Webhook alpha.34 unchanged. Immutable predecessor records preserved. Canonical publication/readback and subsequent ledger reconciliation follow merge; no generated-code hand edits.
- Focused isolated tests cover Snapshot, default compatibility, Master non-retroactivity, direct/bulk rejection, expiration boundary/idempotency/hold/no inventory return and concurrent mutation. Admin UI/Contract and existing Draw/Shipping regression verification are executed; exact final results and fixed-head Required Checks/self-review will be recorded on the PR before merge.
- OLD root capacity is insufficient for safe Image import; no existing artifact deletion is authorized or performed. Activation remains contingent on sufficient disk, current Compose/runtime recheck, one shared-DB migration, canonical amd64 Preview Artifact checksum/Image ID/OCI revision/merge-tree verification and health. Preserve old images until Human acceptance; after shipping-only configuration/issuance use forward recovery, not a simple old-API rollback.
- Storefront implementation and Human browser acceptance remain separate. Production/NEW, unrelated provider/auth/worker changes, algorithms and existing dirty files are out of scope.
- PR #510 initial full integration found a pg_dump/restore representation difference in the new CHECK's implicit varchar-array cast. Isolated backup/restore reproduced the exact one-line difference; 000077 now compares status::text explicitly so schema bytes round-trip without relaxing normalization or the constraint. Shared Test remains unmigrated. Final-head CI is rerun; early PR metadata failures were corrected without changing gates.

## REL-041 — Shipping-only immutable Contract reconciliation

- Issue none; Risk R3; Strict Change; Application Runtime Activation none. SHIPONLY-20260926 PR #510 final head b7075ceb780e104890739132b411e609013bd1a8 passed all five Required Checks, CodeQL/dependency review and fixed-head self-review before squash merge `e2b30805704ed5b9c3fd54492fb86f8b8549bde5`; reviewed and merged trees match. Existing dirty root remains untouched.
- Canonical Contract-only workflow 36223277643, immutable Artifact 10900150259, bundle/client/testkit alpha.41 and independent readback PASS. Outer digest `6d4cf321249d46145f22ce9551ba709a51dd59453ccbe0bd5004d0782d2dfb50`, manifest `b3c7d2a28c0c8332eeea90ac43876245baf4a1e4ce6b7d4f646124212d99f7ee`. Public alpha.37/Admin alpha.35; Webhook alpha.34/Site Schema reference alpha.23 unchanged. Preserve all predecessor records and clear candidate; no same-version republication.
- This separate canonical release-metadata PR changes only ledger, historical-state test fixtures, release runbook and Worklog. No application/schema/generated/ENV/runtime delta. Source migration 000077 and OLD Test activation remain distinct from artifact publication; no Production/NEW operations. Exact final-head checks, review, merge and cleanup are recorded on this PR.
- REL-041's first Quality check found the existing Production-source test assumed latest Artifact was the approved alpha.40. The fixture now selects alpha.40 from immutable history and retains its exact approved Source/Artifact/ancestry assertions. No Production approval is advanced; authority manifests/workflows/runtime remain untouched. This directly necessary sixth test-only path is included in the same PR and all head-dependent checks rerun.
