...
- Issue／Task Policy ceremonyはHuman判断どおり不採用。installed GitHub App wrapperのprivileged delivery hard requirementだけを満たすため、exact 15 paths・必要8 operations限定のroot-owned transient compatibility manifestをcommit外で発行し、merge／branch cleanup後に削除する。
