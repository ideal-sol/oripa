...
- Human subsequently designated the Human authenticated Browser as the authoritative OPS-020 Browser Acceptance environment. Cookie, storage-state, and CDP handoff to Codex are not required. Codex will not retry retained credentials or perform authentication changes; Payment navigation/default/filter, Rank Effect relation-section removal, and User List default/filter results remain pending the Human report.
...
- Required Human input is only the Human Browser result for the three named acceptance areas. No credential, cookie, storage state, or CDP session should be transmitted.
