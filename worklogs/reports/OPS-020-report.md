...
- Canonical build count `1`, dispatch count `1`, API activation count `1`, Admin activation count `1`, check rerun count `0`. The Task remains in progress, so final elapsed, Human wait, merge actor, squash SHA, and final cleanup metrics are not yet available.
