...
- Required Human input is a secure Browser-session handoff usable by the Codex Browser, such as a root-only Playwright storage state or an attached Browser/CDP session. Credential or cookie values must not be transmitted in Issue, PR, Worklog, or chat.
