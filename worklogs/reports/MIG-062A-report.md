...
- Artifact Versionは`2.0.0-alpha.3`。既存`2.0.0-alpha.1`／`.2`を上書きしない。配置先、Source Commit、Manifestおよび4 ArtifactのFile名／SHA-256、Workspace外Clean Install結果はArtifact作成後に確定する。
- Runtime、Preview、DB、Migration、Nginx、V1、Storefront Repositoryは変更しない。
- SITE-007はArtifact整合性とCloseout完了後に再開可能とする。
