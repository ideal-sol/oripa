<?php

namespace App\Http\Resources;

use Illuminate\Http\Request;
use Illuminate\Http\Resources\Json\JsonResource;

class UserResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            'id' => $this->id,
            'name' => $this->name,
            'email' => $this->email,
            'referral_code' => $this->referral_code,
            'line_link_code' => $this->line_link_code,
            'line_linked_at' => $this->line_linked_at?->toIso8601String(),
            'line_linked' => $this->line_linked_at !== null,
            'line_friend_add_url' => config('services.line.friend_add_url'),
            'status' => $this->status,
            'email_verified_at' => $this->email_verified_at?->toIso8601String(),
            'sms_verified_at' => $this->sms_verified_at?->toIso8601String(),
            'sms_verified' => $this->sms_verified_at !== null,
            'wallet' => new WalletResource($this->whenLoaded('wallet')),
            'profile' => new UserProfileResource($this->whenLoaded('profile')),
            'point_lots_count' => $this->whenCounted('pointLots'),
            'point_ledgers_count' => $this->whenCounted('pointLedgers'),
            'created_at' => $this->created_at?->toIso8601String(),
            'updated_at' => $this->updated_at?->toIso8601String(),
        ];
    }
}
