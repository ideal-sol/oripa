...
final class V2CatalogMasterMutationService
{
    ...
    public function updateGacha(
        ...
        return $this->execute(
            ...
            function () use ($context, $admin, $publicId, $payload): object {
                ...
                if ($this->isRestoredDraftPublication($row)) {
                    ...
                    if (
                        $payload['price_points'] !== (int) $version->price_points
                        || $payload['total_count'] !== (int) $version->total_count
                        || $payload['daily_draw_limit'] !==
                            (int) $version->daily_draw_limit
                        || $payload['audience_code'] !==
                            (string) $version->audience_code
                        || $payload['first_time_eligible_days'] !==
                            (int) $version->first_time_eligible_days
                        || $payload['allowed_draw_counts'] !==
                            $this->storedAllowedDrawCounts(
                                $version->allowed_draw_counts
                            )
                        || $requestedStart !== $immutableStart
                    ) {
                        throw new V2CatalogException(
                            'CATALOG_GACHA_POST_PUBLISH_FIELD_IMMUTABLE',
                            409,
                            'Published Gacha sale and economic fields are immutable.'
                        );
                    }
                }
                $canonicalSoldCount = $row->active_draw_state_id === null
                    ? 0
                    : (int) DB::table('gacha_draw_states')
                        ->where('id', $row->active_draw_state_id)
                        ->value('sold_count');
                if ($payload['total_count'] < $canonicalSoldCount) {
                    throw new V2CatalogException(
                ...
                $publishesDraft =
                    ...
                    && (
                        ...
                        || $this->isRestoredDraftPublication($row)
                    );
                if (
        ...
        return $this->execute(
            ...
            function () use (
                ...
                if (
                    ...
                    $this->updatePublishedPrizePresentation(
                        $gacha,
                        $version,
                        $prize,
                        $relation,
                ...
                if ($version->status !== 'draft' || $version->archived_at !== null) {
    ...
    public function scheduleGachaVersionPublish(
        ...
        return $this->execute(
            ...
            function () use (
                ...
                $gachaRevision = (int) $gacha->revision + 1;
                $versionRevision = (int) $version->revision + 1;
                DB::table('catalog_gachas')->where('id', $gacha->id)->update([
                    'management_status' => 'scheduled',
                    'first_published_at' => null,
                    'scheduled_start_at' => $payload['scheduled_for']->toIso8601String(),
                    'published_version_id' => null,
                    'active_draw_state_id' => null,
                    'revision' => $gachaRevision,
                    'updated_at' => DB::raw('CURRENT_TIMESTAMP'),
                ]);
                $publicId = (string) Str::uuid7();
        ...
        return $this->execute(
            ...
            function () use (
                ...
                if (
                    ...
                    || (string) $gacha->management_status !== 'scheduled'
                ) {
                    throw new V2CatalogException(
                        'CATALOG_GACHA_SCHEDULE_CONFLICT',
                        409,
                ...
                $this->cancelInitialScheduledPublication(
                    $context,
                    $admin,
                    $gacha
                );
                $probability = DB::table('catalog_probability_versions')
                    ...
                    ->firstOrFail();
                $cancelled = DB::table('catalog_gacha_publish_schedules')
                    ->where('id', $schedule->id)->firstOrFail();
                $canonicalGacha = DB::table('catalog_gachas')
                    ->where('id', $gacha->id)->firstOrFail();
                $canonicalDraft = DB::table('catalog_gacha_versions')
                    ->where('id', $version->id)->firstOrFail();
                ...
                return $this->mapPublishSchedule(
                    ...
                    $probability->public_id,
                    $probability->snapshot_sha256,
                    (int) $canonicalGacha->revision,
                    (int) $canonicalDraft->revision,
                    $context->requestId
                );
            },
            true,
    ...
    private function executeGachaUnpublish(
        ...
        return $this->execute(
            ...
            function () use (
                ...
                $this->closeActiveDrawState($gacha, 'superseded');
                DB::table('catalog_gachas')->where('id', $gacha->id)->update([
                    'published_version_id' => null,
                    'active_draw_state_id' => null,
    ...
    private function applyManagementStatus(
        ...
        if ($target === 'draft') {
            ...
            if ($current === 'unpublished') {
                $draft = DB::table('catalog_gacha_versions')
                    ->where('gacha_id', $gacha->id)
                    ->where('status', 'draft')
                    ->whereNull('archived_at')
                    ->orderByDesc('version_number')
                    ->lockForUpdate()
                    ->first();
                if ($draft === null && $gacha->first_published_at !== null) {
                    $source = DB::table('catalog_gacha_versions')
                        ->where('gacha_id', $gacha->id)
                    ...
                    $this->clonePublishedVersionForMasterEdit(
                        $gacha,
                        $source,
                        true
                    );
        ...
        if ($target === 'scheduled') {
            throw new V2CatalogException(
                'CATALOG_GACHA_MANAGEMENT_TRANSITION_INVALID',
                422,
                'Initial scheduled publication requires the Publish Schedule operation.'
            );
        }
        if ($target === 'published') {
            if ($current === 'unpublished') {
                throw new V2CatalogException(
                    'CATALOG_GACHA_MANAGEMENT_TRANSITION_INVALID',
                    422,
                    'An unpublished Gacha must return to Draft before publication.'
                );
            }
            if (
                ...
                && CarbonImmutable::parse((string) $gacha->first_published_at)
                    ->lessThanOrEqualTo($this->databaseNow())
                && $current !== 'sales_paused'
                && ! $this->isRestoredDraftPublication($gacha)
            ) {
                throw new V2CatalogException(
                    'CATALOG_GACHA_MANAGEMENT_TRANSITION_INVALID',
                    422,
                    'A Gacha that has been published cannot enter a new publication cycle.'
                );
            }
            $version = DB::table('catalog_gacha_versions')
                ->where('gacha_id', $gacha->id)
            ...
            if ($startsAt->greaterThan($now)) {
                throw new V2CatalogException(
                    'CATALOG_GACHA_MANAGEMENT_TRANSITION_INVALID',
                    422,
                    'Immediate publication requires a current start.'
                );
            }
            $restoresPublication = $this->isRestoredDraftPublication($gacha);
            DB::statement(
            ...
            if ($restoresPublication && (bool) $gacha->sales_paused) {
                DB::statement(
                    'SET CONSTRAINTS catalog_gachas_validate_activation IMMEDIATE'
                );
                DB::statement(
                ...
                DB::table('catalog_gachas')->where('id', $activated->id)->update([
                    'management_status' => 'published',
                    'sales_paused' => false,
                    'scheduled_start_at' => null,
                    'sales_resumed_at' => DB::raw('CURRENT_TIMESTAMP'),
                    'sales_last_mutation_request_id' => $context->requestId,
                    'revision' => (int) $activated->revision + 1,
                    'updated_at' => DB::raw('CURRENT_TIMESTAMP'),
                ]);
            }
            return;
        ...
        if ($target === 'sales_paused') {
            if (
                $current === 'scheduled'
                && CarbonImmutable::parse((string) $gacha->scheduled_start_at)
                    ->greaterThan($this->databaseNow())
            ) {
                throw new V2CatalogException(
                    'CATALOG_GACHA_MANAGEMENT_TRANSITION_INVALID',
                    422,
    ...
    private function usesCurrentPresentation(object $gacha): bool
    {
        $status = (string) ($gacha->management_status ?? 'draft');

        return $status !== 'draft'
            && ! ($status === 'unpublished' && $gacha->first_published_at === null)
            && ($gacha->first_published_at !== null
                || in_array($status, ['published', 'sales_paused'], true));
    }

    private function isRestoredDraftPublication(object $gacha): bool
    {
        return (string) ($gacha->management_status ?? 'draft') === 'draft'
            && $gacha->first_published_at !== null
            && $gacha->public_deactivated_at !== null
            && $gacha->published_version_id === null
            && $gacha->active_draw_state_id === null;
    }

    /** @param array<string, mixed> $payload */
    private function updateCurrentGachaPresentation(
        V2AdminAuthorizationContext $context,
        Admin $admin,
        object $gacha,
        string $gachaPublicId,
        ...
        if (! $payload['updates_draft']) {
            ...
            return $this->find('catalog_gachas', $gachaPublicId, false);
        }
        if ((int) $version->revision !== $payload['expected_version_revision']) {
            throw new V2CatalogException(
                'CATALOG_REVISION_CONFLICT',
                409,
                'The Published Catalog snapshot has changed.'
            );
        }
        ...
        $immutableCounts = $this->storedAllowedDrawCounts(
            $version->allowed_draw_counts
        );
        if (
            $payload['price_points'] !== (int) $version->price_points
            || $payload['total_count'] !== (int) $version->total_count
            || $payload['daily_draw_limit'] !== (int) $version->daily_draw_limit
            || $payload['audience_code'] !== (string) $version->audience_code
            || $payload['first_time_eligible_days'] !==
                (int) $version->first_time_eligible_days
            || $payload['allowed_draw_counts'] !== $immutableCounts
            || $requestedStart !== $immutableStart
        ) {
            throw new V2CatalogException(
                'CATALOG_GACHA_POST_PUBLISH_FIELD_IMMUTABLE',
        ...
        if (
            ...
            && CarbonImmutable::parse((string) $payload['publish_end_at'])
                ->lessThanOrEqualTo($storedStart)
        ) {
            throw $this->validationException();
        }
    ...
    private function cancelInitialScheduledPublication(
        V2AdminAuthorizationContext $context,
        Admin $admin,
        object $gacha
    ): void {
        ...
        $scheduledAt = $schedule === null
            ? null
            : CarbonImmutable::parse((string) $schedule->scheduled_for);
        if (
            $scheduledAt === null
            || $scheduledAt->lessThanOrEqualTo($this->databaseNow())
            || $gacha->first_published_at !== null
            || $gacha->published_version_id !== null
            || $gacha->active_draw_state_id !== null
        ) {
            throw new V2CatalogException(
                'CATALOG_GACHA_SCHEDULE_CONFLICT',
                409,
                'The initial publication schedule can no longer be cancelled.'
            );
        }
        DB::table('catalog_gacha_publish_schedules')
    ...
    private function closeActiveDrawState(object $gacha, string $reason): void
    {
    ...
    private function insertGachaDraft(
        ...
        if (array_sum(array_column($prizes, 'initial_inventory')) > $payload['total_count']) {
    ...
    private function clonePublishedVersionForMasterEdit(
        object $gacha,
        object $source,
        bool $useCurrentPresentation = false
    ): object
    {
        $assetId = $useCurrentPresentation
            ? $gacha->current_presentation_asset_id
            : $source->presentation_asset_id;
        $assetPublicId = $assetId === null ? null
            : DB::table('catalog_presentation_assets')
                ->where('id', $assetId)->value('public_id');
        $categoryId = $useCurrentPresentation
            ? $gacha->category_id
            : $source->category_id;
        $categoryPublicId = $categoryId === null
            ? DB::table('catalog_categories')->where('id', $gacha->category_id)->value('public_id')
            : DB::table('catalog_categories')->where('id', $categoryId)->value('public_id');
        $tagIds = $useCurrentPresentation ? []
            : DB::table('catalog_gacha_version_tags as relation')
                ->join('catalog_tags as tag', 'tag.id', '=', 'relation.tag_id')
                ->where('relation.gacha_version_id', $source->id)
        ...
        $draft = $this->insertGachaDraft($gacha, [
            ...
            'tag_ids' => $tagIds,
            'title' => $useCurrentPresentation
                ? $gacha->current_title
                : $source->title,
            'description' => $useCurrentPresentation
                ? $gacha->current_description
                : $source->description,
            'notices' => $useCurrentPresentation
                ? $gacha->current_notices
                : $source->notices,
            'price_points' => (int) $source->price_points,
            'total_count' => (int) $source->total_count,
            'daily_draw_limit' => (int) ($source->daily_draw_limit ?? 0),
            ...
            'publish_start_at' => (string) ($useCurrentPresentation
                ? $gacha->current_publish_start_at
                : $source->publish_start_at),
            'publish_end_at' => $useCurrentPresentation
                ? $gacha->current_publish_end_at
    ...
    private function assertBeforeFirstPublication(object $gacha): void
    {
        if ($gacha->first_published_at !== null) {
            throw new V2CatalogException(
                'CATALOG_GACHA_POST_PUBLISH_FIELD_IMMUTABLE',
                409,
                'Published Gacha sale, composition, and probability fields are immutable.'
            );
        }
    }

    private function synchronizePendingSchedule(
        int $gachaId,
        ?int $versionId = null,
        ...
        $gacha = DB::table('catalog_gachas')->where('id', $gachaId)
            ->lockForUpdate()->firstOrFail();
        $version = DB::table('catalog_gacha_versions')
            ->where('id', $versionId ?? $schedule->gacha_version_id)
            ->lockForUpdate()->firstOrFail();
        $effectiveStart = CarbonImmutable::parse(
            $scheduledFor ?? (string) $schedule->scheduled_for
        );
        if (
            $gacha->first_published_at !== null
            || (string) $gacha->management_status !== 'scheduled'
            || (int) $version->gacha_id !== $gachaId
            || $version->status !== 'draft'
            || $effectiveStart->lessThanOrEqualTo($this->databaseNow())
        ) {
            throw new V2CatalogException(
                'CATALOG_GACHA_SCHEDULE_CONFLICT',
                409,
    ...
    private function assertGachaInventoryCapacity(
        int $versionId,
        int $totalCount,
        ?int $excludedRelationId = null,
        ?int $prospectiveInventory = null,
        bool $updatesSnapshot = true
    ): void {
        $snapshot = DB::table('catalog_gacha_version_prizes')
            ->where('gacha_version_id', $versionId)
            ->when(
                $updatesSnapshot && $excludedRelationId !== null,
                fn ($query) => $query->where('id', '<>', $excludedRelationId)
            )
            ->sum('initial_inventory');
        $operational = DB::table('prize_inventories as inventory')
            ->join(
            ...
            ->when(
                ...
                fn ($query) => $query->where('relation.id', '<>', $excludedRelationId)
            )
    ...
    private function gachaInventoryCapacityException(): V2CatalogException
    {
        return new V2CatalogException(
            'CATALOG_GACHA_INVENTORY_TOTAL_CONFLICT',
            409,
    ...
    private function gachaPublishPreflight(
        ...
        if (
            ! in_array((string) $gacha->state, ['draft', 'active'], true)
            || $gacha->archived_at !== null
        ) {
            $block(
                'GACHA_MASTER_NOT_ACTIVE',
                'The Gacha Master must be a Draft or active and not archived.'
            );
        }
        if (
            (string) ($gacha->management_status ?? 'draft') === 'unpublished'
            || (
                $gacha->first_published_at !== null
                && ! $this->isRestoredDraftPublication($gacha)
            )
            ...
            $block(
                ...
                'A Gacha that has already been published cannot publish another Version.'
            );
        }
        if ($version->status !== 'draft') {
            $block(
    ...
    private function gachaSalesPreflightResult(
        ...
        if ($operation === 'pause') {
            ...
            $databaseNow = $this->databaseNow();
            $effectiveStart = $gacha->current_publish_start_at
                ?? $version?->publish_start_at;
            $effectiveEnd = $gacha->current_publish_end_at;
            if (
                $version !== null
                && (
                    CarbonImmutable::parse((string) $effectiveStart)
                        ->greaterThan($databaseNow)
                    || (
                        $effectiveEnd !== null
                        && CarbonImmutable::parse((string) $effectiveEnd)
                            ->lessThanOrEqualTo($databaseNow)
                    )
                )
            ) {
    ...
    private function schedulePreflightResult(
        ...
        if (
            $gacha->first_published_at !== null
            || (string) ($gacha->management_status ?? 'draft') !== 'draft'
        ) {
            $add(
                'GACHA_INITIAL_PUBLICATION_ALREADY_COMMITTED',
                'Only a Gacha that has never been published may be scheduled.'
            );
        }
    ...
    private function activateGachaVersion(
        ...
        $databaseNow = $this->databaseNow();
        $restoresPublication = $this->isRestoredDraftPublication($gacha);
        if ((string) ($gacha->management_status ?? 'draft') === 'unpublished') {
        ...
        DB::table('catalog_gachas')->where('id', $gacha->id)->update([
            ...
            'management_status' => 'published',
            'first_published_at' => $restoresPublication
                ? $gacha->first_published_at
            ...
            ...($restoresPublication ? [] : ['sold_count' => 0]),
            'revision' => (int) $gacha->revision + 1,
            'updated_at' => DB::raw('CURRENT_TIMESTAMP'),
        ]);
        $published = $this->find(
    ...
    private function mapGacha(object $row): array
    {
        ...
        $usesDraftVersion = (string) ($row->management_status ?? 'draft') === 'draft'
            || (
                $row->first_published_at === null
                && in_array(
                    (string) ($row->management_status ?? 'draft'),
        ...
        $currentVersion = $usesDraftVersion
            ? DB::table('catalog_gacha_versions')
                ->where('gacha_id', $row->id)
                ->where('status', 'draft')
                ->whereNull('archived_at')
