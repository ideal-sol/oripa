...
final class V2PaymentController
{
    ...
    public function startCardRegistration(Request $request): JsonResponse
    {
        return $this->handle($request, function () use ($request): array {
            if ($request->hasAny([
                'return_url',
                ...
                throw new V2FincodeException(
                    ...
                    422,
                    'Card registration return destinations are generated by the Platform.'
                );
            }
            $cardToken = $request->input('card_token');
            if (! is_string($cardToken) || $cardToken === '' || strlen($cardToken) > 512) {
                throw new V2FincodeException(
                    'CARD_REGISTRATION_REQUEST_INVALID',
                    422,
                    'The card registration request is invalid.'
                );
            }

            return $this->cards->startRegistration(
                $this->user(),
                $cardToken,
                $this->idempotencyKey($request)
            );
    ...
    public function showCardRegistration(Request $request, string $registrationId): JsonResponse
    {
        return $this->handle(
            $request,
            fn (): array => $this->cards->registration($this->user(), $registrationId)
        );
    }

    public function reconcileCardRegistration(Request $request, string $registrationId): JsonResponse
    {
        return $this->handle(
            $request,
            fn (): array => $this->cards->reconcileRegistration($this->user(), $registrationId)
        );
    }

    public function cancelCardRegistration(Request $request, string $registrationId): JsonResponse
    {
        return $this->handle(
            $request,
            fn (): array => $this->cards->cancelRegistration($this->user(), $registrationId)
        );
    }
