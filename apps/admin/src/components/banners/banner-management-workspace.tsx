            ...
            <table className="announcement-table">
              ...
              <tbody>{items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td><div className="announcement-form-actions"><code>{item.asset.public_url}</code><button aria-label={`${item.title}の画像URLをコピー`} className="icon-button" onClick={() => void navigator.clipboard.writeText(publicUrl(item.asset.public_url))} title="URLをコピー" type="button"><Clipboard aria-hidden="true" size={16} /></button><a aria-label={`${item.title}の画像を新しいタブで開く`} className="icon-button" href={item.asset.public_url} rel="noreferrer" target="_blank" title="画像を開く"><ExternalLink aria-hidden="true" size={16} /></a></div></td>
...
function publicUrl(path: string): string { return new URL(path, window.location.origin).toString(); }
