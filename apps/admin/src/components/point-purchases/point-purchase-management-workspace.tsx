...
function PlanList({ canManage, plans }: { canManage: boolean; plans: AdminPointPurchasePlan[] }) {
  ...
  return <div className="table-scroll point-purchase-table"><table><thead><tr>{["ID", "商品名", "支払金額", "有償P", "無償P", "販売期間", "並び順", "対象カテゴリ", "対象タグ", "状態", "編集"].map((label) => <th key={label} scope="col">{label}</th>)}</tr></thead><tbody>{plans.map((item) => <tr key={item.id}><td><code className="table-compact-id" title={item.id}>{`${item.id.slice(0, 8)}…${item.id.slice(-8)}`}</code></td><td>{item.name}</td><td>{yen(item.amount)}</td><td>{points(item.paid_point_amount)}</td><td>{points(item.free_point_amount)}</td><td>{period(item)}</td><td>{item.sort_order}</td><td>{audience(item.audience_code)}</td><td>{targetTag(item)}</td><td><span className={`status-pill ${item.is_active ? "status-active" : "status-muted"}`}>{item.is_active ? "有効" : "無効"}</span></td><td>{canManage ? <Link aria-label={`${item.name}を編集`} className="secondary-button compact-button" href={`/purchase-plans/${item.id}`}>編集</Link> : "閲覧のみ"}</td></tr>)}</tbody></table></div>;
}
