...
class StorefrontContractArtifactTest(unittest.TestCase):
    ...
    def next_candidate_governance(self):
        ...
        packages["@oripa/storefront-client"].update(
            {"version": "2.0.0-alpha.29", "disposition": "publish"}
        )
        ...
        packages["@oripa/storefront-testkit"].update(
            {
                ...
                "disposition": "publish",
                "storefront_client_version": "2.0.0-alpha.29",
            }
        )
        packages["@oripa/storefront-testkit"].pop("sha256")
        value["candidate"] = {
            "release_state": "pending",
            "bundle_version": "2.0.0-alpha.29",
    ...
    def test_pending_alpha_29_bundle_preserves_released_alpha_28(self):
        ...
        self.assertEqual(value["candidate"]["bundle_version"], "2.0.0-alpha.29")
    ...
    def test_latest_alpha_28_evidence_must_match_immutable_history(self):
        value = copy.deepcopy(self.governance())
    ...
    def test_pending_alpha_29_is_publishable_candidate(self):
        candidate = artifact.pending_candidate(ROOT)
        self.assertEqual(candidate["bundle_version"], "2.0.0-alpha.29")
        self.assertEqual(candidate["predecessor_bundle_version"], "2.0.0-alpha.28")
