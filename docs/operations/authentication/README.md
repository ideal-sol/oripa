...
本AlphaはProduction Deployment不可であり、MIG-042のAudit／Outbox永続化接続後に
改めてRelease Gateを評価する。
...
Register、Verification、Login、Logout、MFA、Recovery Code、Rate LimitのEventは
`V2SecurityEventSink`へ送る。Password、Token、Session ID、MFA Secret、Recovery
Code、Full EmailをEventへ含めない。MIG-042までは永続Audit Tableを作らず、
Productionでは永続Sink未接続をFail Closedにする。
